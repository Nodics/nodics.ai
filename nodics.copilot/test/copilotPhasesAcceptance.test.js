...
test("Phase 0-2: module identity, request context, and provider safety are explicit", async () => {
  ...
  const requestService = load(
    "copilotCore/src/service/defaultCopilotRequestService",
  );
  assert.throws(
    () => requestService.normalize({ prompt: "help" }),
    /COPILOT_REQUEST_CONTEXT_REQUIRED/,
  );
  assert.equal(
    requestService.normalize({ tenant: "t1", actor: "a1", prompt: " help " })
      .prompt,
    "help",
  );
  const providerService = load(
    "copilotProviders/modules/copilotProvider/src/service/defaultCopilotProviderService",
  );
  const providerConfiguration = load(
    "copilotProviders/modules/copilotProvider/config/properties",
  ).copilot.providers;
  assert.throws(
    () =>
    ...
    /COPILOT_PROVIDERS_DISABLED/,
  );
  const mock = load("copilotProviders/modules/mockProvider/config/properties")
    .copilot.providers.adapters.mock;
  const enabledConfiguration = Object.assign({}, providerConfiguration, {
    enabled: true,
    adapters: Object.assign({}, providerConfiguration.adapters, { mock: mock }),
  });
  const response = await providerService.invoke(
    { messages: [{ role: "user", content: "hello" }], mockResponse: "ok" },
    {
      configuration: enabledConfiguration,
      services: {
        DefaultCopilotMockProviderAdapterService: load(
          "copilotProviders/modules/mockProvider/src/service/defaultCopilotMockProviderAdapterService",
        ),
      },
    },
  );
  ...
  const ollama = load(
    "copilotProviders/modules/ollamaProvider/config/properties",
  ).copilot.providers.adapters.ollama;
  const switched = Object.assign({}, enabledConfiguration, {
    default: Object.assign({}, enabledConfiguration.default, {
      adapter: "ollama",
    }),
    adapters: Object.assign({}, enabledConfiguration.adapters, {
      ollama: Object.assign({}, ollama, { enabled: true }),
    }),
  });
  const local = await providerService.invoke(
    { messages: [{ role: "user", content: "hello" }] },
    {
      configuration: switched,
      services: {
        DefaultOllamaCopilotProviderAdapterService: {
          invoke: () => ({ model: "qwen2.5-coder:7b", content: "local" }),
        },
      },
    },
  );
...
test("mutation intent is intercepted for clarification before provider generation", () => {
  ...
  const decision = requestService.assessMutationIntent(
    "Create 10 iPhone Pro Max products.",
  );
  ...
  const natural =
    "Create 10 iPhone Pro Max products, code prefix IPM, catalog version electronics-staged, price book uae-retail, currency AED, price 4999";
  assert.equal(requestService.assessMutationIntent(natural).ambiguous, false);
  assert.deepEqual(requestService.parseProductCreateIntent(natural), {
    count: 10,
    name: "iPhone Pro Max",
    codePrefix: "IPM",
    catalogVersion: "electronics-staged",
    priceBookCode: "uae-retail",
    currency: "AED",
    price: "4999",
  });
  assert.equal(
    requestService.assessMutationIntent("Explain how products work").mutation,
    false,
  );
});
...
test("read intent routes live module questions and exports without confusing documentation help", () => {
  ...
  assert.deepEqual(
    requestService.assessReadIntent(
      "How many modules are available in Nodics?",
    ),
    {
      type: "LIVE_READ",
      operation: "framework.modules.count",
      format: undefined,
    },
  );
  assert.deepEqual(
    requestService.assessReadIntent(
      "Export the available Nodics modules to CSV",
    ),
    {
      type: "EXPORT",
      operation: "framework.modules.list",
      format: "csv",
    },
  );
  assert.deepEqual(
...
test("Phase 3-4: conversation records are tenant bound and API routes are secured", () => {
  const conversations = load(
    "copilotConversation/src/service/defaultCopilotConversationService",
  );
  const record = conversations.createRecord(
    { tenant: "t1", actor: "a1", channel: "axis" },
    "c1",
  );
  assert.equal(
    conversations.createMessage(record, "user", "hello", 1).tenant,
    "t1",
  );
  const routes = load("copilotApi/src/router/routers");
  assert.equal(routes.copilotApi.conversations.create.secured, true);
  assert.equal(
    routes.copilotApi.turns.submit.permission,
    "copilot.assistant.use",
  );
  assert.equal(
    routes.copilotApi.turns.stream.responseHandler,
    "copilotSseResponseHandler",
  );
});
...
test("confirmed mutations expose one execution transport and preserve execution authorization", async () => {
  ...
  assert.equal(
    definitions.filter((route) => route.operation === "executeConfirmation")
      .length,
    1,
  );
  assert.equal(
    definitions.some(
      (route) =>
        route.operation === "executeProductPlan" ||
        route.key.includes("/workbench/"),
    ),
    false,
  );
  assert.equal(routes.productPlans.prepareProducts.key, "/products/prepare");
  assert.equal(
    routes.confirmations.execute.permission,
    "copilot.mutation.execute",
  );
  assert.equal(routes.confirmations.execute.secured, true);
  assert.equal(
    load("copilotApi/src/controller/defaultCopilotController")
      .executeProductPlan,
    undefined,
  );
  assert.equal(
    load("copilotApi/src/facade/defaultCopilotFacade").executeProductPlan,
    undefined,
  );
  const orchestration = load(
  ...
  await assert.rejects(
    runtime.executeConfirmation({
      confirmationCode: "approved-action",
      tenant: "tenant-one",
    }),
...
test("conversation titles describe the first request without exposing internal identifiers", async () => {
  ...
  await conversations.acceptTurn(
    conversation,
    Object.assign({}, request, {
      idempotencyKey: "descriptive-title-1",
      message:
        "  Explain   how Nodics framework modules work for a new developer  ",
    }),
    configuration,
  );
  assert.equal(
  ...
  assert.equal(
    (await conversations.listOwned(request, configuration)).items[0].title,
    conversation.title,
  );
...
test("Axis capability is module owned and API responses use the standard data envelope", async () => {
  ...
  try {
    ...
    assert.deepEqual(await facade.createConversation({}), {
      code: "SUC_SYS_00000",
      data: { conversation: { conversationCode: "c1" } },
    });
  } finally {
...
test("Axis turns retrieve authorized evidence before invoking the provider and preserve citations", async () => {
  ...
  global.CONFIG = {
    get: () => ({
      core: {
        ...
        systemPrompt: "Nodics system prompt",
      },
      api: { enabled: true },
      conversation: {
        storage: "VOLATILE_LOCAL",
      ...
      knowledge: { retrieval: { enabled: true, defaultSize: 5 } },
      providers: { enabled: true },
    }),
  };
  global.SERVICE = {
    DefaultCopilotConversationService: conversations,
    DefaultCopilotRequestService: load(
      "copilotCore/src/service/defaultCopilotRequestService",
    ),
        ...
        evidence: [
          {
            id: "source-1",
            title: "Nodics Framework",
            excerpt: "Nodics is a modular enterprise application framework.",
          },
        ],
        citations: [citation],
    ...
    DefaultCopilotProviderService: {
      invoke: async (request) => {
        ...
        return {
          content: "Nodics is a modular framework [source-1].",
          usage: {},
  ...
  try {
    ...
    const result = await orchestration.submitTurn(
      Object.assign({}, identity, {
        conversationCode: created.conversation.conversationCode,
        idempotencyKey: "grounded-turn-1",
        message: "What is Nodics?",
      }),
    );
    assert.match(
...
test("Phase 5-7: evidence stays cited and export is bounded", () => {
  const knowledge = load(
    "copilotKnowledge/src/service/defaultCopilotKnowledgeService",
  );
  ...
  const capability = load(
    "copilotCapability/src/service/defaultCopilotCapabilityService",
  );
  assert.match(
    capability.renderExport([{ code: "p1", name: "Phone" }], "csv", {}).content,
    /"p1","Phone"/,
  );
  assert.throws(
    () => capability.renderExport([{}, {}], "csv", { maximumRows: 1 }),
    /COPILOT_EXPORT_LIMIT_EXCEEDED/,
  );
  assert.equal(
    capability.renderExport([], "xlsx", {}).delegate,
    "nExport/excelExport",
  );
});
...
test("Phase 8-9: workbench clarifies, validates, confirms, then calls an API", async () => {
  const workbench = load(
    "copilotWorkbench/src/service/defaultCopilotWorkbenchService",
  );
  assert.deepEqual(
    workbench.prepareProducts({ count: 10 }).state,
    "CLARIFICATION_REQUIRED",
  );
  const prepared = workbench.prepareProducts({
    planId: "plan-1",
    count: 10,
    name: "iPhone Pro Max",
    codePrefix: "IPM",
    catalogVersion: "electronics-online",
    priceBookCode: "uae-retail",
    currency: "AED",
    price: 4999,
  });
  ...
  await assert.rejects(
    workbench.execute(
      validated,
      Object.assign({}, challenge, { confirmed: false }),
      context,
      () => true,
      policy,
    ),
    /COPILOT_MUTATION_CONFIRMATION_REQUIRED/,
  );
  await assert.rejects(
    workbench.execute(
      validated,
      Object.assign({}, challenge, { confirmed: true }),
      {
        tenant: "default",
        actor: "other",
        permissions: ["copilot.mutation.execute"],
      },
      () => true,
      policy,
    ),
    /COPILOT_MUTATION_CONTEXT_MISMATCH/,
  );
  const executed = await workbench.execute(
    validated,
    Object.assign({}, challenge, { confirmed: true }),
    context,
    (payload) => ({ accepted: payload.records.length }),
    policy,
  );
  ...
  global.SERVICE = {
    DefaultModuleService: {
      invokeModule: (options) => {
        invocation = options;
        return Promise.resolve({ code: "SUC_DBS_00000" });
      },
    },
  };
  try {
...
test("Phase 10-12: policy is fail closed and evaluation exposes review state", () => {
  const policy = load("copilotPolicy/src/service/defaultCopilotPolicyService");
  assert.equal(
    policy.assessAmbiguity({ required: ["catalog"], values: {} }).ambiguous,
    true,
  );
  assert.throws(
    () => policy.authorizeExecution({ confirmed: false }, {}),
    /COPILOT_MUTATION_CONFIRMATION_REQUIRED/,
  );
  const evaluation = load(
    "copilotEvaluation/src/service/defaultCopilotEvaluationService",
  );
  assert.equal(
    evaluation.evaluate({
      code: "e1",
  ...
  const experience = load(
    "copilotCapability/src/service/defaultCopilotExperienceCapabilityService",
  );
  const descriptors = experience.descriptors();
  assert.equal(
    descriptors.find((item) => item.code === "experience.issue.assist")
      .maturity,
    "ADAPTER_REQUIRED",
  );
  assert.equal(
    descriptors.find((item) => item.code === "commerce.checkout.execute")
      .mutates,
    true,
  );
});
...
test("later modules can override focused methods without copying services", () => {
  ...
  const customized = Object.assign({}, base, {
    normalize: (request) =>
      Object.assign({}, base.normalize(request), { channel: "partner" }),
  });
  assert.equal(
    customized.normalize({ tenant: "t1", actor: "a1", prompt: "help" }).channel,
    "partner",
  );
});
