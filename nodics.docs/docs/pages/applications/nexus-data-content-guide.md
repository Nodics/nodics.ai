...
Nexus is a reusable corporate website accelerator under `nodics.accelerators/modules/nexus`. It is a
...
the accelerator content pack provides the reference release package.
...
`sample-v001` is accelerator-owned reference content governed by immutable
manifest sections. Customer extensions use their own release boundaries. Inside it, `content` separates CMS, Media, Editorial, and
...
publication manifest, Staged source record, and accelerator or customer release file.

## Customization and extension guidance

A project can customize Nexus in its own later-index content module, extending
`nexus` and declaring its own immutable releases, adding new headers for additional schemas, adding new
...
`nexus` is composition-only. `nexusCore` owns reusable administration and tooling
descriptors; `nexus.web` owns reference configuration, content, physical assets and
release tests. The independent `nodics.nexus` frontend remains the renderer.
The source move preserves the `nexus.web` identity and every data manifest/checksum.

BackOffice media steps declare `manifestModule: "nexus.web"` and a module-relative
manifest path. The registry supplies the actual root, and real-path containment
rejects traversal and symlink escapes. Application Builder discovers the opted-in
framework pack with source provenance and rejects duplicate customer copies.
Configuration and catalogue discovery do not import or publish any content.

Run `nodics nexus:check` for content and checksum validation. Qualify actual
import, media transfer and Staged-to-Online publication separately after deployment.
