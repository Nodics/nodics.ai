...
// Default coverage belongs to this framework checkout. Other owners must select
// their source, catalogue and output together; sibling presence never selects scope.
...
const selectedRoots = process.argv
    .slice(2)
    .filter((value) => value.startsWith('--source-root='))
    .map((value) => resolve(value.slice(14)));
const customScope = selectedRoots.length > 0;
...
if (
    ...
    throw new Error('Selected project coverage cannot overwrite framework artifacts');
const reportJsonPath = resolve(outputRoot, 'source-backed-documentation-coverage-report.json');
const reportMarkdownPath = resolve(outputRoot, 'source-backed-documentation-coverage-report.md');
...
const scanRoots = customScope
    ? selectedRoots
    : readdirSync(frameworkRoot, { withFileTypes: true })
          .filter(
              (entry) =>
                  entry.isDirectory() &&
                  existsSync(resolve(frameworkRoot, entry.name, 'package.json')) &&
                  readJson(resolve(frameworkRoot, entry.name, 'package.json')).nodics,
          )
          .map((entry) => resolve(frameworkRoot, entry.name))
          .sort();
if (scanRoots.some((folder) => !existsSync(folder)))
    throw new Error('A selected source coverage root is unavailable');
const frameworkDocumentationBacklog = [
    {
        priority: 'P0',
        item: 'Axis setup and user-safe error contracts',
        classification: 'needs-deeper-section',
        sourceAreas: ['nodics.platform/modules/backoffice', 'nodics.platform/modules/axis'],
    ...
    },
    {
        ...
        classification: 'needs-page-or-deeper-section',
        sourceAreas: [
            'nodics.localization/modules/localizationCore',
            'nodics.localization/modules/localizationApi',
        ],
    ...
    },
    {
        ...
        classification: 'needs-page-or-owner-mapping',
        sourceAreas: [
            'nodics.commerce/modules/baseCommerce/modules/shoppingList',
    ...
    },
    {
        ...
        classification: 'needs-page-or-owner-mapping',
        sourceAreas: [
            'nodics.foundation/modules/nCache',
            'nodics.foundation/modules/nCache/redisCache',
            'nodics.foundation/modules/nCache/hazelcastCache',
        ],
    ...
    },
    {
        ...
        classification: 'needs-page-or-deeper-section',
        sourceAreas: [
            'nodics.communication/modules/smtpCommsProvider',
            'nodics.communication/modules/smsCommsProvider',
        ],
...
const documentationBacklog = customScope
...
function collectSignals(entry) {
    walk(entry.path, (file) => {
        ...
        if (relativeFile.includes('/src/service/') && relativeFile.endsWith('.js'))
            addExample(entry, 'services', file);
        if (relativeFile.includes('/src/controller/') && relativeFile.endsWith('.js'))
            addExample(entry, 'controllers', file);
        if (relativeFile.includes('/src/router/') && relativeFile.endsWith('.js'))
            addExample(entry, 'routers', file);
        if (/\/data\/[^/]+\/(?:[^/]+\/)?headers\/.+\.js$/.test(relativeFile))
            addExample(entry, 'dataHeaders', file);
        if (/\/data\/[^/]+\/(?:[^/]+\/)?records\/.+\.js$/.test(relativeFile))
            addExample(entry, 'dataRecords', file);
        if (
            relativeFile.includes('/data/') &&
            (relativeFile.includes('/assets/') || relativeFile.endsWith('/assetManifest.js'))
        )
            addExample(entry, 'assets', file);
        if (relativeFile.includes('/test/') && file.endsWith('.js')) addExample(entry, 'tests', file);
        if (
            /\/src\/App\.(jsx|tsx|js|ts)$/.test(relativeFile) ||
            /\/src\/main\.(jsx|tsx|js|ts)$/.test(relativeFile)
        )
            addExample(entry, 'frontends', file);
    });
}
...
function documentMatches(entry, document) {
    ...
    if (
        document.evidencePaths.some((evidence) => evidence === entry.path || evidence.startsWith(entryPath))
    ) {
        return true;
    }
...
function markdown(reportValue) {
    ...
    reportValue.documentationBacklog.forEach((item) => {
        lines.push(
            `| ${item.priority} | ${item.status} | ${item.item} | ${item.classification} | ${item.sourceAreas.map((source) => `\`${source}\``).join('<br/>')} | ${item.action} |`,
        );
    });
    lines.push(
        '',
    ...
    topGaps.forEach((row) => {
        const signals = Object.entries(row.counts)
            ...
            .join(', ');
        lines.push(
            `| ${row.classification} | ${row.priorityScore} | \`${row.path}\` | ${row.matchedDocuments.join(', ') || 'None'} | ${signals || 'package only'} |`,
        );
    });
    lines.push(
        '',
