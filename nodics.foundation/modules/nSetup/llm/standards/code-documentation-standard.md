...
Do not hardcode project-specific route lists in documentation tooling. A project module must be able to extend schemas, routers, and controllers, then regenerate docs without changing Nodics foundation code.
...
Root `docs/` is excluded from source documentation coverage. It may contain local reference material, migration notes, captured documentation content, or planning files; those files are not framework source and must not distort module documentation coverage. A module keeps one concise `README.md`; detailed permanent human documentation belongs in the canonical documentation content pack, while cross-cutting developer and AI contracts belong under `modules/nSetup/llm`.
...
```bash
modules/nTooling/config/properties.js#tooling.documentationGovernance
```
