...
Framework and product code must not depend on a customer-project name.
...
Runtime selection must explicitly resolve a project environment and server;
framework logic must not hardcode a customer server name.
