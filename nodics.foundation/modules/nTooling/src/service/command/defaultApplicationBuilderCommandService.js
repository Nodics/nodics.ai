...
module.exports = {
  ...
  loadSolution: function (args) {
    ...
    if (!configuredPath) {
      throw new Error(
        "Application Builder command requires --solution=/absolute/or/relative/solution.json",
      );
    }
    const solutionPath = path.resolve(configuredPath);
    if (!fs.existsSync(solutionPath) || !fs.statSync(solutionPath).isFile()) {
      throw new Error(
        "Application Builder solution file is unavailable: " + solutionPath,
      );
    }
  ...
  loadPlan: function (args) {
    ...
    if (!configuredPath) {
      throw new Error(
        "Application Builder command requires --plan=/absolute/or/relative/generation-plan.json",
      );
    }
    const planPath = path.resolve(configuredPath);
    if (!fs.existsSync(planPath) || !fs.statSync(planPath).isFile()) {
      throw new Error(
        "Application Builder plan file is unavailable: " + planPath,
      );
    }
  ...
  loadLock: function (args) {
    ...
    if (!configuredPath) {
      throw new Error(
        "Application Builder upgrade command requires --current-lock=/absolute/or/relative/solution-lock.json",
      );
    }
    const lockPath = path.resolve(configuredPath);
    if (!fs.existsSync(lockPath) || !fs.statSync(lockPath).isFile()) {
      throw new Error(
        "Application Builder current lock file is unavailable: " + lockPath,
      );
    }
  ...
  loadReleaseManifest: function (args) {
    ...
    if (!configuredPath) {
      throw new Error(
        "Application Builder upgrade command requires --release=/absolute/or/relative/release-manifest.json",
      );
    }
    const manifestPath = path.resolve(configuredPath);
    if (!fs.existsSync(manifestPath) || !fs.statSync(manifestPath).isFile()) {
      throw new Error(
        "Application Builder release manifest is unavailable: " + manifestPath,
      );
    }
  ...
  loadGuidedAnswers: function (args) {
    ...
    if (!configuredPath) {
      throw new Error(
        "Application Builder guided command requires --answers=/absolute/or/relative/answers.json",
      );
    }
    const answersPath = path.resolve(configuredPath);
    if (!fs.existsSync(answersPath) || !fs.statSync(answersPath).isFile()) {
      throw new Error(
        "Application Builder guided answers file is unavailable: " +
          answersPath,
      );
    }
  ...
  readAnswerTemplateOptions: function (args) {
    return {
      ...
      preset: this.readOption(args, "--preset", ""),
      country: this.readOption(args, "--country", ""),
      locale: this.readOption(args, "--locale", ""),
      currency: this.readOption(args, "--currency", ""),
      frontends: this.readCsvOption(args, "--frontends", []).map((frontend) =>
  ...
  workflow: function (operation) {
    const steps = [
      {
        ...
        command: "builder:answers-template --output=/path/to/answers.json",
        purpose: "Create editable beginner answers.",
      },
      {
        step: 2,
        operation: "dry-run",
        command: "builder:dry-run --answers=/path/to/answers.json",
        purpose:
          "Review selected capabilities, frontends, renderers, and data packs without writing an application.",
      },
      {
        step: 3,
        operation: "guide",
        command:
          "builder:guide --answers=/path/to/answers.json --workspace=/path/to/review",
      ...
      },
      {
        ...
        operation: "approve",
        command:
          "builder:approve --plan=/path/to/generation-plan.json --approval-reference=CHANGE-ID",
      ...
      },
      {
        ...
        operation: "generate",
        command:
          "builder:generate --solution=/path/to/solution.json --plan=/path/to/approved-plan.json --output=/absent/output/root",
      ...
      },
      {
        ...
        operation: "qualify",
        command:
          "builder:qualify --solution=/path/to/solution.json --plan=/path/to/approved-plan.json --output=/generated/root",
        purpose:
          "Run generated tests, runtime probes, evidence gates, and lock update.",
      },
    ];
    ...
    return {
      ...
      recommendedNext:
        current && current.step < steps.length ? steps[current.step] : null,
      steps: steps,
    };
  },
  ...
  repositoryCoordinates: function (context) {
    return {
      ...
      frontend: this.readOption(
        context.args,
        "--frontend",
        this.readOption(context.args, "--agora", ""),
      ),
      customer: this.readOption(
        context.args,
        "--customer",
        this.readOption(context.args, "--kickoff", ""),
      ),
      experience: this.readOption(
        context.args,
        "--experience",
        this.readOption(context.args, "--exp", ""),
      ),
      frontendCode: this.readOption(context.args, "--frontend-code", ""),
    };
  },
  ...
  resolveCatalogueService: function (context) {
    return toolingCommandService.loadMergedService(
      context.home,
      context.command.catalogueService ||
        "defaultApplicationBuilderCatalogueService",
    );
  },
  ...
  resolvePlanningService: function (context) {
    return toolingCommandService.loadMergedService(
      context.home,
      context.command.planningService ||
        "defaultApplicationBuilderPlanningService",
    );
  },
  ...
  resolveGenerationService: function (context) {
    return toolingCommandService.loadMergedService(
      context.home,
      context.command.generationService ||
        "defaultApplicationBuilderGenerationService",
    );
  },
  ...
  resolveQualificationService: function (context) {
    return toolingCommandService.loadMergedService(
      context.home,
      context.command.qualificationService ||
        "defaultApplicationBuilderQualificationService",
    );
  },
  ...
  resolveUpgradeService: function (context) {
    return toolingCommandService.loadMergedService(
      context.home,
      context.command.upgradeService ||
        "defaultApplicationBuilderUpgradeService",
    );
  },
  ...
  run: async function (context) {
    const catalogue = this.resolveCatalogueService(context).discover(
      this.repositoryCoordinates(context),
    );
    let result;
    if (context.command.operation === "discover") {
      ...
      const questionnaireOptions = {
        defaults: this.readAnswerTemplateOptions(context.args),
        catalogue,
      };
      if (process.stdin && process.stdin.isTTY !== true) {
        questionnaireOptions.scriptedAnswers = fs
          .readFileSync(0, "utf8")
          .split(/\r?\n/);
      }
      const answers =
        await guidedService.runQuestionnaire(questionnaireOptions);
      result = {
        contractVersion: 0,
      ...
      if (outputPath) {
        result.artifact = guidedService.writeAnswersTemplate(
          outputPath,
          answers,
          {
            protectedRoots: catalogue.repositories.map(
        ...
        result.nextCommand =
          "builder:dry-run --answers=" + result.artifact.path;
      }
      ...
      const answers = guidedService.createAnswersTemplate(
        this.readAnswerTemplateOptions(context.args),
        catalogue,
      );
      result = {
        contractVersion: 0,
      ...
      result = this.resolveGuidedService(context).guide(
        this.loadGuidedAnswers(context.args),
        catalogue,
        {
      ...
      result = this.resolveGuidedService(context).dryRun(
        this.loadGuidedAnswers(context.args),
        catalogue,
      );
    } else if (context.command.operation === "approve") {
      const approvalReference = this.readOption(
        context.args,
        "--approval-reference",
        "",
      );
      result = this.resolveGenerationService(context).approvePlan(
        this.loadPlan(context.args),
        approvalReference,
      );
    } else if (context.command.operation === "generate") {
      const outputRoot = this.readOption(context.args, "--output", "");
      result = this.resolveGenerationService(context).generate(
        this.loadPlan(context.args),
        this.loadSolution(context.args),
        catalogue,
        outputRoot,
        {
          protectedRoots: catalogue.repositories.map(
      ...
      const outputRoot = this.readOption(context.args, "--output", "");
      const referenceEvidencePath = this.readOption(
        context.args,
        "--reference-evidence",
        "",
      );
      result = this.resolveQualificationService(context).qualify(
        this.loadPlan(context.args),
        this.loadSolution(context.args),
        catalogue,
        outputRoot,
        {
          ...
          executeCommands:
            this.readOption(context.args, "--execute-commands", "true") !==
            "false",
        },
      );
    } else if (context.command.operation === "release-manifest") {
      result = this.resolveUpgradeService(context).createReleaseManifest(
        this.loadSolution(context.args),
        this.loadPlan(context.args),
        catalogue,
        {
          releaseChannel: this.readOption(
            context.args,
            "--release-channel",
            "LOCAL_BUILDER",
          ),
          signer: this.readOption(
            context.args,
            "--signer",
            "nodics.local.builder",
          ),
        },
      );
    } else if (context.command.operation === "upgrade-plan") {
      result = this.resolveUpgradeService(context).createUpgradePlan(
        this.loadLock(context.args),
        this.loadReleaseManifest(context.args),
        this.loadPlan(context.args),
        this.loadSolution(context.args),
        catalogue,
      );
      ...
      if (context.command.operation === "validate") {
        ...
        throw new Error(
          "Unsupported Application Builder command operation: " +
