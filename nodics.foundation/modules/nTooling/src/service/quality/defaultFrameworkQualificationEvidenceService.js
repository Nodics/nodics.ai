...
module.exports = {
    ...
    /** Ensures qualification evidence is executed from the nodics.ai framework root. */
    assertFrameworkRoot: function () {
        const packagePath = path.join(this.frameworkRoot, 'package.json');
        if (!fs.existsSync(packagePath)) {
            throw new Error('Framework qualification must run from nodics.ai root: ' + this.frameworkRoot);
        }
        const packageJson = JSON.parse(fs.readFileSync(packagePath, 'utf8'));
        if (packageJson.name !== 'nodics.ai') {
            throw new Error('Framework qualification resolved non-framework home: ' + this.frameworkRoot);
        }
    },

    /** Runs one qualification contract file and returns process result plus duration. */
    runContract: function (file, options = {}) {
    ...
    /** Runs automated security-boundary qualification evidence contracts. */
    runSecurityBoundary: function () {
        const cases = [
    ...
    runPublishingSoak: function () {
        ...
        for (let iteration = 1; iteration <= iterations; iteration += 1) {
            for (const file of cases) {
                const { result } = this.runContract(file);
                if (result.status !== 0) {
