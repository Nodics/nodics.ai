...
const globalGuidanceRoot = 'nodics.foundation/modules/nSetup/llm';
...
const requiredRootFiles = [
    ...
    '.cursor/rules/nodics-foundation.mdc',
    globalGuidanceIndex,
...
module.exports = exportedService = {
    ...
    return Array.from(new Set(directories)).sort((left, right) =>
        (this.toRelative || exportedService.toRelative).call(this, left)
            .localeCompare((this.toRelative || exportedService.toRelative).call(this, right)));
},
    ...
    /**
     * Determines whether a package explicitly owns canonical documentation.
     *
     * @param {string} directory Absolute package directory.
     * @returns {boolean} True when package metadata declares documentation ownership.
     * @throws {Error} Propagates malformed package metadata so governance fails closed.
     * @override Later tooling layers may strengthen documentation-owner metadata without
     * weakening the explicit ownership requirement.
     */
    ownsDocumentation: function (directory) {
    let packageJson = JSON.parse(fs.readFileSync(path.join(directory, 'package.json'), 'utf8'));
    let declaredOwnership = packageJson.nodics && packageJson.nodics.owns;
    return Array.isArray(declaredOwnership) && declaredOwnership.includes('documentation');
},
    ...
    (this.walk || exportedService.walk).call(this, rootPath, (entryPath, entry) => {
        if (entry.isFile() && entry.name === 'readme.md') {
            (this.fail || exportedService.fail).call(this, failures, 'Lowercase readme.md is not allowed: ' +
                (this.toRelative || exportedService.toRelative).call(this, entryPath));
        }
    });
},
