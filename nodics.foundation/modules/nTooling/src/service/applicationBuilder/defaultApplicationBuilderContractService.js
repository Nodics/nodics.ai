...
module.exports = {
  ...
  resolveReference: function (rootSchema, reference) {
    if (!reference.startsWith("#/")) {
      throw new Error(
        "Only local Application Builder schema references are supported: " +
          reference,
      );
    }
    return reference
      .slice(2)
      .split("/")
      .reduce((value, segment) => value && value[segment], rootSchema);
  },
  ...
  validateValue: function (
    ...
    if (schema.$ref) {
      ...
      if (!resolved) {
        errors.push(
          location + " references an unknown schema fragment: " + schema.$ref,
        );
        return errors;
      }
    ...
    if (schema.type) {
      ...
      if (!validType) {
        errors.push(
          location + " must be " + schema.type + ", received " + actualType,
        );
        return errors;
      }
    }
  ...
  validateSecretFree: function (value, location = "$", errors = []) {
    const forbidden =
      /^(?:password|secret|secretValue|token|apiKey|privateKey|credential|credentials)$/i;
    if (Array.isArray(value)) {
      value.forEach((item, index) =>
        this.validateSecretFree(item, location + "[" + index + "]", errors),
      );
        ...
        if (forbidden.test(key)) {
          errors.push(
            location +
              "." +
              key +
              " contains forbidden secret material; use a secretReference",
          );
        }
