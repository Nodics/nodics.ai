...
module.exports = {
  ...
  resolveCapabilities: function (selected, catalogue) {
    ...
    const visit = (code) => {
      ...
      if (!capability) {
        errors.push(
          "Selected or required capability is not available: " + code,
        );
        return;
      }
      visiting.add(code);
    ...
    return {
      capabilities: Array.from(resolved.values()).sort((left, right) =>
  ...
  validateSemantics: function (solution, catalogue, resolution) {
    ...
    const resolved = new Set(
      resolution.capabilities.map((capability) => capability.code),
    );
    ...
    const composition = (catalogue.frontendCompositions || []).find(
      (candidate) =>
        candidate.code === solution.experience.composition &&
        candidate.frontend === solution.experience.storefront,
    );
    ...
    resolved.forEach((code) => {
      if (excluded.has(code)) {
        errors.push(
          "Excluded capability is required by the resolved graph: " + code,
        );
      }
    });
    ...
    if (!solution.topology.frontends.includes(solution.experience.storefront)) {
      errors.push("Selected storefront must be present in frontend topology");
    }
    if (!composition) {
      errors.push(
        "Requested frontend composition is unavailable: " +
      ...
      (solution.experience.rendererKeys || []).forEach((rendererKey) => {
        if (!(composition.rendererKeys || []).includes(rendererKey)) {
          errors.push(
            "Requested renderer is unavailable in the frontend composition: " +
              rendererKey,
          );
        }
      });
    }
    const domainCodes = (solution.capabilities.domains || []).map((domain) =>
      domain.toLowerCase(),
    );
    domainCodes.forEach((domainCode) => {
      if (!resolved.has(domainCode)) {
        errors.push(
          "Selected domain is absent from the backend capability graph: " +
            domainCode,
        );
      }
    });
    if (composition) {
      ...
      if (JSON.stringify(activeDomains) !== JSON.stringify(selectedDomains)) {
        errors.push(
          "frontend composition active domains do not match selected domains",
        );
      }
    }
    ...
    const frontendLookup = new Map(
      (catalogue.frontendApps || []).map((app) => [app.code, app]),
    );
    solution.topology.frontends.forEach((code) => {
      if (!frontendLookup.has(code))
        errors.push("Selected frontend is unavailable: " + code);
    });
    const requiredPacks = new Set(
      [].concat(
        composition?.dataPacks || [],
        solution.topology.frontends.flatMap(
          (code) => frontendLookup.get(code)?.dataPacks || [],
        ),
      ),
    );
    requiredPacks.forEach((code) => {
      if (!solution.data.packs.includes(code))
        errors.push("Selected experience requires customer data pack: " + code);
    });
    solution.data.packs.forEach((code) => {
      if (!requiredPacks.has(code))
        errors.push(
          "Customer data pack is not required by the selected experience: " +
            code,
        );
    });
    Object.entries(solution.experience.rendererByDomain || {}).forEach(
      ([domain, renderer]) => {
        if (
          !domainCodes.includes(domain) ||
          !solution.experience.rendererKeys.includes(renderer)
        ) {
          errors.push(
            "Domain renderer must belong to the selected domain and renderer set: " +
              domain,
          );
        }
      },
  ...
  validateSolution: function (solution, catalogue) {
    ...
    if (!structural.valid) {
      return {
        ...
        resolution: { capabilities: [], edges: [], errors: [] },
      };
    }
    const resolution = this.resolveCapabilities(
      solution.capabilities.selected,
      catalogue,
    );
  ...
  buildOperations: function (solution, resolution, sourceDigest) {
    ...
    add(
      "CREATE_FILE",
      "BACKEND",
      "backend/test/api-contract.test.js",
      "GENERATED",
      "ABSENT",
    );
    add(
      "CREATE_FILE",
      "BACKEND",
      "backend/test/runtime-contract.test.js",
      "GENERATED",
      "ABSENT",
    );
    add(
    ...
    add(
      "CREATE_FILE",
      "STOREFRONT",
      "storefront/test/domain-rendering-contract.test.js",
      "GENERATED",
      "ABSENT",
    );
    add(
      "CREATE_FILE",
      "STOREFRONT",
      "storefront/test/storefront-contract.test.js",
      "GENERATED",
      "ABSENT",
    );
    add(
      "CREATE_FILE",
      "STOREFRONT",
      "storefront/generated/frontend-composition.ts",
      "GENERATED",
      "ABSENT",
    );
    (solution.data.packs || [])
      ...
      .forEach((dataPack) => {
        ...
        add(
          "CREATE_FILE",
          "DATA",
          "data/" + dataPack + "/components/hero.json",
          "CUSTOMER_OWNED",
          "ABSENT",
        );
        add(
          "CREATE_FILE",
          "DATA",
          "data/" + dataPack + "/content/sections.json",
          "CUSTOMER_OWNED",
          "ABSENT",
        );
        add(
          "CREATE_FILE",
          "DATA",
          "data/" + dataPack + "/products/starter-products.json",
          "CUSTOMER_OWNED",
          "ABSENT",
        );
        add(
          "CREATE_FILE",
          "DATA",
          "data/" + dataPack + "/prices/starter-prices.json",
          "CUSTOMER_OWNED",
          "ABSENT",
        );
        add(
          "CREATE_FILE",
          "DATA",
          "data/" + dataPack + "/inventory/starter-inventory.json",
          "CUSTOMER_OWNED",
          "ABSENT",
        );
      });
    ...
    add(
      "CREATE_FILE",
      "EVIDENCE",
      "integrations/frontend-wiring.json",
      "GENERATED",
      "ABSENT",
    );
    (solution.topology.frontends || [])
      .filter((code) => code !== solution.experience.storefront)
      .forEach((code) => {
        add(
          "CREATE_FILE",
          "EVIDENCE",
          "integrations/" + code.toLowerCase() + "-wiring.md",
          "GENERATED",
          "ABSENT",
        );
      });
    ...
    add(
      "CREATE_FILE",
      "EVIDENCE",
      "generation-and-validation-report.md",
      "GENERATED",
      "ABSENT",
    );
    resolution.capabilities.forEach((capability) => {
      add(
        "REGISTER_DEPENDENCY",
        "BACKEND",
        "backend/generated/dependencies/" + capability.code + ".json",
        "GENERATED",
        "ABSENT",
      );
    });
  ...
  createPlan: function (solution, catalogue, options = {}) {
    ...
    if (!validation.valid) {
      throw new Error(
        "Application Builder solution is invalid:\n- " +
    ...
    const planSeed = catalogueService.digest({
    ...
    const plan = {
      ...
      planId:
        "builder-plan-" +
        planSeed.slice("sha256:".length, "sha256:".length + 32),
      createdAt: createdAt,
      expiresAt: expiresAt,
      solutionDigest: solutionDigest,
      catalogueDigest: catalogue.catalogueDigest,
      ...
      resolvedCapabilities: validation.resolution.capabilities.map(
        (capability) => ({
          code: capability.code,
          version: capability.version,
          descriptorDigest: capability.metadataDigest,
          selectionReason: explicit.has(capability.code)
            ? "EXPLICIT"
            : "EXTENDED",
        }),
      ),
      backendGraph: {
        nodes: validation.resolution.capabilities.map(
      ...
      frontendGraph: {
        nodes: [
          solution.experience.storefront.toLowerCase() +
            "." +
            solution.experience.composition,
        ]
          .concat(solution.experience.rendererKeys)
          .sort(),
      ...
      excludedCapabilityAssertions: (solution.capabilities.excluded || [])
        .slice()
        .sort(),
      qualification: {
        commands: [
    ...
    if (!structural.valid) {
      throw new Error(
        "Application Builder produced an invalid plan:\n- " +
