...
module.exports = {
  ...
  approvePlan: function (
    ...
    if (!structural.valid) {
      throw new Error(
        "Cannot approve invalid Builder plan:\n- " +
    ...
    if (!approvalReference || !String(approvalReference).trim()) {
      throw new Error(
        "Builder approval requires a non-empty approval reference",
      );
    }
    ...
    const approvedStructural = contractService.validateDocument(
      "plan",
      approved,
    );
    if (!approvedStructural.valid) {
      throw new Error(
        "Builder approval produced an invalid plan:\n- " +
  ...
  validateApprovedPlan: function (
    plan,
    solution,
    catalogue,
    now = new Date().toISOString(),
  ) {
    const errors = contractService
      .validateDocument("plan", plan)
      .errors.slice();
    ...
    if (plan?.catalogueDigest !== catalogue.catalogueDigest) {
      errors.push(
        "Approved Builder plan does not match the current capability catalogue",
      );
    }
    ...
    try {
      ...
      if (this.approvalDigest(canonical) !== this.approvalDigest(plan)) {
        errors.push(
          "Approved Builder plan differs from the current canonical plan",
        );
      }
    } catch (error) {
  ...
  resolveOutputRoot: function (configuredRoot, protectedRoots = []) {
    if (!configuredRoot || !path.isAbsolute(configuredRoot)) {
      throw new Error(
        "Builder generation requires --output as an absolute path",
      );
    }
    ...
    if (forbidden.has(requestedRoot)) {
      throw new Error(
        "Builder generation output is a protected root: " + requestedRoot,
      );
    }
    if (fs.existsSync(requestedRoot)) {
      throw new Error(
        "Builder generation output must be absent: " + requestedRoot,
      );
    }
    ...
    if (!fs.existsSync(ancestor) || !fs.statSync(ancestor).isDirectory()) {
      throw new Error(
        "Builder generation output has no available parent directory",
      );
    }
    if (fs.lstatSync(ancestor).isSymbolicLink()) {
      throw new Error(
        "Builder generation rejects symlinked output ancestors: " + ancestor,
      );
    }
    const realAncestor = fs.realpathSync(ancestor);
    const outputRoot = path.resolve(
      realAncestor,
      path.relative(ancestor, requestedRoot),
    );
    return outputRoot;
  },
  ...
  starterProducts: function (solution) {
    if (solution.data.sampleData !== true) return [];
    const domains = (solution.capabilities.domains || []).map((domain) =>
      domain.toLowerCase(),
    );
    const products = [];
    if (domains.length === 0) {
      products.push({
        ...
        price: { amount: 49, currency: solution.commerce.currencies[0] },
        attributes: {
          catalogue: solution.commerce.catalogs[0],
          stockStatus: "in-stock",
        },
        rendererKey:
          solution.experience.rendererByDomain?.commerce ||
          solution.experience.rendererKeys[0],
      });
    }
    if (domains.includes("apparel")) {
      products.push({
        ...
        price: { amount: 79, currency: solution.commerce.currencies[0] },
        attributes: {
        ...
        rendererKey:
          solution.experience.rendererByDomain?.apparel ||
          solution.experience.rendererKeys[0],
      });
    }
    if (domains.includes("electronics")) {
      products.push({
        ...
        price: { amount: 499, currency: solution.commerce.currencies[0] },
        attributes: {
          brand: "Customer electronics",
          warrantyMonths: 24,
          specs: ["OLED display", "128GB storage"],
        },
        rendererKey:
          solution.experience.rendererByDomain?.electronics ||
          solution.experience.rendererKeys[0],
      });
    }
    if (domains.includes("telco")) {
      products.push({
        ...
        price: {
          amount: 125,
          currency: solution.commerce.currencies[0],
          recurring: "MONTHLY",
        },
        attributes: {
          ...
          dataAllowance: "25GB",
          minutes: "unlimited",
          provisioning: "SIM_OR_ESIM",
        },
        rendererKey:
          solution.experience.rendererByDomain?.telco ||
          solution.experience.rendererKeys[0],
      });
    }
    return products;
  ...
  frontendWiring: function (solution) {
    return {
      contractVersion: 1,
      orchestration:
        "Frontend repositories and deployment endpoints are selected by the customer workspace.",
      generatedApps: Object.fromEntries(
        solution.topology.frontends.map((code) => [
          code.toLowerCase(),
          {
            selected: true,
            code: code,
            role:
              code === solution.experience.storefront
                ? "starter storefront"
                : "Selected supporting frontend",
          },
        ]),
      ),
      extensionRoots: this.customizationRoots(solution),
    };
  },

  /** Returns a markdown list for the generated starter products. */
  starterProductLines: function (solution) {
    return this.starterProducts(solution)
      .map(
        (product) =>
          "- `" +
          product.code +
  ...
  customizationRoots: function (solution) {
    return ["backend/customer", "storefront/customer"].concat(
      (solution.data.packs || [])
        .slice()
        .sort()
        .map((dataPack) => "data/" + dataPack),
    );
  },

  /** Returns the domain represented by a generated data pack. */
  dataPackDomain: function (dataPack, solution) {
    return solution.data.packDomains?.[dataPack] || "content";
  },

  /** Returns starter products that belong in a customer-owned data pack. */
  dataPackProducts: function (solution, dataPack) {
    const domain = this.dataPackDomain(dataPack, solution);
    if (domain === "content") {
      return [];
    }
    return this.starterProducts(solution).filter(
  ...
  dataPackDocument: function (solution, dataPack, documentPath) {
    ...
    if (documentPath === "manifest.json") {
      return Object.assign({}, base, {
        description:
          domain === "content"
            ? "Corporate website starter content data pack"
    ...
    if (documentPath === "pages/home.json") {
      return Object.assign({}, base, {
        ...
        route:
          domain === "content" ? "/" : domain === "common" ? "/" : "/" + domain,
        title:
          domain === "content"
            ? "Welcome to " + projectCode
            : domain === "common"
              ? "Shop " + projectCode
              : "Shop " + domain,
    ...
    if (documentPath === "components/hero.json") {
      return Object.assign({}, base, {
        ...
        headline:
          domain === "content"
            ? "Customer website starter"
            : domain === "common"
              ? "Commerce starter experience"
              : "Curated " + domain + " experience",
        cta: domain === "content" ? "Explore company" : "Shop now",
        rendererHint: solution.experience.rendererKeys[0],
      });
    }
    if (documentPath === "content/sections.json") {
      return Object.assign({}, base, {
        sections: [
          {
            code: "hero",
            title: domain === "content" ? "Corporate story" : "Storefront hero",
          },
          {
            code: "featured-products",
            title: products.length ? "Featured " + domain : "Featured content",
          },
    ...
    if (documentPath === "inventory/starter-inventory.json") {
      return Object.assign({}, base, {
        inventory: products.map((product) => ({
          productCode: product.code,
          stockStatus:
            product.domain === "telco"
              ? "available-with-provisioning"
              : "in-stock",
    ...
    throw new Error(
      "Unknown generated data-pack document: " + dataPack + "/" + documentPath,
    );
  },

  /** Returns starter data-pack summaries for runtime and docs. */
  dataPackSummaries: function (solution) {
    return (solution.data.packs || [])
      .slice()
      .sort()
      .map((dataPack) => ({
        dataPack: dataPack,
        domain: this.dataPackDomain(dataPack, solution),
  ...
  firstThirtyMinutesGuide: function (solution, plan) {
    ...
    return (
      ...
      "This starts the generated backend and storefront on ephemeral local ports and probes health, composition, products, and the Selected page.\n\n" +
      "## 10-20 minutes: understand what was selected\n\n" +
      "- Project: `" +
      projectCode +
      "`\n" +
      "- Backend capabilities: `" +
      plan.backendGraph.nodes.join("`, `") +
      "`\n" +
      "- Frontends: `" +
      solution.topology.frontends.join("`, `") +
      "`\n" +
      "- Active domains: `" +
      (solution.capabilities.domains.length
        ? solution.capabilities.domains.join("`, `")
        : "BASE_COMMERCE") +
      "`\n" +
      "- Data packs: `" +
      solution.data.packs.slice().sort().join("`, `") +
      "`\n\n" +
  ...
  apiCatalogueGuide: function (solution) {
    return (
      ...
      this.dataPackSummaries(solution)
        .map(
          (summary) =>
            "- `" +
            summary.root +
      ...
      "\n\n" +
      "## Domain notes\n\n" +
      "- Starter products use selected domain and renderer metadata; extend customer data with business attributes.\n" +
  ...
  frontendGuide: function (solution) {
    return (
      "# Frontend guide\n\n" +
      "The generated storefront is an Selected starter, intentionally small enough to inspect in one sitting.\n\n" +
      "## Selected renderer hierarchy\n\n" +
      "- Composition: `frontend." +
      solution.experience.composition +
      "`\n" +
      "- Renderer keys: `" +
      solution.experience.rendererKeys.join("`, `") +
      "`\n\n" +
      ...
      "- `storefront/generated/frontend-composition.ts`: generated composition snapshot for the selected hierarchy.\n" +
  ...
  customizationMap: function (solution) {
    return (
      ...
      this.customizationRoots(solution)
        .map((root) => "- `" + root + "`")
        .join("\n") +
      "\n\n" +
      ...
      "- Selected frontend applications source remains in their own repositories.\n" +
  ...
  renderFile: function (relativePath, solution, plan, catalogue) {
    ...
    const capability = (code) =>
      plan.resolvedCapabilities.find((item) => item.code === code);
        ...
        JSON.stringify(
          {
            ...
            ownership: {
              ...
              projectData:
                "Customer-owned product, content, page, component, price, inventory, and catalogue data belongs in customer data packs.",
              frontend:
                "Customer workspace tooling may orchestrate frontends; Selected frontend applications remain separate application repositories.",
              generatedRoot:
                "Generated files may be regenerated from the approved plan; customer-owned roots are extension points.",
            },
            nextCommands: [
              "npm test",
              "npm run verify:runtime",
    ...
    if (relativePath === "README.md") {
      return (
        ...
        "It is intentionally small: it references the selected framework/domain/frontend/data boundaries and gives a new team a runnable backend, starter storefront, wiring notes, and local deployment draft.\n\n" +
        "## What was selected\n\n" +
        "- Project code: `" +
        projectCode +
        "`\n" +
        ...
        "- Customer workspace tooling can orchestrate frontends, but it does not own selected frontend source source.\n\n" +
        "## Handoff file\n\n" +
        ...
        "- Storefront renders domain-specific Selected product cards from selected renderer keys.\n" +
        "- `integrations/` explains selected frontend wiring without copying those repositories.\n" +
    ...
    if (relativePath === "verify-generated.js") {
      const expected = plan.operations
        .filter((operation) => operation.operation !== "CREATE_DIRECTORY")
      ...
      return (
        ...
        "if (handoff.project.projectCode !== '" +
        projectCode +
        "') throw new Error('Builder handoff project identity mismatch');\n" +
        "if (!handoff.ownership.framework.includes('nodics.ai')) throw new Error('Builder handoff ownership boundary is incomplete');\n" +
        "const graph = JSON.parse(fs.readFileSync(path.join(__dirname, 'backend/generated/" +
        projectCode +
        "-module-graph.json'), 'utf8'));\n" +
        ...
        "console.log('Generated Nodics application verified: " +
        projectCode +
        "');\n"
      );
    }
    if (relativePath === "verify-runtime.js") {
      return (
        ...
        '    if (composition.statusCode !== 200 || !composition.body.includes(\'"projectCode":"' +
        projectCode +
        "\"')) throw new Error('Backend composition probe failed');\n" +
        ...
        "    if (page.statusCode !== 200 || !page.body.includes('data-frontend-composition=\"true\"')) throw new Error('Storefront page probe failed');\n" +
        "    if (!page.body.includes('data-pack-summary=\"true\"')) throw new Error('Storefront data-pack summary probe failed');\n" +
        "    console.log('Generated Nodics runtime verified: " +
        projectCode +
        "');\n" +
        "  } finally {\n" +
    ...
    if (
      ...
      relativePath === "storefront/package.json"
    ) {
      const area = relativePath.split("/")[0];
      return (
        JSON.stringify(
          {
            name: projectCode + "-" + area,
            version: "0.0.0",
            private: true,
            scripts: {
              ...
              test:
                area === "backend"
                  ? "node test/runtime-contract.test.js && node test/api-contract.test.js"
                  : "node test/storefront-contract.test.js && node test/domain-rendering-contract.test.js",
            },
          },
          null,
          2,
    ...
    if (relativePath === "backend/src/starterData.js") {
      return (
        ...
        ";\n\n" +
        "const products = " +
        JSON.stringify(this.starterProducts(solution), null, 2) +
        ";\n\n" +
        "const dataPackSamples = " +
        JSON.stringify(this.dataPackSummaries(solution), null, 2) +
        ";\n\n" +
    ...
    if (relativePath === "backend/test/runtime-contract.test.js") {
      return (
        ...
        "assert.strictEqual(backend.composition.projectCode, '" +
        projectCode +
        "', 'Backend composition must preserve project identity');\n" +
        "assert.deepStrictEqual(backend.composition.backendGraph.nodes, " +
        JSON.stringify(plan.backendGraph.nodes) +
        ", 'Backend graph must match the approved plan');\n" +
        "assert.strictEqual(backend.composition.security.authorization, 'BACKEND_OWNED', 'Backend must own authorization');\n" +
        "console.log('Generated backend contract validated: " +
        projectCode +
        "');\n"
      );
    }
    if (relativePath === "backend/test/api-contract.test.js") {
      const expectedDomains = Array.from(
        new Set(
          this.starterProducts(solution).map((product) => product.domain),
        ),
      ).sort();
      return (
        "const assert = require('assert');\n" +
        ...
        "assert.strictEqual(backend.starterData.products.length, " +
        this.starterProducts(solution).length +
        ", 'Generated backend must respect sample-data selection');\n" +
        "assert.deepStrictEqual(backend.starterData.dataPackSamples.map(item => item.dataPack).sort(), " +
        JSON.stringify(solution.data.packs.slice().sort()) +
        ", 'Generated backend must expose selected data-pack samples');\n" +
        "assert.deepStrictEqual(Array.from(new Set(backend.starterData.products.map(product => product.domain))).sort(), " +
        JSON.stringify(expectedDomains) +
        ", 'Starter products must match selected domains');\n" +
        ...
        "});\n" +
        "console.log('Generated backend API depth validated: " +
        projectCode +
        "');\n"
      );
    }
    if (
      relativePath ===
      "backend/generated/" + projectCode + "-module-graph.json"
    ) {
      return (
        JSON.stringify(
          {
            contractVersion: 1,
            projectCode: projectCode,
            capabilities: plan.backendGraph.nodes,
            edges: plan.backendGraph.edges,
            runtimeRoles: solution.topology.backendRuntimes,
          },
          null,
          2,
        ) + "\n"
    ...
    if (
      relativePath.startsWith("backend/generated/dependencies/") &&
      ...
      if (!resolved) {
        throw new Error(
          "Planned dependency has no resolved capability: " + code,
        );
      }
      return JSON.stringify(resolved, null, 2) + "\n";
    }
    if (relativePath === "storefront/generated/frontend-composition.ts") {
      const content = {
        contractVersion: 1,
        projectCode: projectCode,
    ...
    if (relativePath === "storefront/index.html") {
      return (
        ...
        "</title>\n" +
        "</head>\n<body>\n" +
        '  <main id="root" data-frontend-composition="true"></main>\n' +
    ...
    if (relativePath === "storefront/src/renderers.js") {
      return (
        ...
        JSON.stringify(
          Object.fromEntries(
            solution.experience.rendererKeys.map((key) => [
              key,
              "Selected product renderer: " + key,
            ]),
          ),
        ) +
        ";\n\n" +
    ...
    if (relativePath === "storefront/src/pages.js") {
      return (
        ...
        '  return \'<!doctype html><html lang="\' + composition.locale + \'"><head><meta charset="utf-8"><title>' +
        displayName +
        "</title></head>' +\n" +
        '    \'<body><main id="root" data-frontend-composition="true"><section class="hero"><h1>' +
        displayName +
        "</h1>' +\n" +
        "    '<p>Selected ' + composition.composition + ' storefront for ' + composition.market + '</p></section>' +\n" +
    ...
    if (relativePath === "storefront/src/server.js") {
      return (
        ...
        "  createServer().listen(port, '127.0.0.1', () => console.log('Generated storefront ready on http://127.0.0.1:' + port));\n" +
        "}\n\n" +
    ...
    if (relativePath === "storefront/test/storefront-contract.test.js") {
      return (
        ...
        "assert.strictEqual(storefront.composition.projectCode, '" +
        projectCode +
        "', 'Storefront composition must preserve project identity');\n" +
        "assert.deepStrictEqual(storefront.composition.rendererKeys, " +
        JSON.stringify(solution.experience.rendererKeys) +
        ", 'Storefront renderers must match selected hierarchy');\n" +
        "assert(storefront.page().includes('data-frontend-composition=\"true\"'), 'Storefront page must expose the frontend composition marker');\n" +
        "assert(storefront.page().includes('data-pack-summary=\"true\"'), 'Storefront page must expose customer data-pack summaries');\n" +
        "console.log('Generated storefront contract validated: " +
        projectCode +
        "');\n"
      );
    }
    if (relativePath === "storefront/test/domain-rendering-contract.test.js") {
      return (
        ...
        "  if (" +
        JSON.stringify(solution.data.sampleData === true) +
        ...
        "console.log('Generated domain rendering depth validated: " +
        projectCode +
        "');\n"
      );
    }
    ...
    if (relativePath.startsWith("data/")) {
      ...
      return (
        JSON.stringify(
          this.dataPackDocument(solution, match[1], match[2]),
          null,
          2,
        ) + "\n"
      );
    ...
    const selectedFrontend = solution.topology.frontends.find(
      (code) =>
        relativePath === "integrations/" + code.toLowerCase() + "-wiring.md",
    );
    if (selectedFrontend) {
      return (
        "# " +
        selectedFrontend +
        " wiring\n\n" +
        "Configure the selected frontend repository with this customer backend endpoint and authentication policy.\n" +
        "Frontend source and production deployment remain owned by the selected repository.\n"
      );
    }
    if (relativePath === "deployment/README.md") {
      return (
        "# Local deployment draft\n\n" +
        "This folder gives a beginner-friendly local packaging sketch for the generated backend and starter storefront.\n\n" +
        ...
        "- Selected frontend applications repositories remain independently managed; this generated project only records how they are expected to connect.\n"
      );
    }
    ...
    if (relativePath === "generation-and-validation-report.md") {
      return (
        ...
        "`\n" +
        "- Approval: `" +
        plan.approval.approvalReference +
        "`\n" +
    ...
    throw new Error(
      "Builder has no deterministic template for planned file: " + relativePath,
    );
  },
  ...
  generate: function (plan, solution, catalogue, configuredRoot, options = {}) {
    const validation = this.validateApprovedPlan(
      plan,
      solution,
      catalogue,
      options.now,
    ...
    if (!validation.valid) {
      throw new Error(
        "Builder generation validation failed:\n- " +
    ...
    try {
      ...
      plan.operations.forEach((operation, index) => {
        ...
        if (
          ...
          throw new Error(
            "Planned target escapes generation root: " + operation.targetPath,
          );
        }
      ...
      const lock = {
        ...
        builder: {
          engineVersion: "0.1.0",
          contractVersion: 1,
          mode: solution.environment.mode,
        },
        packages: plan.resolvedCapabilities.map((item) => ({
          code: item.code,
          version: item.version,
          digest: item.descriptorDigest,
          source: solution.environment.mode,
        })),
        templates: [
          {
            code: "builder.minimal",
            version: "0.0.0",
            digest: catalogueService.digest("builder.minimal@0.1.0"),
            source: solution.environment.mode,
          },
        ],
        ...
        effectiveGraphs: {
          backend: {
            nodes: plan.backendGraph.nodes,
            digest: catalogueService.digest(plan.backendGraph),
          },
          frontend: {
            nodes: plan.frontendGraph.nodes,
            digest: catalogueService.digest(plan.frontendGraph),
          },
        },
        dataPacks: solution.data.packs
          .slice()
        ...
        qualification: {
          ...
          evidenceDigest: catalogueService.digest(plan.qualification),
          gates: plan.qualification.expectedEvidence,
      ...
      if (!lockValidation.valid) {
        throw new Error(
          "Builder produced an invalid solution lock:\n- " +
            lockValidation.errors.join("\n- "),
        );
      }
      fs.writeFileSync(
        path.join(outputRoot, "solution-lock.json"),
