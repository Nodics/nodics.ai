...
module.exports = {
  ...
  resolveGeneratedRoot: function (configuredRoot) {
    if (!configuredRoot || !path.isAbsolute(configuredRoot)) {
      throw new Error(
        "Builder qualification requires --output as an absolute path",
      );
    }
    const outputRoot = path.resolve(configuredRoot);
    if (!fs.existsSync(outputRoot) || !fs.statSync(outputRoot).isDirectory()) {
      throw new Error(
        "Builder qualification output root is unavailable: " + outputRoot,
      );
    }
    return outputRoot;
  },
  ...
  loadLock: function (outputRoot) {
    ...
    if (!fs.existsSync(lockPath) || !fs.statSync(lockPath).isFile()) {
      throw new Error(
        "Builder qualification requires a generated solution-lock.json",
      );
    }
    ...
    if (!validation.valid) {
      throw new Error(
        "Generated solution lock is invalid:\n- " +
  ...
  validateBinding: function (lock, solution, plan, catalogue) {
    ...
    if (errors.length > 0) {
      throw new Error(
        "Builder qualification binding failed:\n- " + errors.join("\n- "),
      );
    }
  },
  ...
  readGeneratedBackendGraph: function (outputRoot, solution) {
    ...
    if (!fs.existsSync(graphPath)) {
      throw new Error(
        "Generated backend graph is missing: " +
          path.relative(outputRoot, graphPath),
      );
    }
    return JSON.parse(fs.readFileSync(graphPath, "utf8"));
  },

  /** Reads the generated frontend composition object from the TypeScript artifact. */
  readGeneratedComposition: function (outputRoot) {
    const compositionPath = path.join(
      outputRoot,
      "storefront",
      "generated",
      "frontend-composition.ts",
    );
    if (!fs.existsSync(compositionPath)) {
      throw new Error(
        "Generated frontend composition is missing: storefront/generated/frontend-composition.ts",
      );
    }
    ...
    if (!match) {
      throw new Error(
        "Generated frontend composition does not contain a parseable composition object",
      );
    }
  ...
  qualifyBackendGraph: function (outputRoot, solution, plan, lock) {
    ...
    if (
      JSON.stringify(actual.nodes) !==
        JSON.stringify(plan.backendGraph.nodes) ||
      JSON.stringify(actual.edges) !==
        JSON.stringify(plan.backendGraph.edges) ||
      catalogueService.digest(plan.backendGraph) !==
        lock.effectiveGraphs.backend.digest
    ) {
  ...
  qualifyFrontendComposition: function (outputRoot, solution, plan, lock) {
    ...
    if (
      composition.composition !== solution.experience.composition ||
      JSON.stringify(composition.rendererKeys) !==
        JSON.stringify(solution.experience.rendererKeys) ||
      catalogueService.digest(plan.frontendGraph) !==
        lock.effectiveGraphs.frontend.digest
    ) {
      return this.evidence(
        "frontend.composition",
        "ARTIFACT",
        "FAILED",
        "Generated frontend composition differs from the selected solution or lock.",
      );
    }
    return this.evidence(
      "frontend.composition",
      "ARTIFACT",
      "PASSED",
      "Generated frontend composition matches the selected experience and frontend graph.",
    );
  },
  ...
  qualifyCustomerOwnership: function (outputRoot, solution) {
    ...
    const missingOrPopulated = requiredDirectories.filter((relativePath) => {
      ...
      return (
        ...
        (relativePath.endsWith("/customer") &&
    ...
    if (missingOrPopulated.length > 0) {
      return this.evidence(
        ...
        "ARTIFACT",
        "FAILED",
        "Customer-owned generated roots are missing or pre-populated: " +
          missingOrPopulated.join(", "),
      );
    }
    return this.evidence(
      "customer.ownership",
  ...
  qualifySecurityBoundary: function (solution) {
    ...
    if (
      !validation.valid ||
      solution.security.authorization !== "BACKEND_OWNED"
    ) {
      return this.evidence(
        "security.boundary",
        "SEMANTIC",
  ...
  qualifyGeneratedHandoff: function (outputRoot, solution, plan) {
    ...
    try {
      ...
      if (
        handoff.project.projectCode !== solution.identity.projectCode ||
        JSON.stringify(handoff.selected.backendCapabilities) !==
          JSON.stringify(plan.backendGraph.nodes) ||
        JSON.stringify(handoff.selected.dataPacks) !==
  ...
  qualifyGeneratedSelfTest: function (outputRoot, executeCommands) {
    ...
    try {
      ...
      return this.evidence(
        ...
        "COMMAND",
        "PASSED",
        "Generated self-test completed successfully.",
        { command: command },
      );
    } catch (error) {
      return this.evidence(
  ...
  qualifyGeneratedRuntime: function (outputRoot, executeCommands) {
    ...
    try {
      ...
      return this.evidence(
        ...
        "COMMAND",
        "PASSED",
        "Generated backend and storefront runtime probes completed successfully.",
        { command: command },
      );
    } catch (error) {
      return this.evidence(
        ...
        "COMMAND",
        "FAILED",
        "Generated backend or storefront runtime probe failed.",
        { command: command },
      );
    }
  },
  ...
  qualifyReferenceWorkspace: function (referenceEvidencePath, outputRoot) {
    ...
    if (!fs.existsSync(resolved) || !fs.statSync(resolved).isFile()) {
      throw new Error(
        "Reference qualification evidence is unavailable: " + resolved,
      );
    }
    const source = fs.readFileSync(resolved, "utf8");
    const passed =
      /Status:\s*\*\*PASSED\*\*/.test(source) &&
      /all nine runtimes READY/i.test(source) &&
      /fresh-database qualification/i.test(source);
    return [
      this.evidence(
        "reference.workspace.nine-runtimes",
        "REFERENCE_DOCUMENT",
        passed ? "PASSED" : "FAILED",
        passed
          ? "Reference workspace fresh-database nine-runtime qualification is recorded as passed."
  ...
  createReport: function (
    ...
    const reportPath =
      options.reportPath || "qualification/builder-qualification-report.json";
    const fullGeneratedApplication =
      allPassed && gates.includes("generated.runtime");
    const scope = fullGeneratedApplication
      ? "FULL_GENERATED_APPLICATION"
      : options.referenceEvidencePath
        ? "REFERENCE_WORKSPACE"
        : "GENERATED_SKELETON";
    const report = {
      contractVersion: 1,
      qualificationId:
        ...
        seed.slice("sha256:".length, "sha256:".length + 32),
      createdAt: createdAt,
      solutionDigest: lock.solutionDigest,
      planDigest: lock.planDigest,
    ...
    const validation = contractService.validateDocument(
      "qualification",
      report,
    );
    if (!validation.valid) {
      throw new Error(
        "Builder produced an invalid qualification report:\n- " +
          validation.errors.join("\n- "),
      );
    }
    return report;
  ...
  renderHumanSummary: function (report, solution, outputRoot) {
    ...
    return (
      "# Builder qualification summary\n\n" +
      "- Project: `" +
      solution.identity.projectCode +
      "`\n" +
      "- State: `" +
      report.state +
      "`\n" +
      ...
      "## Passed gates\n\n" +
      (passed.length
        ? passed.map((gate) => "- `" + gate + "`").join("\n")
        : "- None") +
      "\n\n" +
      "## Failed gates\n\n" +
      (failed.length
        ? failed
            .map((entry) => "- `" + entry.gate + "`: " + entry.summary)
            .join("\n")
        : "- None") +
      "\n\n" +
      "## Next commands\n\n" +
  ...
  writeQualification: function (outputRoot, lock, report) {
    ...
    const summaryPath = path.join(
      path.dirname(report.lockUpdate.reportPath),
      "builder-qualification-summary.md",
    );
    fs.writeFileSync(
      path.join(outputRoot, summaryPath),
      this.renderHumanSummary(
        report,
        { identity: { projectCode: report.projectCode } },
        outputRoot,
      ),
      { encoding: "utf8" },
    );
    ...
    if (!lockValidation.valid) {
      throw new Error(
        "Builder qualification produced an invalid solution lock:\n- " +
  ...
  qualify: function (plan, solution, catalogue, configuredRoot, options = {}) {
    ...
    const evidence = [
      ...
      this.qualifyGeneratedSelfTest(
        outputRoot,
        options.executeCommands !== false,
      ),
      this.qualifyGeneratedRuntime(
        outputRoot,
        options.executeCommands !== false,
      ),
    ].concat(
      this.qualifyReferenceWorkspace(options.referenceEvidencePath, outputRoot),
    );
    ...
    const nextLock =
      options.write === false
        ? Object.assign({}, lock, { qualification: report.lockUpdate })
