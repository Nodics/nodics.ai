...
module.exports = {
  ...
  createReleaseManifest: function (solution, plan, catalogue, options = {}) {
    const solutionValidation = contractService.validateDocument(
      "solution",
      solution,
    );
    ...
    if (plan?.solutionDigest !== catalogueService.digest(solution)) {
      errors.push(
        "Builder release manifest plan does not match the supplied solution",
      );
    }
    if (plan?.catalogueDigest !== catalogue.catalogueDigest) {
      errors.push(
        "Builder release manifest plan does not match the current catalogue",
      );
    }
    if (
      plan?.approval?.approvedPlanDigest !==
      generationService.approvalDigest(plan)
    ) {
      errors.push("Builder release manifest approval digest is invalid");
    }
    if (errors.length > 0) {
      throw new Error(
        "Builder release manifest validation failed:\n- " + errors.join("\n- "),
      );
    }
    ...
    const manifest = {
      ...
      packages: plan.resolvedCapabilities
        .map((item) => ({
          code: item.code,
  ...
  verifyReleaseManifest: function (manifest) {
    ...
    if (manifest.signature.algorithm !== "LOCAL_SHA256_DIGEST") {
      errors.push(
        "Unsupported Builder release signature algorithm: " +
  ...
  comparePackages: function (currentPackages, targetPackages) {
    const current = new Map(
      (currentPackages || []).map((item) => [item.code, item]),
    );
    const target = new Map(
      (targetPackages || []).map((item) => [item.code, item]),
    );
    const added = [];
    const removed = [];
    const changed = [];
    target.forEach((targetPackage, code) => {
      ...
      if (!currentPackage) {
        ...
        currentPackage.digest !== targetPackage.digest ||
        currentPackage.version !== targetPackage.version
      ) {
        changed.push(code);
      }
    });
  ...
  createUpgradePlan: function (
    ...
    const lockValidation = contractService.validateDocument(
      "lock",
      currentLock,
    );
    ...
    if (
      targetManifest.planDigest !== targetPlan?.approval?.approvedPlanDigest
    ) {
      errors.push(
        "Target release manifest does not match the supplied approved plan",
      );
    }
    if (
      targetManifest.solutionDigest !== catalogueService.digest(targetSolution)
    ) {
      errors.push(
        "Target release manifest does not match the supplied solution",
      );
    }
    if (targetManifest.catalogueDigest !== catalogue.catalogueDigest) {
      errors.push(
        "Target release manifest does not match the current catalogue",
      );
    }
    const packageDiff = this.comparePackages(
      currentLock.packages,
      targetManifest.packages,
    );
    const currentDataPacks = new Set(
      (currentLock.dataPacks || []).map((item) => item.code),
    );
    const targetDataPacks = new Set(targetManifest.dataPacks || []);
    const dataPackDiff = {
      added: Array.from(targetDataPacks)
        .filter((code) => !currentDataPacks.has(code))
        .sort(),
      removed: Array.from(currentDataPacks)
        .filter((code) => !targetDataPacks.has(code))
        .sort(),
    };
    const changed =
      currentLock.solutionDigest !== targetManifest.solutionDigest ||
      ...
      dataPackDiff.added.length > 0 ||
      dataPackDiff.removed.length > 0;
    const createdAt = options.createdAt || new Date().toISOString();
    return {
      contractVersion: 1,
      upgradePlanId:
      ...
      operations:
        errors.length > 0 || !changed
          ? []
          : [
              {
                sequence: 1,
                operation: "VERIFY_CURRENT_LOCK",
                precondition: "LOCKED_HASH_MATCH",
              },
              {
                sequence: 2,
                operation: "BACKUP_CURRENT_GENERATED_ARTIFACTS",
                precondition: "DIRECTORY_EXISTS",
              },
              {
                sequence: 3,
                operation: "APPLY_APPROVED_GENERATED_ARTIFACTS",
                precondition: "GENERATED_REGION_MATCH",
              },
              {
                sequence: 4,
                operation: "RUN_BUILDER_QUALIFICATION",
                precondition: "DIRECTORY_EXISTS",
              },
            ],
