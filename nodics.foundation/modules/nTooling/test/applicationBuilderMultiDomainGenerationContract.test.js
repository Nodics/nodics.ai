...
const tempParent = fs.mkdtempSync(
  path.join(os.tmpdir(), "nodics-builder-wp-b5-"),
);
...
function approve(solution, reference) {
  return generationService.approvePlan(
    planningService.createPlan(solution, catalogue, {
...
function composition(root) {
  const source = fs.readFileSync(
    path.join(root, "storefront/generated/frontend-composition.ts"),
    "utf8",
  );
  const match = source.match(/const composition = ([\s\S]+) as const;/);
  assert(
    match,
    "Generated Agora composition must contain a deterministic object",
  );
...
try {
  cases.forEach((testCase) => {
    ...
    generationService.generate(plan, solution, catalogue, output, {
    ...
    assert.deepStrictEqual(
      ...
      "Backend graph must be the exact capability union",
    );
    assert.strictEqual(
      new Set(graph.capabilities).size,
    ...
    if (graph.capabilities.includes("telco")) {
      assert(
        graph.edges.some(
          (edge) => edge.from === "telco" && edge.to === "electronics",
        ),
    ...
    assert.deepStrictEqual(
      frontend.rendererKeys,
      testCase.renderers,
      "Generated renderer selection must exactly match the solution",
    );
    assert.strictEqual(
      new Set(frontend.rendererKeys).size,
      frontend.rendererKeys.length,
      "Generated renderer selection must not contain duplicates",
    );
    const discoveredComposition = catalogue.frontendCompositions.find(
      (item) => item.code === frontend.composition,
    );
    assert.deepStrictEqual(
      discoveredComposition.domains,
      testCase.domains,
      "Generated composition must use the source-backed active domains",
    );
    frontend.rendererKeys.forEach((key) =>
      assert(
        discoveredComposition.rendererKeys.includes(key),
        "Generated renderer must exist in the source-backed Agora composition: " +
          key,
      ),
    );
    assert.deepStrictEqual(
      fs.readdirSync(path.join(output, "data")).sort(),
      testCase.packs,
      "Generated customer data-pack directories must exactly match the solution",
    );
    const lock = JSON.parse(
      fs.readFileSync(path.join(output, "solution-lock.json"), "utf8"),
    );
    assert.deepStrictEqual(
      lock.dataPacks.map((pack) => pack.code),
      testCase.packs,
      "Solution lock must contain the exact customer data-pack union",
    );
    assert.strictEqual(
      new Set(lock.packages.map((item) => item.code)).size,
      lock.packages.length,
      "Solution lock must not duplicate package coordinates",
    );
    const handoff = JSON.parse(
      fs.readFileSync(path.join(output, "builder-handoff.json"), "utf8"),
    );
    assert.deepStrictEqual(
      handoff.selected.dataPacks,
      testCase.packs,
      "Generated handoff must show the exact selected customer data-pack union",
    );
    assert.deepStrictEqual(
      handoff.selected.rendererKeys,
      testCase.renderers,
      "Generated handoff must show the exact selected renderer hierarchy",
    );
    assert(
      handoff.ownership.projectData.includes("Customer-owned"),
      "Generated handoff must explain customer data ownership",
    );
    const wiring = JSON.parse(
      fs.readFileSync(
        path.join(output, "integrations/frontend-wiring.json"),
        "utf8",
      ),
    );
    assert.strictEqual(
      wiring.generatedApps.agora.selected,
      true,
      "Agora wiring must always be selected for generated commerce experiences",
    );
    assert.strictEqual(
      wiring.generatedApps.axis?.selected || false,
      solution.topology.frontends.includes("AXIS"),
      "Axis wiring must follow selected frontends without copying Axis source",
    );
    assert.strictEqual(
      wiring.generatedApps.nexus?.selected || false,
      solution.topology.frontends.includes("NEXUS"),
      "Nexus wiring must follow selected frontends without copying Nexus source",
    );
    assert.strictEqual(
      fs.existsSync(path.join(output, "integrations/axis-wiring.md")),
      solution.topology.frontends.includes("AXIS"),
      "Axis wiring notes must be generated only when Axis is selected",
    );
    assert.strictEqual(
      fs.existsSync(path.join(output, "integrations/nexus-wiring.md")),
      solution.topology.frontends.includes("NEXUS"),
    ...
    assert(
      fs
        .readFileSync(
          path.join(output, "deployment/local-compose.yaml"),
          "utf8",
        )
        .includes("NODICS_PROJECT_CODE"),
    ...
    testCase.renderers.forEach((renderer) =>
      assert(
        frontendGuide.includes(renderer),
    ...
    testCase.packs.forEach((dataPack) =>
      assert(
        customizationMap.includes("data/" + dataPack),
    ...
    testCase.packs.forEach((dataPack) => {
      ...
      const page = JSON.parse(
        fs.readFileSync(path.join(packRoot, "pages/home.json"), "utf8"),
      );
      const products = JSON.parse(
        fs.readFileSync(
          path.join(packRoot, "products/starter-products.json"),
          "utf8",
        ),
      );
      const prices = JSON.parse(
        fs.readFileSync(
          path.join(packRoot, "prices/starter-prices.json"),
          "utf8",
        ),
      );
      const inventory = JSON.parse(
        fs.readFileSync(
          path.join(packRoot, "inventory/starter-inventory.json"),
          "utf8",
        ),
      );
      assert.strictEqual(
      ...
      if (
        solution.data.sampleData === true &&
        solution.data.packDomains?.[dataPack] !== "content"
      ) {
        const expectedDomain = solution.data.packDomains[dataPack];
        assert(
          products.products.length > 0,
          "Domain data pack must include at least one starter product: " +
            dataPack,
        );
        products.products.forEach((product) =>
          assert.strictEqual(
            product.domain,
            expectedDomain,
            "Domain data pack must only include its domain products: " +
              dataPack,
          ),
        );
      } else {
    ...
    const repeat = path.join(
      tempParent,
      solution.identity.projectCode + "-repeat",
    );
    generationService.generate(plan, solution, catalogue, repeat, {
      now: "2026-08-16T10:10:00.000Z",
    });
    assert.deepStrictEqual(
      tree(repeat),
      tree(output),
      testCase.fixture + " must generate deterministically",
    );
  });
  ...
  const rendererValidation = planningService.validateSolution(
    invalidRenderer,
    catalogue,
  );
  assert.strictEqual(
    rendererValidation.valid,
    false,
    "Cross-domain renderer selection must fail",
  );
  assert(
    rendererValidation.errors.some((error) =>
      error.includes("renderer is unavailable"),
    ),
  ...
  const packValidation = planningService.validateSolution(
    invalidPack,
    catalogue,
  );
  assert.strictEqual(
    ...
    false,
    "Telco must not silently include an Electronics customer data pack",
  );
  assert(
    packValidation.errors.some((error) =>
      error.includes("not required by the selected experience"),
    ),
  ...
  const approvedJson = childProcess.execFileSync(
    process.execPath,
    [cli, "builder:approve"].concat(roots, [
      "--plan=" + cliPlanPath,
      "--approval-reference=WP-B5-CLI",
    ]),
  ...
  const result = JSON.parse(
    childProcess.execFileSync(
      ...
      [cli, "builder:generate"].concat(roots, [
        ...
        "--solution=" + path.join(fixtureRoot, "solution-combined.json"),
        "--output=" + cliOutput,
      ]),
      { cwd: frameworkRoot, encoding: "utf8" },
    ),
  ...
  assert.strictEqual(
    result.generated,
    true,
    "Governed CLI must generate the approved Combined application",
  );
...
console.log(
  "Application Builder WP-B5 multi-domain generation contract validated",
);
