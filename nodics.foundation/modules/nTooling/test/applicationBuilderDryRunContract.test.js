...
assert.strictEqual(
  result.guided,
  true,
  "Dry run must stay on the beginner guided path",
);
assert.strictEqual(
  result.dryRun,
  true,
  "Dry run must identify itself explicitly",
);
assert.strictEqual(
  result.writePerformed,
  false,
  "Dry run must not write review or generated application files",
);
assert.strictEqual(
  result.dryRunPlan.writePerformed,
  false,
  "Dry-run projection must repeat no-write semantics",
);
assert.strictEqual(
  result.dryRunPlan.generatedApplication,
  false,
  "Dry run must not claim generated backend or frontend files",
);
assert.strictEqual(
  result.dryRunPlan.safety.mode,
  "READ_ONLY_PLAN",
  "Dry run must be reported as a read-only plan",
);
assert.strictEqual(
  result.dryRunPlan.safety.generationBlocked,
  true,
  "Dry run must keep generation blocked until explicit approval",
);
assert(
  result.dryRunPlan.beginnerQuestions.some(
    (question) =>
      question.id === "domain" &&
...
assert(
  result.dryRunPlan.customerProjectOutputs.backend.includes(
    "backend/src/server.js",
  ),
  "Dry run must show planned backend output without writing it",
);
assert(
  result.dryRunPlan.customerProjectOutputs.storefront.includes(
    "storefront/src/server.js",
  ),
  "Dry run must show planned storefront output without writing it",
);
assert(
  result.dryRunPlan.ownershipRules.some((rule) =>
    rule.includes("framework and domain behavior stays in nodics.ai"),
  ),
...
assert.strictEqual(
  commands["builder:dry-run"].operation,
  "dry-run",
  "WP-B10 must expose a beginner dry-run operation",
);
assert.strictEqual(
  commands["builder:dry-run"].handler,
...
assert.strictEqual(
  cliResult.dryRun,
  true,
  "Governed CLI must expose dry-run Builder output",
);
assert.strictEqual(
  cliResult.dryRunPlan.safety.mode,
  "READ_ONLY_PLAN",
  "Governed CLI dry run must preserve read-only safety semantics",
);
assert(
  cliResult.dryRunPlan.selectedResult.transitiveBackendDependencies.includes(
    "electronics",
  ),
  "Governed CLI dry run must explain Telco dependency resolution",
);
assert.strictEqual(
  cliResult.beginnerWorkflow.currentOperation,
  "dry-run",
  "Governed CLI dry run must include beginner workflow guidance",
);
assert.strictEqual(
  cliResult.beginnerWorkflow.recommendedNext.operation,
  "guide",
  "Governed CLI dry run must recommend the guide/review step next",
);
assert(
  cliResult.beginnerWorkflow.steps.some((step) => step.operation === "qualify"),
