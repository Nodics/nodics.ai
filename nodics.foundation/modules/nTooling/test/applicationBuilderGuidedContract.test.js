...
function fixture(state, fileName) {
  return JSON.parse(
    fs.readFileSync(path.join(fixtureRoot, state, fileName), "utf8"),
  );
}
...
const reviewParent = fs.mkdtempSync(
  path.join(os.tmpdir(), "nodics-builder-guide-"),
);
const reviewRoot = path.join(reviewParent, "acme-mobile-review");
const canonicalReviewRoot = path.join(
  fs.realpathSync(reviewParent),
  "acme-mobile-review",
);
...
assert.deepStrictEqual(
  persisted.artifacts.map((artifact) => artifact.path).sort(),
  [
    "beginner-summary.md",
...
assert.strictEqual(
  fs.existsSync(path.join(reviewRoot, "solution.json")),
  true,
...
assert.strictEqual(
  contractService.validateDocument(
    "solution",
    JSON.parse(fs.readFileSync(path.join(reviewRoot, "solution.json"), "utf8")),
  ).valid,
  true,
  "Persisted guided solution must satisfy the Application Builder contract",
);
const persistedPlan = JSON.parse(
  fs.readFileSync(path.join(reviewRoot, "generation-plan.json"), "utf8"),
);
assert.strictEqual(
  contractService.validateDocument("plan", persistedPlan).valid,
  true,
  "Persisted guided generation plan must satisfy the Application Builder contract",
);
assert.strictEqual(
  persistedPlan.approval.state,
  "PENDING",
  "Persisted guided generation plan must remain approval-required",
);
const persistedSummary = fs.readFileSync(
  path.join(reviewRoot, "beginner-summary.md"),
  "utf8",
);
assert(
  persistedSummary.includes("Nodics Application Builder review"),
  "Persisted guided summary must be beginner-readable Markdown",
);
assert(
  persistedSummary.includes("builder:approve"),
  "Persisted guided summary must include the next approval command",
);
const persistedReport = JSON.parse(
  fs.readFileSync(path.join(reviewRoot, "builder-guide-report.json"), "utf8"),
);
assert.strictEqual(
  persistedReport.writeScope,
  "GUIDED_REVIEW_ONLY",
  "Persisted guided report must identify review-only writes",
);
assert.strictEqual(
  persistedReport.generatedApplication,
  false,
...
assert.throws(
  () =>
    guidedService.guide(request, expCatalogue, {
      workspaceRoot: "relative-review",
    }),
  /absolute path/,
  "Guided Builder must reject relative review workspaces",
);
assert.throws(
  () =>
    guidedService.guide(request, expCatalogue, { workspaceRoot: reviewRoot }),
  /must be absent/,
  "Guided Builder must reject an existing review workspace",
);
assert.throws(
  () =>
    guidedService.guide(request, expCatalogue, {
  ...
  /protected root/,
  "Guided Builder must reject protected review roots",
);

assert.throws(
  () =>
    guidedService.guide(
      fixture("invalid", "guided-request-no-agora.json"),
      catalogue,
    ),
  /Guided Builder requires selected storefront: AGORA/,
  "Guided Builder must give corrective missing-storefront diagnostics",
);
...
assert.strictEqual(
  commands["builder:guide"].handler,
...
const cliResult = JSON.parse(
  childProcess.execFileSync(
    ...
    [
      ...
      "--kickoff=" + kickoffRoot,
      "--answers=" +
        path.join(fixtureRoot, "valid", "guided-request-telco.json"),
    ],
...
assert.strictEqual(
  cliResult.guided,
  true,
  "Governed CLI must expose guided Builder output",
);
assert.strictEqual(
  cliResult.solution.experience.composition,
  "telco",
  "Governed CLI must generate the Telco solution from beginner answers",
);
const uiCliResult = JSON.parse(
  childProcess.execFileSync(
    process.execPath,
    [
      cli,
...
assert.strictEqual(
  uiCliResult.solution.experience.composition,
  "telco",
  "Governed CLI must resolve Agora through nodics.exp for beginner guided output",
);
assert(
  uiCliResult.plan.frontendGraph.nodes.includes("agora.telco"),
...
const cliPersistedResult = JSON.parse(
  childProcess.execFileSync(
    process.execPath,
    [
      cli,
...
assert.strictEqual(
  cliPersistedResult.writePerformed,
  true,
  "Governed CLI must persist guided review artifacts when --workspace is supplied",
);
assert.strictEqual(
  fs.existsSync(path.join(cliReviewRoot, "beginner-summary.md")),
  true,
  "Governed CLI must write the guided beginner summary artifact",
);

console.log(
  "Application Builder WP-B8 guided beginner review contract validated",
);
