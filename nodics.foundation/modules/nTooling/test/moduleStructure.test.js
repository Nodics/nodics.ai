...
const nPrefixedCapability = {
    name: 'nInventory',
    relativePath: 'nodics.foundation/modules/nInventory',
    path: rootPathForFixture('nodics.foundation/modules/nInventory'),
    packageJson: {
        name: 'nInventory',
        nodics: {
            kind: 'capability'
...
const nToolingSrcPath = path.join(process.cwd(), 'nodics.foundation', 'modules', 'nTooling', 'src');
const nToolingModule = modules.find(moduleObject => moduleObject.relativePath === 'nodics.foundation/modules/nTooling');
...
fs.readdirSync(nToolingSrcPath, { withFileTypes: true })
    ...
    .forEach(entry => {
        assert.strictEqual(entry.name, 'service',
            'nTooling source must stay service-owned; unexpected src folder: nodics.foundation/modules/nTooling/src/' + entry.name);
    });
