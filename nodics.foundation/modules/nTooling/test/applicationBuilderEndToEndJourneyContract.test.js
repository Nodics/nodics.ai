...
const tempParent = fs.mkdtempSync(
  path.join(os.tmpdir(), "nodics-builder-wp-b15-"),
);
...
async function runJourney(testCase) {
  ...
  const answers = await guidedService.runQuestionnaire({
    scriptedAnswers: [
  ...
  assert.strictEqual(
    ...
    true,
    "Questionnaire must produce valid guided answers for " +
      testCase.projectCode,
  );
  ...
  assert.strictEqual(
    dryRun.dryRunPlan.safety.mode,
    "READ_ONLY_PLAN",
    "Dry run must stay read-only for " + testCase.projectCode,
  );
  assert.deepStrictEqual(
    dryRun.dryRunPlan.selectedResult.activeDomains,
    testCase.expectedDomains,
    "Dry run must show the selected active domains for " + testCase.projectCode,
  );
  assert.deepStrictEqual(
    dryRun.dryRunPlan.selectedResult.dataPacks.slice().sort(),
    testCase.expectedPacks,
  ...
  assert.strictEqual(
    review.writePerformed,
    true,
    "Guide review must write review artifacts for " + testCase.projectCode,
  );
  assert.strictEqual(
    fs.existsSync(path.join(reviewRoot, "beginner-summary.md")),
    true,
  ...
  const generation = generationService.generate(
    approved,
    review.solution,
    catalogue,
    outputRoot,
  ...
  assert.strictEqual(
    fs.existsSync(path.join(outputRoot, "README.md")),
    true,
    "Generated output must include README handoff for " + testCase.projectCode,
  );
  assert.strictEqual(
    fs.existsSync(path.join(outputRoot, "builder-handoff.json")),
    true,
    "Generated output must include machine-readable handoff for " +
      testCase.projectCode,
  );
  ...
  assert.strictEqual(
    ...
    "Qualified generated output must prove runtime probes for " +
      testCase.projectCode,
  );
  assert(
    qualification.gates.includes("generated.handoff"),
    "Qualification must include the generated handoff gate for " +
      testCase.projectCode,
  );
  assert.strictEqual(
    fs.existsSync(path.join(outputRoot, qualification.summaryPath)),
    true,
...
(async () => {
  try {
    ...
    console.log(
      "Application Builder WP-B15 beginner end-to-end journey contract validated",
    );
  } finally {
