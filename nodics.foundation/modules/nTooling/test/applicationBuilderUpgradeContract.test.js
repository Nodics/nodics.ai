...
const tempParent = fs.mkdtempSync(
  path.join(os.tmpdir(), "nodics-builder-upgrade-"),
);
...
function approve(solution, reference) {
  return generationService.approvePlan(
    planningService.createPlan(solution, catalogue, {
      ...
      expiresAt: "2099-08-17T13:00:00.000Z",
    }),
    reference,
...
try {
  ...
  const manifest = upgradeService.createReleaseManifest(
    targetSolution,
    targetPlan,
    catalogue,
    {
  ...
  assert.strictEqual(
    upgradeService.verifyReleaseManifest(manifest).valid,
    true,
    "Builder release manifest must verify through its local digest signature",
  );
  assert.strictEqual(
    manifest.artifactPolicy.customerOwnedOverwrite,
    "NEVER",
    "Builder release manifest must preserve customer-owned overwrite protection",
  );

  const upgradePlan = upgradeService.createUpgradePlan(
    currentLock,
    manifest,
    targetPlan,
    targetSolution,
  ...
  const rejected = upgradeService.createUpgradePlan(
    currentLock,
    tamperedManifest,
    targetPlan,
    targetSolution,
  ...
  assert(
    rejected.errors.some((error) => error.includes("signature digest")),
    "Builder upgrade diagnostics must explain manifest signature failure",
  );

  const noopPlan = upgradeService.createUpgradePlan(
    currentLock,
    upgradeService.createReleaseManifest(
      currentSolution,
      currentPlan,
      catalogue,
      {
  ...
  const releaseOutput = childProcess.execFileSync(
    process.execPath,
    [
      cli,
      ...
      "--exp=" + expRoot,
      "--kickoff=" + kickoffRoot,
      "--solution=" + targetSolutionPath,
      "--plan=" + targetPlanPath,
      "--release-channel=LOCAL_TEST",
    ],
    { cwd: frameworkRoot, encoding: "utf8" },
  );
  fs.writeFileSync(releasePath, releaseOutput);
  const cliUpgrade = JSON.parse(
    childProcess.execFileSync(
      ...
      [
        ...
        "--exp=" + expRoot,
        "--kickoff=" + kickoffRoot,
        "--current-lock=" + path.join(outputRoot, "solution-lock.json"),
        "--release=" + releasePath,
        "--solution=" + targetSolutionPath,
        "--plan=" + targetPlanPath,
      ],
  ...
  assert.strictEqual(
    commands["builder:release-manifest"].operation,
    "release-manifest",
    "Builder must expose release manifest creation",
  );
  assert.strictEqual(
    commands["builder:upgrade-plan"].operation,
    "upgrade-plan",
...
console.log(
  "Application Builder WP-B9 registry and upgrade contract validated",
);
