...
const validFixtures = {
  solution: [
    "solution-commerce.json",
    "solution-apparel.json",
    "solution-electronics.json",
    "solution-telco.json",
    "solution-combined.json",
  ],
...
Object.keys(validFixtures).forEach((kind) => {
  validFixtures[kind].forEach((fileName) => {
    const result = contractService.validateDocument(
      kind,
      fixture("valid", fileName),
    );
    assert.strictEqual(
      result.valid,
      true,
...
Object.keys(invalidFixtures).forEach((fileName) => {
  const result = contractService.validateDocument(
    invalidFixtures[fileName],
    fixture("invalid", fileName),
  );
  assert.strictEqual(result.valid, false, fileName + " must be rejected");
  assert(
    result.errors.length > 0,
    fileName + " must report actionable diagnostics",
  );
});

assert.throws(
  () => contractService.loadSchema("unknown"),
  /Unsupported Application Builder contract kind/,
