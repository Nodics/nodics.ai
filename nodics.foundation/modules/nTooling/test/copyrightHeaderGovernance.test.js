...
const shorterHeaderFile = path.join(tempRoot, 'shorterHeader.mjs');
...
fs.writeFileSync(shorterHeaderFile, copyrightHeaders.compatibleImmutableReleaseHeader + '\nexport default {};\n', 'utf8');

let initial = copyrightHeaders.inspectFiles({
    rootDir: tempRoot,
    fix: false
});
assert.strictEqual(initial.filesChecked, 4);
...
assert.strictEqual((fs.readFileSync(shorterHeaderFile, 'utf8').match(/Copyright \(c\)/g) || []).length, 1,
    'a compatible immutable-release header must remain accepted without duplication');
assert(fs.readFileSync(shorterHeaderFile, 'utf8').startsWith(copyrightHeaders.compatibleImmutableReleaseHeader),
