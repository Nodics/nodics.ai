...
const tempParent = fs.mkdtempSync(
  path.join(os.tmpdir(), "nodics-builder-safety-"),
);
...
try {
  ...
  assert.strictEqual(
    planningService.validateSolution(invalidRenderer, catalogue).valid,
    false,
  ...
  assert.strictEqual(
    planningService.validateSolution(invalidDataPack, catalogue).valid,
    false,
  ...
  assert.strictEqual(
    planningService.validateSolution(invalidDomain, catalogue).valid,
    false,
  ...
  assert.throws(
    () => generationService.approvePlan(pendingPlan, ""),
  ...
  assert.throws(
    ...
    "Builder generation must reject a pending plan",
  );
  ...
  assert.throws(
    () =>
      generationService.generate(
        ...
        catalogue,
        path
          .join(tempParent, "relative-output")
          .replace(tempParent + path.sep, ""),
      ),
    /absolute path/,
    "Builder generation must require an absolute output root",
  );
  ...
  assert.throws(
    ...
    "Builder generation must not overwrite an existing generated root",
  );

  const tamperedDataPath = path.join(
    outputRoot,
    "data",
    "agora.electronics",
    "products",
    "starter-products.json",
  );
  fs.unlinkSync(tamperedDataPath);
  const tampered = qualificationService.qualify(
    approved,
    electronics,
    catalogue,
    outputRoot,
  ...
  const tamperedReport = JSON.parse(
    fs.readFileSync(path.join(outputRoot, tampered.reportPath), "utf8"),
  );
  assert(
    tamperedReport.evidence.some(
      (entry) =>
        entry.gate === "generated.self-test" && entry.state === "FAILED",
    ),
