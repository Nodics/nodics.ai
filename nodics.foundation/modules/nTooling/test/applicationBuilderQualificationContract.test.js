...
const referenceEvidencePath = path.join(
  workspaceRoot,
  "actionsRepo",
  "Agora",
...
const tempParent = fs.mkdtempSync(
  path.join(os.tmpdir(), "nodics-builder-wp-b6-"),
);
...
try {
  ...
  generationService.generate(approved, solution, catalogue, outputRoot, {
  ...
  assert.strictEqual(
    initialLock.qualification.state,
    "NOT_RUN",
    "Generated lock must start with NOT_RUN qualification",
  );
  assert.strictEqual(
    initialLock.qualification.scope,
    "GENERATED_SKELETON",
  ...
  const result = qualificationService.qualify(
    approved,
    solution,
    catalogue,
    outputRoot,
  ...
  assert.strictEqual(
    result.qualified,
    true,
    "Qualification must pass for an intact generated output",
  );
  assert.strictEqual(
    result.scope,
  ...
  assert(
    result.gates.includes("backend.graph"),
    "Qualification must include backend graph evidence",
  );
  assert(
    result.gates.includes("frontend.composition"),
    "Qualification must include frontend composition evidence",
  );
  assert(
    result.gates.includes("customer.ownership"),
    "Qualification must include customer ownership evidence",
  );
  assert(
    result.gates.includes("security.boundary"),
    "Qualification must include security boundary evidence",
  );
  assert(
    result.gates.includes("generated.handoff"),
    "Qualification must include generated handoff evidence",
  );
  assert(
    result.gates.includes("generated.self-test"),
    "Qualification must include generated self-test evidence",
  );
  assert(
    result.gates.includes("generated.runtime"),
    "Qualification must include generated runtime evidence",
  );
  if (hasReferenceEvidence) {
    assert(
      result.gates.includes("reference.workspace.nine-runtimes"),
  ...
  const report = JSON.parse(
    fs.readFileSync(path.join(outputRoot, result.reportPath), "utf8"),
  );
  assert.strictEqual(
    contractService.validateDocument("qualification", report).valid,
    true,
    "Qualification report must satisfy its schema",
  );
  assert.strictEqual(
    fs.existsSync(path.join(outputRoot, result.summaryPath)),
    true,
    "Qualification must write a human-readable summary beside the JSON report",
  );
  const summary = fs.readFileSync(
    path.join(outputRoot, result.summaryPath),
    "utf8",
  );
  assert(
    summary.includes("Builder qualification summary"),
    "Qualification summary must be human-readable",
  );
  assert(
    summary.includes("generated.handoff"),
  ...
  assert.strictEqual(
    nextLock.qualification.state,
    "PASSED",
    "Qualification must update the generated solution lock",
  );
  assert.strictEqual(
    nextLock.qualification.evidenceDigest,
  ...
  const fullGeneratedResult = qualificationService.qualify(
    approved,
    solution,
    catalogue,
    fullGeneratedOutput,
  ...
  generationService.generate(approved, solution, catalogue, tamperedOutput, {
    now: "2026-08-16T11:10:00.000Z",
  });
  ...
  const tamperedResult = qualificationService.qualify(
    approved,
    solution,
    catalogue,
    tamperedOutput,
  ...
  assert.strictEqual(
    tamperedResult.state,
    "FAILED",
    "Tampered generated artifacts must fail qualification",
  );
  const tamperedLock = JSON.parse(
    fs.readFileSync(path.join(tamperedOutput, "solution-lock.json"), "utf8"),
  );
  assert.strictEqual(
    tamperedLock.qualification.state,
    "FAILED",
  ...
  generationService.generate(cliPlan, cliSolution, catalogue, cliOutput, {
    now: "2026-08-16T11:10:00.000Z",
  });
  ...
  const cliArgs = [
    ...
    "--exp=" + expRoot,
    "--kickoff=" + kickoffRoot,
    "--solution=" + path.join(fixtureRoot, "solution-commerce.json"),
    "--plan=" + cliPlanPath,
    "--output=" + cliOutput,
  ];
  if (hasReferenceEvidence) {
  ...
  const cliResult = JSON.parse(
    childProcess.execFileSync(process.execPath, cliArgs, {
      cwd: frameworkRoot,
      encoding: "utf8",
    }),
  );
  assert.strictEqual(
    cliResult.qualified,
    true,
    "Governed CLI must qualify generated Builder output",
  );
  ...
  assert.strictEqual(
    commands["builder:qualify"].handler,
