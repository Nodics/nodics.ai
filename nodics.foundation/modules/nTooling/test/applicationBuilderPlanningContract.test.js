...
const expCatalogue = catalogueService.discover({
  framework: frameworkRoot,
  exp: expRoot,
  kickoff: kickoffRoot,
});
...
assert.strictEqual(
  catalogue.readOnly,
  true,
  "Builder discovery must be read-only",
);
assert(
  catalogue.capabilities.some(
    (capability) => capability.code === "nodics.commerce",
  ),
  "Catalogue must discover Commerce",
);
assert(
  catalogue.capabilities.some((capability) => capability.code === "apparel"),
  "Catalogue must discover Apparel",
);
assert(
  catalogue.capabilities.some(
    (capability) => capability.code === "electronics",
  ),
  "Catalogue must discover Electronics",
);
assert(
  catalogue.capabilities.some((capability) => capability.code === "telco"),
  "Catalogue must discover Telco",
);
assert(
  catalogue.frontendCompositions.some(
    (composition) => composition.code === "combined",
  ),
  "Catalogue must discover the combined Agora composition",
);
assert(
  catalogue.customerDataPacks.some(
    (dataPack) => dataPack.code === "agora.telco",
  ),
  "Catalogue must discover the customer-owned Telco data-pack boundary",
);
assert(
  /^sha256:[a-f0-9]{64}$/.test(catalogue.catalogueDigest),
...
assert.strictEqual(
  ...
  catalogueService.digest(portableCatalogue),
  "Catalogue digest must exclude machine-specific absolute repository roots",
);
assert(
  expCatalogue.repositories.some(
    (repository) => repository.code === "experience",
  ),
);
assert(expCatalogue.frontendApps.some((app) => app.code === "AGORA"));
assert.deepStrictEqual(
  expCatalogue.capabilities,
  catalogue.capabilities,
  "Using nodics.exp to resolve the same Agora app must keep backend capability semantics",
);
const expCompositionByCode = new Map(
  expCatalogue.frontendCompositions.map((composition) => [
    composition.code,
    composition,
  ]),
);
["apparel", "electronics", "telco", "combined", "commerce"].forEach((code) => {
  assert(
    expCompositionByCode.has(code),
    "Using nodics.exp must expose the " + code + " Agora composition semantics",
  );
});
assert.deepStrictEqual(
  expCompositionByCode.get("combined").domains,
  ["apparel", "electronics", "telco"],
  "Split nodics.exp apps must preserve combined multi-domain planning semantics",
);
assert.deepStrictEqual(
  expCompositionByCode.get("combined").rendererKeys,
  catalogue.frontendCompositions.find(
    (composition) => composition.code === "combined",
  ).rendererKeys,
  "Split nodics.exp apps must preserve combined renderer availability",
);
assert(
  expCatalogue.frontendCompositions.every(
    (composition) => composition.evidence === "DECLARED_CUSTOMER_COMPOSITION",
  ),
  "Customer composition declarations must not be represented as live frontend verification",
);
assert.deepStrictEqual(
  expCatalogue.customerDataPacks,
  catalogue.customerDataPacks,
  "Using nodics.exp to resolve the same Agora app must keep Kickoff data-pack semantics",
);

const solution = JSON.parse(
  fs.readFileSync(path.join(fixtureRoot, "solution-combined.json"), "utf8"),
);
...
assert(
  validation.resolution.capabilities.some(
    (capability) => capability.code === "electronics",
  ),
  "Combined solution must resolve Electronics",
);
assert(
  validation.resolution.edges.some(
...
const firstPlan = planningService.createPlan(
  solution,
  catalogue,
  planningClock,
);
const secondPlan = planningService.createPlan(
  solution,
  catalogue,
  planningClock,
);
assert.deepStrictEqual(
  firstPlan,
  secondPlan,
  "Same inputs and clock must produce the same plan",
);
assert.strictEqual(
  JSON.stringify(solution),
  originalSolution,
  "Planning must not mutate the solution document",
);
assert.strictEqual(
  firstPlan.approval.state,
  "PENDING",
  "Plans must start pending approval",
);
assert.strictEqual(
  firstPlan.approval.required,
  true,
  "Plans must require explicit approval",
);
assert(
  firstPlan.operations.every(
    (operation) =>
      !path.isAbsolute(operation.targetPath) &&
      !operation.targetPath.split("/").includes(".."),
  ),
  "Every planned target must remain relative and traversal-free",
);
assert(
  firstPlan.operations.some(
    (operation) => operation.ownership === "CUSTOMER_OWNED",
  ),
  "Plan must distinguish customer-owned extension paths",
);
assert(
  firstPlan.excludedCapabilityAssertions.length === 0,
...
const invalidResult = planningService.validateSolution(
  invalidSolution,
  catalogue,
);
assert.strictEqual(
  invalidResult.valid,
  false,
  "Required excluded dependencies must fail validation",
);
assert(
  invalidResult.errors.some((error) => error.includes("Excluded capability")),
  "Dependency exclusion failure must be actionable",
);

const unresolvedSolution = JSON.parse(JSON.stringify(solution));
unresolvedSolution.decisions.unresolved = [
  "Choose the production identity provider",
];
assert.strictEqual(
  planningService.validateSolution(unresolvedSolution, catalogue).valid,
  false,
...
assert.throws(
  ...
  /explicit frontend root/,
  "Discovery must not infer a missing parallel repository without nodics.exp",
);
assert.throws(
  () =>
    catalogueService.discover({
      framework: frameworkRoot,
      exp: path.join(workspaceRoot, "missing-exp"),
      kickoff: kickoffRoot,
    }),
  /repository root is unavailable: experience/,
...
assert.strictEqual(
  commands["builder:plan"].operation,
  "plan",
  "Builder plan must remain a planning operation",
);
assert.strictEqual(
  commands["builder:guide"].operation,
  "guide",
  "WP-B7 must expose beginner guided creation",
);
assert.strictEqual(
  commands["builder:approve"].operation,
  "approve",
  "WP-B4 must expose explicit approval",
);
assert.strictEqual(
  commands["builder:generate"].operation,
  "generate",
  "WP-B4 must expose guarded generation",
);
assert.strictEqual(
  commands["builder:qualify"].operation,
  "qualify",
  "WP-B6 must expose governed qualification",
);

const builderContract = fs.readFileSync(
  path.join(__dirname, "../llm/contracts/application-builder.md"),
  "utf8",
);
[
  ...
  assert(
    builderContract.includes(clause),
    "Application Builder contract must preserve beginner guidance: " + clause,
  );
});
