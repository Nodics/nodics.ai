...
assert.strictEqual(
  answers.preset,
  "telco",
  "Answer template must preserve the beginner preset",
);
...
assert.strictEqual(
  dryRun.dryRun,
  true,
  "Generated answers must delegate to dry-run planning",
);
assert(
  dryRun.dryRunPlan.selectedResult.transitiveBackendDependencies.includes(
    "electronics",
  ),
  "Generated Telco answers must still explain Electronics dependency resolution",
);
assert.deepStrictEqual(
  dryRun.dryRunPlan.selectedResult.dataPacks,
  ["agora.telco"],
  "Generated Telco answers must not select Electronics data",
);

const tempParent = fs.mkdtempSync(
  path.join(os.tmpdir(), "nodics-builder-answers-"),
);
const answersPath = path.join(tempParent, "acme-mobile-answers.json");
const canonicalAnswersPath = path.join(
  fs.realpathSync(tempParent),
  "acme-mobile-answers.json",
);
const artifact = guidedService.writeAnswersTemplate(answersPath, answers, {
  protectedRoots: [frameworkRoot, expRoot, kickoffRoot],
});
assert.strictEqual(
  artifact.path,
  canonicalAnswersPath,
  "Answer template write must target the canonical explicit absent file",
);
assert.strictEqual(
...
assert.throws(
  () => guidedService.writeAnswersTemplate(answersPath, answers),
  /must be absent/,
  "Answer template write must reject overwriting existing files",
);
assert.throws(
  () =>
    guidedService.writeAnswersTemplate(
      path.join(frameworkRoot, "answers.json"),
      answers,
      {
        protectedRoots: [frameworkRoot],
      },
    ),
  /protected root|output parent/,
  "Answer template write must reject protected repository roots",
);

const commands = toolingCommandService.loadCommands(frameworkRoot);
assert.strictEqual(
  commands["builder:answers-template"].operation,
  "answers-template",
  "WP-B11 must expose beginner answers-template operation",
);
assert.strictEqual(
  commands["builder:answers-template"].handler,
...
const cliResult = JSON.parse(
  childProcess.execFileSync(
    ...
    [
      ...
      "--frontends=AGORA,AXIS",
      "--sample-data=true",
      "--output-root=/workspace/generated/acmeMobile",
...
assert.strictEqual(
  contractService.validateDocument("guided", cliResult.answers).valid,
  true,
...
console.log(
  "Application Builder WP-B11 beginner answers-template contract validated",
);
