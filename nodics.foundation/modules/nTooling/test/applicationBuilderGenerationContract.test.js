...
const agoraRoot = sourceWorkspace.frontend;
...
const catalogue = catalogueService.discover({
  framework: frameworkRoot,
  exp: expRoot,
  kickoff: sourceWorkspace.customer,
});
const tempParent = fs.mkdtempSync(
  path.join(os.tmpdir(), "nodics-builder-wp-b4-"),
);
...
function approvedPlan(solutionObject, reference) {
  const plan = planningService.createPlan(solutionObject, catalogue, {
    ...
    expiresAt: "2099-08-17T08:00:00.000Z",
  });
  return generationService.approvePlan(
    plan,
    reference,
  ...
  fs.readdirSync(directory, { withFileTypes: true })
    ...
    .forEach((entry) => {
      ...
      if (entry.isDirectory()) {
        ...
        result[path.relative(root, target).split(path.sep).join("/")] =
...
try {
  ["solution-commerce.json", "solution-apparel.json"].forEach((fileName) => {
    ...
    const approved = approvedPlan(
      solutionObject,
      "WP-B4-TEST-" + solutionObject.identity.projectCode,
    );
    const outputRoot = path.join(
      tempParent,
      solutionObject.identity.projectCode,
    );
    const result = generationService.generate(
      approved,
      solutionObject,
      catalogue,
      outputRoot,
    ...
    assert(
      fs.statSync(path.join(outputRoot, "backend/customer")).isDirectory(),
    ...
    assert(
      fs
        .statSync(path.join(outputRoot, "deployment/local-compose.yaml"))
        .isFile(),
    ...
    assert(
      fs
        .statSync(path.join(outputRoot, "integrations/frontend-wiring.json"))
        .isFile(),
      "Generated project must include frontend wiring metadata",
    );
    assert(
      fs
        .statSync(path.join(outputRoot, "backend/src/customerHooks.js"))
        .isFile(),
    ...
    const graph = JSON.parse(
      fs.readFileSync(
        path.join(
          outputRoot,
          "backend/generated",
          solutionObject.identity.projectCode + "-module-graph.json",
        ),
        "utf8",
      ),
    );
    ...
    assert(
      firstThirty.includes("npm test") &&
        firstThirty.includes("npm run verify:runtime"),
    ...
    assert(
      apiCatalogue.includes("/api/commerce/products") &&
        apiCatalogue.includes("/api/commerce/data-packs") &&
    ...
    assert(
      customization.includes("backend/customer") &&
        customization.includes("nodics.ai"),
      "Customization map must separate customer roots from framework ownership",
    );
    solutionObject.data.packs.forEach((dataPack) => {
      assert(
        fs
          .statSync(path.join(outputRoot, "data", dataPack, "manifest.json"))
          .isFile(),
        "Generated data pack must include a manifest: " + dataPack,
      );
      assert(
        fs
          .statSync(path.join(outputRoot, "data", dataPack, "pages/home.json"))
          .isFile(),
      ...
      assert(
        fs
          .statSync(
            path.join(outputRoot, "data", dataPack, "components/hero.json"),
          )
          .isFile(),
      ...
      const manifest = JSON.parse(
        fs.readFileSync(
          path.join(outputRoot, "data", dataPack, "manifest.json"),
          "utf8",
        ),
      );
      assert.strictEqual(
        manifest.ownership,
        "CUSTOMER_OWNED",
        "Generated data pack manifest must keep customer ownership: " +
          dataPack,
      );
    });
    const handoff = JSON.parse(
      fs.readFileSync(path.join(outputRoot, "builder-handoff.json"), "utf8"),
    );
    assert.strictEqual(
      handoff.project.projectCode,
      solutionObject.identity.projectCode,
      "Generated handoff must preserve project identity",
    );
    assert.deepStrictEqual(
      handoff.selected.backendCapabilities,
      graph.capabilities,
      "Generated handoff must expose the approved backend graph",
    );
    assert(
      handoff.ownership.framework.includes("nodics.ai"),
      "Generated handoff must explain framework ownership",
    );
    childProcess.execFileSync(
      process.execPath,
      [path.join(outputRoot, "verify-generated.js")],
      {
        cwd: outputRoot,
        stdio: "pipe",
      },
    ...
    childProcess.execFileSync("npm", ["run", "verify:runtime"], {
      cwd: outputRoot,
      stdio: "pipe",
    });
    const storefront = require(
      path.join(outputRoot, "storefront/src/server.js"),
    );
    assert.strictEqual(
      storefront.dataPackSamples.length,
      solutionObject.data.packs.length,
      "Generated storefront must expose selected data-pack summaries",
    );
    assert(
      storefront.page().includes('data-pack-summary="true"'),
      "Generated storefront must render selected customer data-pack summaries",
    );
    assert(
      fs.statSync(path.join(outputRoot, "backend/src/server.js")).isFile(),
      "Generated backend must include a bootable server entrypoint",
    );
    assert(
      fs.statSync(path.join(outputRoot, "storefront/src/server.js")).isFile(),
    ...
    assert.throws(
      ...
      "Generation must never overwrite an existing root",
    );
  });
  ...
  assert.deepStrictEqual(
    generatedTree(repeatRoot),
    generatedTree(path.join(tempParent, "referenceCommerce")),
  ...
  assert.throws(
    () =>
      generationService.generate(
        ...
        commerce,
        catalogue,
        path.join(tempParent, "tampered-output"),
      ),
    /digest is invalid|differs from the current canonical plan/,
  ...
  assert.throws(
    () =>
      generationService.generate(approved, commerce, catalogue, rollbackRoot, {
        now: "2026-08-16T08:10:00.000Z",
        failAfterOperation: 2,
      }),
    /Injected Builder generation failure/,
    "Injected failure must exercise rollback",
  );
  assert.strictEqual(
    fs.existsSync(rollbackRoot),
    false,
    "Rollback must remove only the root created by generation",
  );
  ...
  assert.throws(
    () =>
      generationService.resolveOutputRoot(path.join(symlinkParent, "output")),
    /symlinked output ancestors/,
    "Generation must reject symlinked output ancestors",
  );
  assert.throws(
    () => generationService.resolveOutputRoot("relative-output"),
    /absolute path/,
  ...
  fs.writeFileSync(
    cliPlanPath,
    JSON.stringify(
      planningService.createPlan(commerce, catalogue, {
        createdAt: "2026-08-16T08:00:00.000Z",
        expiresAt: "2099-08-17T08:00:00.000Z",
      }),
      null,
  ...
  const roots = ["--exp=" + expRoot, "--kickoff=" + sourceWorkspace.customer];
  const approvedOutput = childProcess.execFileSync(
    process.execPath,
    [cli, "builder:approve"].concat(roots, [
      "--plan=" + cliPlanPath,
      "--approval-reference=WP-B4-CLI",
    ]),
  ...
  const generatedOutput = JSON.parse(
    childProcess.execFileSync(
      process.execPath,
      [cli, "builder:generate"].concat(roots, [
        "--plan=" + cliApprovedPath,
        "--solution=" + cliSolutionPath,
        "--output=" + cliOutputRoot,
      ]),
      { cwd: frameworkRoot, encoding: "utf8" },
    ),
  );
  assert.strictEqual(
    ...
    true,
    "Governed CLI must execute the approved Commerce plan",
  );
  childProcess.execFileSync("npm", ["test"], {
    cwd: cliOutputRoot,
    stdio: "pipe",
