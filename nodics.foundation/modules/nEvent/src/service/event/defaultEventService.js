...
/**
 * @module nodics.foundation/modules/nEvent/src/service/event/defaultEventService
        ...
        return new Promise((resolve, reject) => {
            if (NODICS.getServerState() === 'started') {
                if (NODICS.getActiveChannel() !== 'test' && !NODICS.isNTestRunning()) {
                    const eventConfig = CONFIG.get('event') || {};
                    if (eventConfig.remotePublishEnabled === false) {
                        resolve({
                            code: 'SUC_EVNT_00002',
                            result: {
                                skipped: true,
                                reason: 'remote_event_publish_disabled'
                            }
                        });
                    } else if (eventConfig.publishAllActive) {
