...
global.CONFIG = { get: key => key === 'moduleIdentityAliases' ?
    { 'nodics.core': 'nodics.foundation' } : undefined };
assert.strictEqual(initService.normalizeModuleIdentity('nodics.core'), 'nodics.foundation',
    'legacy Core input must normalize to the single Foundation runtime identity');
assert.strictEqual(initService.normalizeModuleIdentity('nodics.platform'), 'nodics.platform',
    'canonical module identities must remain unchanged');
...
function moduleFixture(name, kind, parent, index) {
    return {
        ...
        path: path.join(repoRoot, 'nodics.foundation', 'modules', 'nConfig', 'test', 'fixtures', parent || '', name),
        parent: parent,
        children: [],
        index: index,
        metaData: {
