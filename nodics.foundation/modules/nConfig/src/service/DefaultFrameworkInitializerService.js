...
module.exports = {
    ...
    getServerConfigurationLoadOrder: function () {
        return [
            'project config: ' + NODICS.getEnvironmentPath() + '/config/properties.js',
            'environment config: ' + NODICS.getServerRootPath() + '/config/properties.js',
    ...
    getConfiguredActiveModuleGroups: function (serverProperties) {
        ...
        (serverProperties.activeModules ? serverProperties.activeModules.groups || [] : []).forEach(groupName => {
    ...
    normalizeModuleIdentity: function (moduleName) {
        let configurationReady = typeof CONFIG !== 'undefined' && CONFIG &&
            typeof CONFIG.get === 'function' && typeof CONFIG.getProperties === 'function' && CONFIG.getProperties();
        let aliases = configurationReady ? CONFIG.get('moduleIdentityAliases') || {} :
    ...
    resolveExtendedModuleGroups: function (moduleName, groups, visiting) {
        ...
        if (!moduleObject) this.failConfiguration('extends references unknown module: ' + moduleName);
        if (visiting.includes(moduleName)) {
            this.failConfiguration('circular module extends hierarchy: ' + visiting.concat([moduleName]).join(' -> '));
        }
        ...
        parents.filter(Boolean).forEach(parentName => {
            this.resolveExtendedModuleGroups(parentName, groups, visiting.concat([moduleName]));
        });
        if (metadata.kind === 'group' && !groups.includes(moduleName)) groups.push(moduleName);
    },
    ...
    validateSelectedRuntimeKinds: function () {
        this.validateModuleKind(NODICS.getEnvironmentName(), 'application', 'selected project');
    ...
    validateConfiguredModules: function (serverProperties) {
        ...
        this.getConfiguredActiveModuleGroups(serverProperties).forEach(groupName => {
            let moduleName = groupName;
    ...
    validateSelectedRuntimeHierarchy: function () {
        ...
        if (!environmentModule) {
            this.failConfiguration('selected project module is not valid: ' + environmentName);
        }
        if (!serverModule) {
        ...
        if (serverRootModule.parent !== environmentName) {
            this.failConfiguration('selected environment ' + serverRootName + ' must be a child of project ' + environmentName);
        }
        ...
        this.validateModuleIndexOrder(environmentModule, serverRootModule, 'project to environment');
        this.validateModuleIndexOrder(serverRootModule, serverModule, 'environment to server');
        if (NODICS.getNodeName()) {
