...
module.exports = {
    ...
    loadRawModuleList: function (homePath) {
        this.loadRawModuleRoots([homePath]);
    },

    /** Discovers one effective runtime from independently versioned repository roots. */
    loadRawModuleRoots: function (rootPaths) {
        let modulesList = {};
        let records = [];
        (rootPaths || []).forEach(rootPath => {
            if (!rootPath || !fs.existsSync(rootPath)) {
                throw new Error('Nodics module root does not exist: ' + rootPath);
            }
            this.collectModuleRecords(rootPath, records, null);
        });
