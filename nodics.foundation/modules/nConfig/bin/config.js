...
module.exports = function () {
  ...
  this.get = function (key, tenant) {
    ...
    if (!tntProperties) {
      this.LOG.error(
        "System could not find any properties for current tenant: ",
        tenant || this.getConfiguredTenantCodes(),
      );
      return null;
    }
  ...
  this.changeTenantProperties = function (config, tenant) {
    if (tenant) {
      const inherited = _tntProperties[tenant] || {};
      _tntProperties[tenant] = configurationBindings.merge(
        inherited,
        configurationBindings.resolve(config, inherited),
      );
    } else {
      const updates = this.getConfiguredTenantCodes().map((tenant) => {
        const inherited = _tntProperties[tenant] || {};
        return [
          tenant,
      ...
      updates.forEach(([tenant, properties]) => {
        _tntProperties[tenant] = properties;
      });
    }
  };
