...
const logs = [];
let apiExposure = require("../../nSystem/config/properties").apiExposure;
const logger = {
  debug: function () {},
...
global.SERVICE = {
  ...
  DefaultPipelineService: Object.assign(
    {},
    require("../../nPipeline/src/service/pipeline/defaultPipelineService"),
    { LOG: logger },
  ),
  DefaultRouterOperationService: Object.assign(
    {},
    require("../src/service/router/defaultRouterOperationService"),
    { LOG: logger },
  ),
  DefaultRouterService: {
    LOG: logger,
  },
  DefaultRequestHandlerService: Object.assign(
    {},
    require("../src/service/defaultRequestHandlerService"),
    { LOG: logger },
  ),
  DefaultRequestHandlerPipelineService: Object.assign(
    {},
    require("../src/service/request/defaultRequestHandlerPipelineService"),
    { LOG: logger },
  ),
  DefaultNonSecuredRequestPipelineService: Object.assign(
    {},
    require("../src/service/request/defaultNonSecuredRequestPipelineService"),
    { LOG: logger },
  ),
  DefaultJsonResponseHandlerService: Object.assign(
    {},
    require("../src/service/handlers/response/defaultJsonResponseHandlerService"),
    { LOG: logger },
  ),
  ...
  DefaultCacheService: {
    ...
    get: function () {
      return Promise.reject(
        new global.CLASSES.NodicsError({
...
(async function run() {
  controllerCalls = [];
  let successResponse = await execute(
    "success",
    {
  ...
  assert.strictEqual(
    ...
    "Controller request processed successfully",
  );
  ...
  assert.strictEqual(
    controllerErrorResponse.payload.message,
    "Controller rejected request",
  );
  assert.strictEqual(
    controllerErrorResponse.payload.contexts,
    undefined,
  ...
  assert.strictEqual(
    validationErrorResponse.payload.message,
    "Authentication or enterprise header is required",
  );
  assert.deepStrictEqual(controllerCalls, []);

  controllerCalls = [];
  let publicProbeResponse = await executeRoute(
    Object.assign(createRoute("live"), {
      publicProbe: true,
    }),
    {},
  );
  ...
  controllerCalls = [];
  let publicDocumentationResponse = await executeRoute(
    Object.assign(createRoute("success"), {
      publicAccess: true,
  ...
  assert.strictEqual(
    publicDocumentationResponse.payload.code,
    "SUC_TEST_00000",
  );
  ...
  controllerCalls = [];
  let tenantScopedPublicResponse = await executeRoute(
    Object.assign(createRoute("success"), {
      publicAccess: true,
    }),
    {
  ...
  controllerCalls = [];
  let persistedOpenApiRouteResponse = await executeRoute(
    Object.assign(createRoute("success"), {
      secured: true,
      apiExposure: "openApiContract",
    }),
  ...
  assert.strictEqual(
    persistedOpenApiRouteResponse.payload.code,
    "SUC_TEST_00000",
  );
  ...
  assert.strictEqual(
    disabledExposureResponse.payload.message,
    "Access denied: API category is disabled for this runtime: testExecution",
  );
  assert.deepStrictEqual(controllerCalls, []);
  ...
  assert.strictEqual(
    inactiveRouteResponse.payload.message,
    "Process failed with errors",
  );
  assert.strictEqual(
    inactiveRouteResponse.payload.metadata,
    undefined,
