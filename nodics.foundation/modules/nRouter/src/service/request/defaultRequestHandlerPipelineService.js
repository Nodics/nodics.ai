 ...
 * in a later-loaded module to change request governance without changing Nodics foundation code.
 *
...
module.exports = {
  ...
  validateApiExposure: function (request, response, process) {
    ...
    process.error(
      ...
      new CLASSES.NodicsError(
        "ERR_AUTH_00003",
        "API category is disabled for this runtime: " + category,
      ),
    );
  },
    ...
    if (request.originalUrl.endsWith("?help")) {
      if (request.router.help) {
        ...
        process.error(
          request,
          response,
          new CLASSES.NodicsError("ERR_HLP_00000"),
        );
      }
    } else {
      process.nextSuccess(request, response);
  ...
  normalizeAuthHeaders: function (request) {
    let authorizationHeader = this.getHeaderValue(request.httpRequest, [
      "Authorization",
    ]);
    let modernAuthToken = this.getBearerToken(authorizationHeader);
    let legacyAuthToken = this.getHeaderValue(request.httpRequest, [
      "authToken",
    ]);
    let modernApiKey = this.getHeaderValue(request.httpRequest, [
      "x-api-key",
      "X-API-Key",
    ]);
    let legacyApiKey = this.getHeaderValue(request.httpRequest, ["apiKey"]);
    let modernEntCode = this.getHeaderValue(request.httpRequest, [
      "x-enterprise-code",
      "X-Enterprise-Code",
    ]);
  ...
  parseHeader: function (request, response, process) {
    ...
    if (request.auth.deprecated) {
      this.LOG.warn(
        "Deprecated auth header(s) used: " +
          request.auth.legacyHeaders.join(", "),
      );
    }
    ...
    if (
      this.isPublicRequest(request.router) &&
      !request.auth.credentials.length &&
      !request.auth.entCode
    ) {
      process.nextSuccess(request, response);
      return;
    }
    if (!request.auth.credentials.length && !request.auth.entCode) {
      process.error(
        request,
        response,
        new CLASSES.NodicsError("ERR_AUTH_00002"),
      );
    } else {
      process.nextSuccess(request, response);
    }
  ...
  handleSpecialRequest: function (request, response, process) {
    if (request.special) {
      ...
      try {
        CONTROLLER[request.router.handler][request.router.operation](
          request,
          (error, success) => {
            if (error) {
  ...
  lookupCache: function (request, response, process) {
    ...
    try {
      let keyHash = UTILS.generateHash(
        SERVICE.DefaultCacheConfigurationService.createApiKey(request),
      );
      request.apiCacheKeyHash = request.router.prefix
        ? request.router.prefix + "_" + keyHash
        : keyHash;
      ...
      if (
        ...
        SERVICE.DefaultCacheService.get({
          ...
          channelName: SERVICE.DefaultCacheService.getRouterCacheChannel(
          ...
          if (error) {
            ...
            if (cacheDecision.cacheable) {
              SERVICE.DefaultCacheService.put({
                ...
                ttl: request.router.cache
                  ? request.router.cache.ttl
                  : undefined,
              })
                .then((cuccess) => {
