...
module.exports = {
    ...
    safeSearch: function (request, callback) {
        ...
        request.schemaName = 'localizationValue';
        if (callback) {
            FACADE.DefaultLocalizationValueFacade.safeSearch(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultLocalizationValueFacade.safeSearch(request);
        }
    },
    capabilities: function (request, callback) {
        request.schemaName = 'localizationValue';
        if (callback) {
            FACADE.DefaultLocalizationValueFacade.capabilities(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultLocalizationValueFacade.capabilities(request);
        }
    },
    ...
    deleteImpact: function (request, callback) {
        ...
        request.schemaName = 'localizationValue';
        if (callback) {
            FACADE.DefaultLocalizationValueFacade.deleteImpact(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultLocalizationValueFacade.deleteImpact(request);
        }
    },
    ...
    executeGenericMutation: function (request, callback, execute, operation) {
        let result = Promise.resolve().then(() => {
            SERVICE.DefaultSchemaAuthoringPolicyService.assertMutationAllowed(request.moduleName, 'localizationValue', operation);
            return execute();
        });
        if (callback) {
