...
module.exports = {
    ...
    safeSearch: function (request, callback) {
        ...
        request.schemaName = 'engagementBatchRun';
        if (callback) {
            FACADE.DefaultEngagementBatchRunFacade.safeSearch(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultEngagementBatchRunFacade.safeSearch(request);
        }
    },
    capabilities: function (request, callback) {
        request.schemaName = 'engagementBatchRun';
        if (callback) {
            FACADE.DefaultEngagementBatchRunFacade.capabilities(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultEngagementBatchRunFacade.capabilities(request);
        }
    },
    ...
    deleteImpact: function (request, callback) {
        ...
        request.schemaName = 'engagementBatchRun';
        if (callback) {
            FACADE.DefaultEngagementBatchRunFacade.deleteImpact(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultEngagementBatchRunFacade.deleteImpact(request);
        }
    },
    ...
    executeGenericMutation: function (request, callback, execute, operation) {
        let result = Promise.resolve().then(() => {
            SERVICE.DefaultSchemaAuthoringPolicyService.assertMutationAllowed(request.moduleName, 'engagementBatchRun', operation);
            return execute();
        });
        if (callback) {
