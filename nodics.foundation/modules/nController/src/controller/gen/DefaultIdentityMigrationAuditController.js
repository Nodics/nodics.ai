...
module.exports = {
    ...
    safeSearch: function (request, callback) {
        ...
        request.schemaName = 'identityMigrationAudit';
        if (callback) {
            FACADE.DefaultIdentityMigrationAuditFacade.safeSearch(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultIdentityMigrationAuditFacade.safeSearch(request);
        }
    },
    capabilities: function (request, callback) {
        request.schemaName = 'identityMigrationAudit';
        if (callback) {
            FACADE.DefaultIdentityMigrationAuditFacade.capabilities(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultIdentityMigrationAuditFacade.capabilities(request);
        }
    },
    ...
    deleteImpact: function (request, callback) {
        ...
        request.schemaName = 'identityMigrationAudit';
        if (callback) {
            FACADE.DefaultIdentityMigrationAuditFacade.deleteImpact(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultIdentityMigrationAuditFacade.deleteImpact(request);
        }
    },
    ...
    executeGenericMutation: function (request, callback, execute, operation) {
        let result = Promise.resolve().then(() => {
            SERVICE.DefaultSchemaAuthoringPolicyService.assertMutationAllowed(request.moduleName, 'identityMigrationAudit', operation);
            return execute();
        });
        if (callback) {
