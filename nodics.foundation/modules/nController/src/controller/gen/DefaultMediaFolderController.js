...
module.exports = {
    ...
    safeSearch: function (request, callback) {
        ...
        request.schemaName = 'mediaFolder';
        if (callback) {
            FACADE.DefaultMediaFolderFacade.safeSearch(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultMediaFolderFacade.safeSearch(request);
        }
    },
    capabilities: function (request, callback) {
        request.schemaName = 'mediaFolder';
        if (callback) {
            FACADE.DefaultMediaFolderFacade.capabilities(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultMediaFolderFacade.capabilities(request);
        }
    },
    ...
    deleteImpact: function (request, callback) {
        ...
        request.schemaName = 'mediaFolder';
        if (callback) {
            FACADE.DefaultMediaFolderFacade.deleteImpact(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultMediaFolderFacade.deleteImpact(request);
        }
    },
    ...
    executeGenericMutation: function (request, callback, execute, operation) {
        let result = Promise.resolve().then(() => {
            SERVICE.DefaultSchemaAuthoringPolicyService.assertMutationAllowed(request.moduleName, 'mediaFolder', operation);
            return execute();
        });
        if (callback) {
