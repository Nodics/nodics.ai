...
module.exports = {
    ...
    safeSearch: function (request, callback) {
        ...
        request.schemaName = 'principalScopeAssignment';
        if (callback) {
            FACADE.DefaultPrincipalScopeAssignmentFacade.safeSearch(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultPrincipalScopeAssignmentFacade.safeSearch(request);
        }
    },
    capabilities: function (request, callback) {
        request.schemaName = 'principalScopeAssignment';
        if (callback) {
            FACADE.DefaultPrincipalScopeAssignmentFacade.capabilities(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultPrincipalScopeAssignmentFacade.capabilities(request);
        }
    },
    ...
    deleteImpact: function (request, callback) {
        ...
        request.schemaName = 'principalScopeAssignment';
        if (callback) {
            FACADE.DefaultPrincipalScopeAssignmentFacade.deleteImpact(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultPrincipalScopeAssignmentFacade.deleteImpact(request);
        }
    },
    ...
    executeGenericMutation: function (request, callback, execute, operation) {
        let result = Promise.resolve().then(() => {
            SERVICE.DefaultSchemaAuthoringPolicyService.assertMutationAllowed(request.moduleName, 'principalScopeAssignment', operation);
            return execute();
        });
        if (callback) {
