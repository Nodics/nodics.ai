...
module.exports = {
    ...
    safeSearch: function (request, callback) {
        ...
        request.schemaName = 'customerReviewHelpfulness';
        if (callback) {
            FACADE.DefaultCustomerReviewHelpfulnessFacade.safeSearch(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultCustomerReviewHelpfulnessFacade.safeSearch(request);
        }
    },
    capabilities: function (request, callback) {
        request.schemaName = 'customerReviewHelpfulness';
        if (callback) {
            FACADE.DefaultCustomerReviewHelpfulnessFacade.capabilities(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultCustomerReviewHelpfulnessFacade.capabilities(request);
        }
    },
    ...
    deleteImpact: function (request, callback) {
        ...
        request.schemaName = 'customerReviewHelpfulness';
        if (callback) {
            FACADE.DefaultCustomerReviewHelpfulnessFacade.deleteImpact(request).then(success => {
                callback(null, success);
            }).catch(error => {
                callback(error);
            });
        } else {
            return FACADE.DefaultCustomerReviewHelpfulnessFacade.deleteImpact(request);
        }
    },
    ...
    executeGenericMutation: function (request, callback, execute, operation) {
        let result = Promise.resolve().then(() => {
            SERVICE.DefaultSchemaAuthoringPolicyService.assertMutationAllowed(request.moduleName, 'customerReviewHelpfulness', operation);
            return execute();
        });
        if (callback) {
