...
Projects may configure `data.dataReleases.initializationProfiles` on a target
runtime. A profile contains bounded `init`, `core`, or `sample` steps and may
optionally select explicit immutable release codes. nImport discovers the
actual releases, calculates current/action/blocked state, validates the complete
plan without writes, and executes steps sequentially through the existing
release authority. A retry skips already-current releases and retains failed or
pending releases as actionable. Clients must not calculate ordering or execute
profile steps themselves.

## Publication review metadata

An immutable publishable data-release section may provide a client-safe
`publicationReview` contract. nImport validates its source/target roles,
site/catalog scope, bounded entity change counts, impact/recovery guidance, and
post-publication capabilities before exposing it in the release catalogue.
The metadata is descriptive and non-executable. Publication authority remains
with nPublish and the owning domain workflow; clients must bind approval to the
server-projected release checksum, publication code, and workflow reference.
...
3. Axis uploads the selected file through the secured media upload route.
4. media validates upload policy, stores the file with the active provider,
...
5. Axis starts the media-backed import route with the selected target model and
   `mediaCode`.
6. nImport asks media for a backend-only import source descriptor, stages the
...
The Back Office file-import journey must compose `media` and `nImport`
...
1. Axis uploads the file to `media` using `folderCode=importSources` and
   `formatCode=importFile`.
2. `media` validates the upload request, file size, MIME type, extension,
...
6. `DefaultMediaImportSourceStagingService` asks `media` for a trusted backend
...
authorities. `media` owns file storage. `nImport` owns import execution. Axis
...
`media`, creates a run-owned `data/`, `success/`, and `error/` workspace,
...
- use `media` provider behavior to read or stage the file safely;
...
- overriding allowed extensions or maximum file sizes in layered
  `properties.js`;
- adding a new media storage provider behind the `media` provider contract;
...
- parsing import data inside `media`;
