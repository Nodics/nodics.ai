/*
    Nodics - Enterprice Micro-Services Management Framework

    Copyright (c) 2026 Nodics All rights reserved.

    This software is governed by the Nodics Source-Available Commercial License.
    You may use, copy, modify, deploy, or distribute it only as permitted by the
    root LICENSE file or a separate written agreement with Nodics.

 */
/**
 * @generated
 * @module generated/facade/DefaultCmsOnlinePublicationPointerFacade
 * @description Generated facade for schema `cmsOnlinePublicationPointer` owned by module `cms`. This file is recreated by clean/build from the effective schema and common facade template.
 * @layer facade
 * @owner cms
 * @schema cmsOnlinePublicationPointer
 * @model CmsOnlinePublicationPointerModel
 * @sourceTemplate /src/facade/common.js
 * @override Do not edit generated files directly. Customize behavior by adding a later module in the hierarchy that overrides this generated artifact or its source template contract.
 */
module.exports = {
    init: function (options) {
        return new Promise((resolve, reject) => {
            resolve(true);
        });
    },
    postInit: function (options) {
        return new Promise((resolve, reject) => {
            resolve(true);
        });
    },
    get: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.get(request);
    },
    safeSearch: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.safeSearch(request);
    },
    capabilities: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.capabilities(request);
    },
    getById: function (id, tenant) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.getById(id, tenant);
    },
    getByCode: function (code, tenant) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.getByCode(code, tenant);
    },
    save: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.save(request);
    },
    saveAll: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.saveAll(request);
    },
    remove: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.remove(request);
    },
    deleteImpact: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.deleteImpact(request);
    },
    removeById: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.remove(request);
    },
    removeByCode: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.remove(request);
    },
    update: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.update(request);
    },
    doRefresh: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doRefresh(request);
    },
    doCheckHealth: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doCheckHealth(request);
    },
    doExists: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doExists(request);
    },
    doGet: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doGet(request);
    },
    doSearch: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doSearch(request);
    },
    doSave: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doSave(request);
    },
    doBulk: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doBulk(request);
    },
    doRemove: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doRemove(request);
    },
    doRemoveByQuery: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doRemoveByQuery(request);
    },
    doGetSchema: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doGetSchema(request);
    },
    doUpdateSchema: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doUpdateSchema(request);
    },
    doRemoveIndex: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doRemoveIndex(request);
    },
    doIndexing: function (request) {
        return SERVICE.DefaultCmsOnlinePublicationPointerService.doIndexing(request);
    }
};