  ...
  global.SERVICE = {
    ...
    DefaultLoyaltyRewardPaymentProviderService: {
      ...
      execute: async (request) => ({
        status: "REFUNDED",
        reference: request.reversalOfEntryCode || request.providerReference,
        loyalty: { walletCode: request.walletCode },
      }),
    },
      ...
      get: async (request) => ({
        result: transactions.filter((item) =>
          Object.entries(request.query).every(
            ([key, value]) =>
              key.split(".").reduce((v, k) => v?.[k], item) === value,
          ),
        ),
      }),
      save: async (request) => {
...
test("Payment refund execution supports partial delayed refunds and records reconciliation evidence", async () => {
  const result = await refund.executeRefund({
    ...
    payload: {
      amount: "12.50",
      currency: "USD",
      providerToken: "tok_test_refund_delay",
    },
...
test("Payment refund execution converts provider failure into action-required reconciliation", async () => {
  const result = await refund.executeRefund({
    ...
    payload: {
      amount: "10.00",
      currency: "USD",
      providerToken: "tok_test_refund_fail",
    },
