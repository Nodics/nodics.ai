...
module.exports = {
  /** Calculates a tenant cart using exact arithmetic and the Pricing, Tax, Promotion and Inventory ports; missing owner ports fail. */
  calculate: async function (cart, ports) {
    if (
      ...
      throw new Error("Complete tenant cart and owner ports are required");
    let subtotal = "0";
    const entries = [];
    for (const entry of cart.entries) {
      const availability = await ports.inventory({
        tenant: cart.tenant,
    ...
    const discount = await ports.promotion({
      tenant: cart.tenant,
      enterpriseCode: cart.enterpriseCode,
      ownerId: cart.ownerId,
      cartCode: cart.code,
      subtotal,
      currency: cart.currency,
      productCodes: entries.map((entry) => entry.productCode),
      customerGroup: cart.customerGroup,
      couponCode: cart.couponCode,
      idempotencyKey: cart.idempotencyKey,
    });
    const taxable = ports.exact.add(
      subtotal,
      "-" + ports.exact.normalize(discount.discountAmount || "0"),
    );
    const tax = await ports.tax({
      tenant: cart.tenant,
    ...
    return Object.freeze({
      ...
      entries,
      subtotal,
      discountAmount: ports.exact.normalize(discount.discountAmount || "0"),
      taxAmount: tax.taxAmount,
      totalAmount: ports.exact.add(taxable, tax.taxAmount),
      currency: cart.currency,
      decisions: { discount, tax },
      correlationId: cart.correlationId,
    });
  },
};
