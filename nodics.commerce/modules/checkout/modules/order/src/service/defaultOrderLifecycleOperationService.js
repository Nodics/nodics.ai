...
module.exports = {
  ...
  repository: function () {
    return SERVICE.DefaultOrderLifecycleRepositoryService;
  },
    ...
    return Object.assign(
      {
        ...
        created: now,
        updated: now,
        occurredAt: model.occurredAt || now,
      },
      model,
    );
  },
  ...
  policyFor: function (requestType) {
    ...
    return (
      policies[requestType] || {
        eligible: false,
        reasonCodes: [],
        downstreamOwners: ["order"],
      }
    );
  },
  ...
  evidence: function (request) {
    ...
    return Object.assign(evidence, {
      quantity:
        Number.isFinite(quantity) && quantity > 0 ? String(quantity) : "1",
      productCodes: Array.isArray(productCodes)
        ? productCodes
        : [productCodes].filter(Boolean),
      returnMethod: evidence.returnMethod || input.returnMethod,
      refundMethod:
        evidence.refundMethod || input.refundMethod || "ORIGINAL_PAYMENT",
      customerComment:
        evidence.customerComment || evidence.comment || input.comment,
      replacementProductCode:
        evidence.replacementProductCode || input.replacementProductCode,
      preferredResolution:
        evidence.preferredResolution || input.preferredResolution,
      appealReferenceCode:
        evidence.appealReferenceCode || input.appealReferenceCode,
  ...
  refundPreview: function (request, evidence) {
    ...
    return {
      ...
      reconciliationRequired:
        evidence.reconciliationStatus === "REQUIRED" ||
        (request.payload && request.payload.requestType) === "REFUND",
    };
  },
  ...
  automationPlan: function (request, policy, evidence) {
    ...
    if (requestType === "CANCELLATION") {
      ...
      plan.push({
        step: "digital-revocation",
        owner: "digitalCommerce",
        trigger: "digital entitlement policy",
        customerVisibleState: "Digital purchase review",
      });
    }
    if (["RETURN", "EXCHANGE", "REPLACEMENT"].includes(requestType)) {
      plan.push({
        ...
        owner: "fulfillment",
        trigger: evidence.returnMethod || "RETURN_METHOD_REQUIRED",
        customerVisibleState: "Return method selected",
      });
      plan.push({
        step: "inspection-disposition",
    ...
    if (["EXCHANGE", "REPLACEMENT"].includes(requestType)) {
      plan.push({
        ...
        owner: "inventory",
        trigger:
          evidence.replacementProductCode ||
          evidence.preferredResolution ||
          "REPLACEMENT_SELECTION_REQUIRED",
      ...
      plan.push({
        ...
        customerVisibleState: "Replacement shipment preparation",
      });
    }
    if (["RETURN", "REFUND", "EXCHANGE"].includes(requestType)) {
      plan.push({
        ...
        owner: "payment",
        trigger: evidence.refundMethod || "ORIGINAL_PAYMENT",
        customerVisibleState: "Refund review in progress",
      });
    }
    if (["CANCELLATION", "RETURN", "REFUND"].includes(requestType)) {
      plan.push({
        step: "digital-reversal-policy",
        owner: "digitalCommerce",
        trigger: "claim and redemption state",
        customerVisibleState: "Digital eligibility checked",
      });
    }
    if (requestType === "APPEAL" || policy.rejectionAppealSupported === true) {
      plan.push({
        step: "appeal-sla-review",
        owner: "workflow+order",
        trigger: evidence.appealReferenceCode || "REJECTION_OR_DELAY",
        customerVisibleState:
          requestType === "APPEAL"
            ? "Appeal submitted"
            : "Appeal available if rejected",
      });
    }
    return plan;
  },
  ...
  preview: async function (request) {
    if (
      ...
      ![
        ...
        "REPLACEMENT",
        "APPEAL",
      ].includes(request.payload.requestType)
    )
      throw new Error("Order and lifecycle request type are required");
    const policy = this.policyFor(request.payload.requestType);
    const evidence = this.evidence(request);
    const requiresRma = ["RETURN", "EXCHANGE", "REPLACEMENT"].includes(
    ...
    const preview = {
      ...
      replacementSelectionRequired:
        policy.replacementSelectionRequired === true,
      ...
      rmaCode: requiresRma
        ? evidence.rmaCode || `${request.orderCode}:RMA:${Date.now()}`
        : undefined,
  ...
  downstreamActionEvidence: async function (request, record) {
    ...
    if (
      ...
      const replacementRequest = {
        ...
        actorId: request.actorId,
        idempotencyKey:
          payload.inventoryIdempotencyKey ||
          [record.code, request.actionCode, "replacement"].join(":"),
        payload: Object.assign({}, payload, {
          productCodes: payload.productCodes || recordEvidence.productCodes,
          replacementProductCode:
            payload.replacementProductCode ||
            recordEvidence.replacementProductCode,
          preferredResolution:
            payload.preferredResolution || recordEvidence.preferredResolution,
        }),
        correlationId: request.correlationId || request.requestId,
        authData: request.authData,
      };
      if (
        SERVICE.DefaultInventoryReplacementReservationService &&
        typeof SERVICE.DefaultInventoryReplacementReservationService
          .reserveReplacement === "function"
      ) {
        evidence.inventory =
          await SERVICE.DefaultInventoryReplacementReservationService.reserveReplacement(
            replacementRequest,
          );
      }
      if (
        SERVICE.DefaultFulfillmentExchangeShipmentService &&
        typeof SERVICE.DefaultFulfillmentExchangeShipmentService
          .createExchangeShipment === "function"
      ) {
        evidence.fulfillment =
          await SERVICE.DefaultFulfillmentExchangeShipmentService.createExchangeShipment(
            Object.assign({}, replacementRequest, {
              idempotencyKey:
                payload.fulfillmentIdempotencyKey ||
                [record.code, request.actionCode, "exchangeShipment"].join(":"),
            }),
          );
      }
    }
    if (
      record.requestType === "APPEAL" &&
      request.actionCode === "APPROVE" &&
      SERVICE.DefaultWorkflowAppealSlaService &&
      typeof SERVICE.DefaultWorkflowAppealSlaService.startAppealReview ===
        "function"
    ) {
      evidence.workflow =
        await SERVICE.DefaultWorkflowAppealSlaService.startAppealReview({
          tenant: request.tenant,
          ownerId: record.ownerId,
          orderCode: record.orderCode,
          ...
          idempotencyKey:
            ...
            [record.code, request.actionCode, "appealSla"].join(":"),
          payload: Object.assign({}, payload, {
            appealReferenceCode:
              payload.appealReferenceCode || recordEvidence.appealReferenceCode,
  ...
  action: async function (request) {
    ...
    if (record.evidence?.caseCode && record.evidence?.approval)
      throw new Error(
        "Use the scoped refund recovery operation for this record",
      );
    if (record.requestType === "DISPUTE")
      throw new Error("Use the scoped manual review operation for this case");
    if (record.ownerId === request.actorId)
    ...
    return this.repository().update(
      ...
      record,
      {
        status: this.actionStatus(request.actionCode),
