...
module.exports = {
     ...
     * @override Later-loaded modules may replace this member through the standard merge contract.
     */
    supportedTypes: function () { return new Set(this.policy().supportedListTypes || ['WISHLIST', 'COMPARE', 'SAVE_FOR_LATER']); },
    /**
    ...
    normalizeType: function (listType) {
        ...
        if (!this.supportedTypes().has(value)) throw new Error('Unsupported shopping list type');
        return value;
    },
    /**
    ...
    limit: function (listType) {
        if (listType === 'COMPARE') return Number(this.policy().maximumCompareItems || 4);
        if (listType === 'SAVE_FOR_LATER') return Number(this.policy().maximumSaveForLaterItems || this.policy().maximumWishlistItems || 100);
        return Number(this.policy().maximumWishlistItems || 100);
    },
    /**
    ...
    loadEntries: async function (request, list) {
        const response = await SERVICE.DefaultShoppingListEntryService.get({ tenant: request.tenant, authData: request.authData, query: { tenant: request.tenant, ownerId: request.ownerId, listCode: list.code, status: 'ACTIVE' }, pageSize: this.limit(list.listType) + 1 });
        const result = this.unwrap(response);
    ...
    addEntry: async function (request) {
        const payload = request.payload || {};
        if (!payload.productCode) throw new Error('Product code is required for shopping list entry');
        ...
        if (!existing && current.length >= this.limit(list.listType)) throw new Error('Shopping list item limit exceeded');
        const model = {
            code,
            tenant: request.tenant,
            ownerId: request.ownerId,
        ...
        await SERVICE.DefaultShoppingListEntryService.save({ tenant: request.tenant, authData: request.authData, model }).then(this.unwrap);
    ...
    removeEntry: async function (request) {
        if (!request.entryCode) throw new Error('Shopping list entry code is required');
        const list = await this.loadList(request);
        ...
        if (SERVICE.DefaultShoppingListEntryService.update) await SERVICE.DefaultShoppingListEntryService.update({ tenant: request.tenant, authData: request.authData, query: { tenant: request.tenant, ownerId: request.ownerId, code: request.entryCode, listCode: list.code }, model }).then(this.unwrap);
        else await SERVICE.DefaultShoppingListEntryService.save({ tenant: request.tenant, authData: request.authData, model }).then(this.unwrap);
        return this.response(request, list);
    },
    /**
