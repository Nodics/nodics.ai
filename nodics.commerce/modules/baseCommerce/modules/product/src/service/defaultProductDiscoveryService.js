...
module.exports = {
  ...
  policy: function () {
    return (CONFIG.get("product") || {}).discovery || {};
  },
  ...
  serviceAuthData: function (request) {
    return Object.assign({}, request.authData || {}, {
      entCode:
        request.entCode ||
        (request.enterprise && request.enterprise.code) ||
  ...
  indexConfiguration: async function (request) {
    ...
    if (
      !SERVICE.DefaultDiscoveryConfigurationResolverService ||
      typeof SERVICE.DefaultDiscoveryConfigurationResolverService
        .resolveIndexConfiguration !== "function"
    )
      return undefined;
    try {
      return await SERVICE.DefaultDiscoveryConfigurationResolverService.resolveIndexConfiguration(
        {
          ...
          indexCode:
            request.query &&
            (request.query.indexCode || request.query.indexConfigurationCode),
  ...
  discoveryMetadata: function (request, configuration) {
    ...
    return {
      ...
      rankingProfileCode:
        (configuration && configuration.rankingProfileCode) ||
        (request.query && request.query.rankingProfileCode),
      indexName:
        (configuration && configuration.indexName) ||
        policy.searchIndexName ||
        "productLocalized",
      storeCode: request.storeCode,
      locale: request.locale,
    };
  ...
  query: function (request) {
    ...
    if (input.categoryCode || input.collectionCode)
      query["payload.categoryCodes"] =
        input.categoryCode || input.collectionCode;
    if (input.domainCode)
      query["payload.classificationValues.domain"] = input.domainCode;
  ...
  searchOptions: function (request) {
    let policy = this.policy(),
      input = request.query || {},
      maximum = Number(policy.maximumPageSize || 100);
    if (
      SERVICE.DefaultDiscoveryQueryBuilderService &&
      typeof SERVICE.DefaultDiscoveryQueryBuilderService.options === "function"
    ) {
    ...
    return {
      pageSize: this.boundedInteger(
        input.pageSize || input.limit,
        Number(policy.defaultPageSize || 24),
        1,
        maximum,
      ),
  ...
  search: async function (request, query, searchOptions) {
    ...
    let searchRequest = {
      ...
      moduleName: "product",
      indexName:
        (request.indexConfiguration && request.indexConfiguration.indexName) ||
        policy.searchIndexName ||
        "productLocalized",
      query: query,
    ...
    if (typeof service.doSearch === "function") {
      ...
      records = await this.completePartialPageFromProjectionStore(
        request,
        searchRequest,
        query,
        records,
  ...
  completePartialPageFromProjectionStore: async function (
    request,
    searchRequest,
    query,
    records,
    ...
    if (
      ...
      records.length === 0 ||
      !requestedPageSize ||
      records.length >= requestedPageSize
    )
      return records;
    if (
      ((CONFIG.get("product") || {}).discovery || {})
        .projectionStoreFallback === false
    )
      return records;
    let fallbackRecords = await this.projectionStoreRecords(
      Object.assign({}, searchRequest, {
        query: this.projectionStoreQuery(query),
        searchOptions: query.text
          ? this.textSearchFallbackOptions(searchOptions)
    ...
    if (query.text)
      fallbackRecords = this.paginateRecords(
        this.filterByText(fallbackRecords, query.text),
        searchOptions,
      );
  ...
  textSearchFallbackOptions: function (options) {
    let policy = this.policy(),
      maximum = Number(
        policy.textSearchFallbackCandidateLimit ||
  ...
  paginateRecords: function (records, options) {
    let pageSize = this.boundedInteger(
      options && (options.pageSize || options.limit),
      Number(this.policy().defaultPageSize || 24),
      1,
      Number(this.policy().maximumPageSize || 100),
    );
    let pageNumber = this.boundedInteger(
      options && (options.pageNumber || options.page),
      1,
      1,
      10000,
    );
  ...
  searchIndexRecords: async function (request) {
    let searchModel =
      global.NODICS && typeof NODICS.getSearchModel === "function"
        ? NODICS.getSearchModel(
            request.moduleName || "product",
            request.tenant,
            request.indexName || "productLocalized",
          )
        : undefined;
    if (!searchModel || typeof searchModel.doSearch !== "function") return [];
    let searchRequest = Object.assign({}, request, {
    ...
    if (
      ...
      return this.records(
        await SERVICE.DefaultPipelineService.start(
  ...
  textMatches: function (projection, term) {
    let payload = (projection && projection.payload) || {},
      localizedAttributes = payload.localizedAttributes || {};
    let values = [
      projection && projection.productCode,
      payload.code,
      payload.name,
      payload.description,
      payload.slug,
      payload.seo && payload.seo.title,
      payload.seo && payload.seo.description,
    ].concat(
  ...
  collectTextValues: function (value) {
    ...
    if (Array.isArray(value))
      return value.flatMap((item) => this.collectTextValues(item));
    if (typeof value === "object")
      return Object.keys(value).flatMap((key) =>
        this.collectTextValues(value[key]),
      );
    return [];
  },
  ...
  projectionStoreRecords: async function (request) {
    let options = request.searchOptions || {},
      pageSize = this.boundedInteger(
    ...
    let bestRecords = [];
    if (
      SERVICE.DefaultProductSearchProjectionService &&
      typeof SERVICE.DefaultProductSearchProjectionService.get === "function"
    ) {
      let response =
        await SERVICE.DefaultProductSearchProjectionService.get(pagedRequest);
      generatedRecords = this.records(response);
      bestRecords = generatedRecords;
      if (generatedRecords.length > 0 && generatedRecords.length >= pageSize)
        return generatedRecords;
    }
    let models = [
      request.schemaModel,
      this.projectionStoreModel(request),
      this.rawProjectionStoreModel(request),
    ].filter(Boolean);
    for (let model of models) {
      let records = await this.readProjectionStoreModelRecords(
        model,
        request.query || {},
        options,
        pageNumber,
  ...
  readProjectionStoreModelRecords: async function (
    model,
    query,
    options,
    pageNumber,
    ...
    if (typeof model.find === "function") {
      ...
      let records = await this.readProjectionCursor(
        cursor,
        options,
        pageNumber,
        pageSize,
  ...
  readProjectionCursor: async function (cursor, options, pageNumber, pageSize) {
    ...
    if (typeof cursor.skip === "function")
      cursor = cursor.skip((pageNumber - 1) * pageSize);
  ...
  projectionStoreModel: function (request) {
    ...
    if (global.NODICS && typeof NODICS.getModels === "function") {
      ...
      if (models.ProductSearchProjectionModel)
        return models.ProductSearchProjectionModel;
    }
    if (
      !SERVICE.DefaultDatabaseConfigurationService ||
      typeof SERVICE.DefaultDatabaseConfigurationService.getTenantDatabase !==
        "function"
    )
      return undefined;
    let database =
      SERVICE.DefaultDatabaseConfigurationService.getTenantDatabase(
        moduleName,
        tenant,
      );
    let master = database && (database.master || database.default || database);
    let connection =
      master && typeof master.getConnection === "function"
        ? master.getConnection()
        : undefined;
    return connection && typeof connection.collection === "function"
      ? connection.collection("ProductSearchProjectionModel")
      : undefined;
  },
  ...
  mediaDeliveryBaseUrl: function () {
    let value =
      this.policy().mediaDeliveryBaseUrl || "/nodics/media/v0/content";
  ...
  mediaDeliveryUrl: function (mediaCode) {
    return mediaCode
      ? this.mediaDeliveryBaseUrl() + "/" + encodeURIComponent(mediaCode)
      : undefined;
  },
  ...
  mediaDescriptor: function (candidate, role, altText) {
    ...
    let descriptor = {
      ...
      publicUrl:
        candidate.publicUrl || directUrl || this.mediaDeliveryUrl(code),
      altText: candidate.altText || altText,
    };
    Object.keys(descriptor).forEach(
      (key) => descriptor[key] === undefined && delete descriptor[key],
    );
    return descriptor.deliveryUrl || descriptor.mediaCode
      ? descriptor
      : undefined;
  },
  ...
  media: function (payload) {
    ...
    let primary = this.mediaDescriptor(
      productMedia.primary || productMedia.primaryImage,
      "primary",
      productMedia.primaryAlt || payload.name,
    );
    let secondary = this.mediaDescriptor(
      productMedia.secondary || productMedia.secondaryImage,
      "secondary",
      productMedia.primaryAlt || payload.name,
    );
    ...
    if (
      primary &&
      !gallery.some(
        (item) => item.mediaCode && item.mediaCode === primary.mediaCode,
      )
    )
    ...
    if (
      secondary &&
      !gallery.some(
        (item) => item.mediaCode && item.mediaCode === secondary.mediaCode,
      )
    )
      gallery.push(secondary);
    return {
      primary: primary,
      secondary: secondary,
      gallery: gallery,
  ...
  gallery: function (payload) {
    return this.media(payload)
      .gallery.map((item) => item.deliveryUrl)
      .filter(Boolean);
  },
  ...
  facets: function (products) {
    ...
    (products || []).forEach((product) => {
      (product.categoryCodes || []).forEach((code) =>
    ...
    return {
      categories: Array.from(categories.entries()).map((entry) => ({
        code: entry[0],
        count: entry[1],
      })),
      availability: Array.from(availability.entries()).map((entry) => ({
        code: entry[0],
        count: entry[1],
      })),
    };
  ...
  list: async function (request) {
    ...
    if (this.policy().catalogue && this.policy().catalogue.enabled === true) {
      return SERVICE.DefaultProductCatalogueQueryService.list(request);
    }
    let searchOptions = this.searchOptions(request),
      records = await this.search(request, this.query(request), searchOptions);
  ...
  detail: async function (request) {
    if (
      !request.productCode ||
      !/^[A-Za-z][A-Za-z0-9._-]{0,127}$/.test(String(request.productCode))
    ) {
      throw new Error("Product code is invalid");
    }
    request.indexConfiguration = await this.indexConfiguration(request);
    let records = await this.search(request, this.query(request), {
      limit: 2,
      pageSize: 2,
      pageNumber: 1,
    });
