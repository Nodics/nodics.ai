...
const navigation = providers.flatMap(capability => capability.navigation); assert.equal(navigation.length, 26);
...
assert.equal(reversal.length, 3); assert(reversal.every(item => item.parentId === 'order-lifecycle-operations'));
