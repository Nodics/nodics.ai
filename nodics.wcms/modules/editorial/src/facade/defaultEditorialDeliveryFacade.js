...
module.exports = {
    ...
    /** Lists visible articles with structured data. */ listStructured: async function (request) { let result = await SERVICE.DefaultEditorialDeliveryService.list(request); return { items: result.items.map(item => ({ article: item, structuredData: SERVICE.DefaultEditorialStructuredDataService.project(item) })), limit: result.limit }; },
    /** Builds a bounded RSS projection. */ rss: async function (request) { return SERVICE.DefaultEditorialSyndicationService.rss((await SERVICE.DefaultEditorialDeliveryService.list(request)).items); },
    /** Builds a bounded sitemap projection. */ sitemap: async function (request) { return SERVICE.DefaultEditorialSyndicationService.sitemap((await SERVICE.DefaultEditorialDeliveryService.list(request)).items); }
};
