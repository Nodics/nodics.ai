...
/**
 * @module nodics.wcms/media/test/mediaContextPolicyContract
...
(async function () {
    ...
    assert(
        ...
        'later configuration layers must be able to add media contexts without changing media source'
    );

    console.log('media context policy contract validated');
})().catch(error => {
    console.error(error);
    process.exit(1);
});
