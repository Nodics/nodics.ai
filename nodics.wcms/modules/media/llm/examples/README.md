...
The caller does not provide a filesystem path. `media` chooses the provider,
...
```text
customer.project/envs/local/mediaServer/temp/media/data/import/default/default/tenant/2026/07/tenant-upload-july.xlsx
```
...
`nRouter` invokes the media route-declared parser. `media` parses and
...
`nImport` resolves the trusted media descriptor through `media`, copies or
