...
This file gives AI coding agents mandatory guidance for the Nodics WCMS media capability.
...
`nodics.wcms/media` is the framework authority for media lifecycle:
...
`media` does not own product, CMS, import, documentation, or storefront business meaning. Those modules reference media or media sets when they need assets.
...
owning module. `media` owns the reusable set of files and variants; Product
...
- Do not add a new upload parser or router-level body parser from inside `media`. HTTP request parsing belongs to `gFramework/nRouter`; `media` may consume parsed file descriptors.
...
- NAS, S3, Azure Blob, Google Cloud Storage, FTP, SFTP, CDN, or partner storage
  integrations must be provider services behind `media`;
...
- public callers may receive a safe media identity and delivery descriptor
  only through an media-owned contract;
- media delivery must go through `/nodics/media/v0/content/{mediaCode}` or a
  later media-owned delivery route. Never expose local filesystem paths,
...
- public delivery may serve only policy-allowed `PUBLIC` media. `SIGNED` and
  `PRIVATE` delivery must remain blocked until a real media access policy is
