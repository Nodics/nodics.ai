...
module.exports = {
  ...
  getPersistenceAuthData: function (authData) {
    ...
    return identity && typeof identity.getSystemAuthData === "function"
      ? identity.getSystemAuthData()
      : authData;
  },
  ...
  getModel: function (tenant) {
    ...
    if (!model)
      throw new Error("Functional-module catalogue model is unavailable");
    return model;
  },
  ...
  getReceiptModel: function (tenant) {
    let models = NODICS.getModels("backoffice", tenant);
    let model =
      models && models.BackofficeFunctionalModuleActivationReceiptModel;
    if (!model)
      throw new Error(
        "Functional-module activation receipt model is unavailable",
      );
    return model;
  },
  ...
  getRecords: function (request) {
    ...
    return SERVICE.DefaultPipelineService.start(
  ...
  getAllRecords: async function (request, receipts) {
    ...
    let pageSize = Number(
      config.eligibilityPageSize || CONFIG.get("defaultPageSize"),
    );
    if (!Number.isSafeInteger(pageSize) || pageSize < 1) {
      throw new CLASSES.NodicsError(
        "ERR_BOF_00000",
        "Invalid functional-module eligibility page size",
      );
    }
    let records = [];
    let reader = receipts ? this.getReceiptRecords : this.getRecords;
    for (let pageNumber = 1; ; pageNumber++) {
      let response = await reader.call(
        this,
        Object.assign({}, request, {
          searchOptions: {
            ...
            sort: receipts
              ? { packageCode: 1, code: 1 }
              : { functionalModule: 1, code: 1 },
          },
        }),
      );
      if (!response || !Array.isArray(response.result)) {
        throw new CLASSES.NodicsError(
          "ERR_BOF_00000",
          "Functional-module eligibility query failed",
        );
      }
  ...
  getCode: function (project, functionalModule) {
    return (
      String(project) + "::" + this.normalizeFunctionalModule(functionalModule)
    );
  },
  ...
  getLegacyFunctionalModules: function (functionalModule) {
    ...
    return Object.keys(aliases).filter(
      (identity) => String(aliases[identity]) === canonical,
    );
  },
  ...
  normalizeActivationDataPackages: function (packages, ownerFallback) {
    ...
    [].concat(packages || []).forEach((item) => {
      ...
      let normalized = {
        ...
        trigger: String(
          item.trigger || (item.required === false ? "USER" : "ACTIVATION"),
        ),
      ...
      if (item.targetModule)
        normalized.targetModule = String(item.targetModule);
      if (item.targetServer)
        normalized.targetServer = String(item.targetServer);
      if (item.targetDatabase)
        normalized.targetDatabase = String(item.targetDatabase);
      if (normalized.code)
        byCode[this.getActivationPackageKey(normalized)] = normalized;
    });
  ...
  getActivationDataConfiguration: function () {
    return CONFIG.get("backofficeFunctionalModuleActivationData") || {};
  },
  ...
  getActivationDataPackages: function (functionalModule, record) {
    ...
    let configured = [].concat(
      (modules[this.normalizeFunctionalModule(functionalModule)] || {})
        .dataPackages || [],
    );
    let observed = [].concat((record && record.activationDataPackages) || []);
    let configuredCodes = new Set(configured.map((pack) => pack.code));
    configured = configured.flatMap((pack) => {
      const sources = observed.filter((item) => item.code === pack.code);
      if (sources.length)
        return sources.map((source) => Object.assign({}, source, pack));
      // A routing-only override cannot invent package facts before its owner registers.
      return pack.classification || pack.dataType || pack.operation
        ? [pack]
        : [];
    });
    return this.normalizeActivationDataPackages(
      observed
        .filter((pack) => !configuredCodes.has(pack.code))
        .concat(configured),
      functionalModule,
    );
  },
  ...
  getActivationDependencies: function (functionalModule) {
    ...
    return this.uniqueSorted(
      []
        ...
        .map((item) => this.normalizeFunctionalModule(item))
        .filter(Boolean),
    );
  },
  ...
  inferActivationDataType: function (item) {
    ...
    if (operation.includes("SAMPLE") || classification.includes("sample"))
      return "sample";
    if (operation.includes("INIT") || classification.includes("init"))
      return "init";
    return "core";
  },
  ...
  getReceiptKey: function (project, functionalModule, packageCode, pack) {
    let key = [
      String(project),
      this.normalizeFunctionalModule(functionalModule),
      String(packageCode),
    ].join("::");
    if (
      !pack ||
      ![pack.targetServer, pack.targetModule, pack.targetDatabase].some(Boolean)
    )
      return key;
    return (
      key +
      "::target::" +
      [pack.targetServer, pack.targetModule, pack.targetDatabase]
    ...
    return this.getAllRecords(
      {
        ...
        query: {
          ...
          functionalModule: this.normalizeFunctionalModule(functionalModule),
        },
      },
      true,
    );
  ...
  getActivationReceiptMap: async function (project, functionalModule, request) {
    let records = await this.getActivationReceipts(
      project,
      functionalModule,
      request,
    ).catch(() => []);
    return records.reduce((map, record) => {
      let key = this.getActivationPackageKey(
        Object.assign({}, record, { code: record.packageCode }),
      );
      if (
        !map[key] ||
        new Date(record.lastAttemptAt || 0) >=
          new Date(map[key].lastAttemptAt || 0)
      )
        map[key] = record;
      return map;
    }, {});
  ...
  upsertActivationReceipt: async function (
    record,
    pack,
    status,
    context,
    ...
    let authData = this.getPersistenceAuthData(
      context && context.request && context.request.authData,
    );
    let now = new Date();
    let receiptKey = this.getReceiptKey(
      record.projectCode,
      record.functionalModule,
      pack.code,
      pack,
    );
    let existing = await this.getReceiptRecords({
      tenant: tenant,
      authData: authData,
      query: { code: receiptKey },
      searchOptions: { pageSize: 1, pageNumber: 1 },
    })
      .then((response) => this.getItems(response)[0])
      .catch(() => undefined);
    let model = {
      code: receiptKey,
      active: true,
      ...
      targetModule: pack.targetModule,
      targetServer: pack.targetServer,
      targetDatabase: pack.targetDatabase,
      operation: pack.operation,
      dataType: pack.dataType,
      status: status,
      executionMode: details.executionMode || "NIMPORT_RELEASE",
      message: String(details.message || ""),
      importRunId: details.importRunId,
      releaseStatus: details.releaseStatus,
      revision: Number((existing && existing.revision) || 0) + 1,
      lastAttemptAt: now,
      updatedAt: now,
      updatedBy: (context && context.actor) || "activation-data-service",
      correlationId: context && context.correlationId,
    };
    if (existing)
      return this.updateReceiptRecord({
        tenant: tenant,
        authData: authData,
        query: { code: receiptKey },
        model: model,
      }).then(() => Object.assign({}, existing, model));
    return this.saveReceiptRecord({
      tenant: tenant,
      authData: authData,
      model: model,
    }).then(() => model);
  ...
  getFunctionalDependencyStates: async function (record, request) {
    ...
    for (let dependency of dependencies) {
      ...
      let dependencyRecord = await this.getRecord(
        record.projectCode,
        dependency,
        request,
      ).catch(() => {
      ...
      let state = {
        ...
        displayName:
          (dependencyRecord && dependencyRecord.displayName) || dependency,
        registrationState:
          (dependencyRecord && dependencyRecord.registrationState) ||
          (lookupFailed ? "UNKNOWN" : "UNAVAILABLE"),
        enabled: dependencyRecord && dependencyRecord.enabled === true,
        runtimeState:
          (dependencyRecord && dependencyRecord.runtimeState) || "OFFLINE",
        satisfied: Boolean(
          dependencyRecord &&
          dependencyRecord.registrationState === "REGISTERED" &&
          dependencyRecord.enabled === true &&
          dependencyRecord.runtimeState === "ACTIVE",
        ),
      };
      states.push(
        Object.assign(state, this.describeFunctionalDependency(state)),
      );
    }
    return states;
  },
  ...
  describeFunctionalDependency: function (state) {
    ...
    if (state.satisfied)
      return { reason: "Registered, activated, and running.", resolution: "" };
    if (state.registrationState === "UNKNOWN")
      return {
        reason: "The current status of " + name + " could not be checked.",
        resolution:
          "Retry the registry status check. If it still fails, ask an administrator to check the module registry.",
      };
    if (state.registrationState === "UNAVAILABLE")
      return {
        reason: name + " has not been discovered in this project.",
        resolution:
          "Start the runtime that provides " +
          name +
          ", then register and activate the module.",
      };
    let reasons = [];
    let actions = [];
    ...
    if (missing.length) {
      throw new CLASSES.NodicsError(
        ...
        "Functional module activation is blocked until required modules are active: " +
          missing
            .map((item) => item.displayName || item.functionalModule)
            .join(", "),
      );
    }
    return true;
  ...
  buildActivationDataPlan: function (record, action, context) {
    context = context || {};
    let packages = this.getActivationDataPackages(
      record.functionalModule,
      record,
    );
    ...
    let dependencies = this.getActivationDependencies(record.functionalModule);
    let missingDependencies = dependencyStates
      .filter((item) => item.satisfied !== true)
      .map((item) => item.functionalModule);
    let dryRun = context && context.dryRun === true;
    let blockedReasons = [];
    if (
      ["activate", "dryRun"].includes(action) &&
      record.registrationState !== "REGISTERED"
    )
      blockedReasons.push("MODULE_NOT_REGISTERED");
    if (
      ["activate", "dryRun"].includes(action) &&
      record.runtimeState !== "ACTIVE"
    )
      blockedReasons.push("RUNTIME_NOT_ACTIVE");
    if (["activate", "dryRun"].includes(action))
      missingDependencies.forEach((item) =>
        blockedReasons.push("MISSING_DEPENDENCY:" + item),
      );
    let receipts = packages.map((item) => {
      ...
      return Object.assign({}, item, {
        ...
        idempotent: true,
        executionMode:
          (persisted && persisted.executionMode) ||
          (item.required && item.trigger === "ACTIVATION"
            ? "NIMPORT_RELEASE"
            : "USER_TRIGGERED"),
        releaseStatus: persisted && persisted.releaseStatus,
        importRunId: persisted && persisted.importRunId,
        lastAttemptAt: persisted && persisted.lastAttemptAt,
    ...
    let failed = receipts.some(
      (item) => item.required && item.status === "FAILED",
    );
    let running = receipts.some(
      (item) =>
        item.required &&
        ["QUEUED", "RUNNING", "PENDING_IMPORT"].includes(item.status),
    );
    let requiredPending = receipts.some(
      (item) => item.required && item.status === "PENDING_IMPORT",
    );
    return {
      action: action,
      dryRun: dryRun,
      executionMode: receipts.some(
        (item) => item.executionMode === "NIMPORT_RELEASE",
      )
        ? "NIMPORT_RELEASE"
        : "USER_TRIGGERED",
      readiness:
        blockedReasons.length || missingDependencies.length
          ? "BLOCKED"
          : failed
            ? "DATA_FAILED"
            : running || requiredPending
              ? "DATA_RUNNING"
              : "READY",
      preflight: {
      ...
      receipts: receipts,
      nextActions: [].concat(
        failed ? ["RETRY_REQUIRED_DATA_IMPORT"] : [],
        receipts.some((item) => item.status === "SKIPPED_USER_TRIGGERED")
          ? ["OPTIONAL_SAMPLE_DATA_USER_ACTION"]
          : [],
  ...
  sameList: function (left, right) {
    return (
      JSON.stringify(this.uniqueSorted(left)) ===
      JSON.stringify(this.uniqueSorted(right))
    );
  },
  ...
  sameActivationDataPackages: function (left, right) {
    return (
      JSON.stringify(this.normalizeActivationDataPackages(left)) ===
  ...
  buildObservations: function (batch) {
    ...
    return roots.map((root) => {
      ...
      let technicalModules = registrations
        .filter(
          (item) =>
      ...
      let activationDataPackages = registrations
        .filter(
          (item) =>
            item === root ||
            this.resolveFunctionalRoot(item, registrationsByName) === root,
      ...
      return {
        ...
        activationDataPackages: this.normalizeActivationDataPackages(
  ...
  buildLeaseFunctionalModuleIndex: function (batch) {
    ...
    registrations.forEach((registration) => {
      ...
      if (root && root.functionalModule && root.functionalModule.identity) {
        index[registration.moduleName] = this.normalizeFunctionalModule(
          root.functionalModule.identity,
        );
      }
    });
  ...
  getRecordForExactIdentity: async function (
    project,
    functionalModule,
    request,
  ) {
    let tenant = this.getTenant(request);
    let response = await this.getRecords({
      tenant: tenant,
      authData: this.getPersistenceAuthData(request && request.authData),
      query: { code: String(project) + "::" + String(functionalModule) },
      searchOptions: { pageSize: 1, pageNumber: 1 },
    });
  ...
  getRecord: async function (project, functionalModule, request) {
    let canonical = this.normalizeFunctionalModule(functionalModule);
    let identities = [canonical].concat(
      this.getLegacyFunctionalModules(canonical),
    );
    for (let identity of identities) {
      let record = await this.getRecordForExactIdentity(
        project,
        identity,
        request,
      );
  ...
  retireDuplicateLegacyRecords: async function (
    project,
    functionalModule,
    request,
    canonicalRecord,
    ...
    if (
      !canonicalRecord ||
      canonicalRecord.code !== this.getCode(project, functionalModule)
    )
      return 0;
    let tenant = this.getTenant(request);
    ...
    for (let identity of this.getLegacyFunctionalModules(functionalModule)) {
      let legacy = await this.getRecordForExactIdentity(project, identity, {
        tenant: tenant,
        authData: authData,
      });
      if (
        !legacy ||
        legacy.code === canonicalRecord.code ||
        (legacy.registrationState === "DEREGISTERED" &&
          legacy.functionalModule === functionalModule)
      )
        continue;
      await this.updateRecord({
        tenant: tenant,
        authData: authData,
        query: { code: legacy.code },
        model: {
          functionalModule: this.normalizeFunctionalModule(functionalModule),
          ...
          updatedAt: new Date(),
          updatedBy: "functional-identity-migrator",
        },
      });
      retired++;
    }
  ...
  reconcileObservation: async function (observation, request, attempt) {
    ...
    let existing = await this.getRecord(
      ...
      observation.functionalModule,
      { tenant: tenant, authData: authData },
    );
    observation = Object.assign({}, observation, {
      functionalModule: this.normalizeFunctionalModule(
        observation.functionalModule,
      ),
    });
    let now = new Date();
      ...
      let model = Object.assign(
        {
          ...
          enabled: observation.required,
          runtimeState: "ACTIVE",
          observedServers: [observation.observedServer],
          catalogueRevision: 1,
          registeredAt: now,
          registeredBy: "runtime-reconciler",
          updatedAt: now,
          updatedBy: "runtime-reconciler",
          lastObservedAt: now,
        },
        observation,
      );
    ...
    await this.retireDuplicateLegacyRecords(
      observation.projectCode,
      observation.functionalModule,
      { tenant: tenant, authData: authData },
      existing,
    ...
    observation.technicalModules = this.uniqueSorted(
      [].concat(
        existing.technicalModules || [],
        observation.technicalModules || [],
      ),
    );
    observation.activationDataPackages = this.normalizeActivationDataPackages(
      [].concat(
        existing.activationDataPackages || [],
        observation.activationDataPackages || [],
      ),
      observation.functionalModule,
    );
    let observedServers = this.uniqueSorted(
      (existing.observedServers || []).concat([observation.observedServer]),
    );
    let desiredRegistrationState = observation.required
      ? "REGISTERED"
      : existing.registrationState === "DEREGISTERED"
        ? "AVAILABLE"
        : existing.registrationState;
    let desiredEnabled = observation.required
      ? true
      : existing.enabled === true;
    let canonicalCode = this.getCode(
      observation.projectCode,
      observation.functionalModule,
    );
    let changed =
      existing.code !== canonicalCode ||
      existing.functionalModule !== observation.functionalModule ||
      existing.registrationState !== desiredRegistrationState ||
      existing.enabled !== desiredEnabled ||
      existing.runtimeState !== "ACTIVE" ||
      existing.registeredVersion !== observation.registeredVersion ||
      existing.displayName !== observation.displayName ||
      existing.moduleIndex !== observation.moduleIndex ||
      existing.required !== observation.required ||
      !this.sameList(existing.technicalModules, observation.technicalModules) ||
      !this.sameActivationDataPackages(
        existing.activationDataPackages,
        observation.activationDataPackages,
      ) ||
    ...
    let model = {
      code: canonicalCode,
      functionalModule: observation.functionalModule,
      runtimeState: "ACTIVE",
      displayName: observation.displayName,
      registeredVersion: observation.registeredVersion,
      moduleIndex: observation.moduleIndex,
      required: observation.required,
      technicalModules: observation.technicalModules,
      ...
      updatedBy: changed ? "runtime-reconciler" : existing.updatedBy,
      lastObservedAt: now,
    };
    if (decisionChanged)
      Object.assign(model, {
    ...
    let response = await this.updateRecord({
      tenant: tenant,
      authData: authData,
      query: {
        code: existing.code,
        catalogueRevision: existing.catalogueRevision,
      },
      model: model,
    });
    if (this.getMatchedCount(response) !== 1) {
      if ((attempt || 0) < 2)
        return this.reconcileObservation(incoming, request, (attempt || 0) + 1);
      throw new CLASSES.NodicsError(
        "ERR_BOF_00000",
        "Functional-module observation changed concurrently; retry registration",
      );
    }
  ...
  reconcileActiveRuntimeLeases: async function (
    activeLeases,
    affectedProjects,
    request,
  ) {
    ...
    let projects = new Set(
      (affectedProjects || []).filter(Boolean).map(String),
    );
    activeLeases.forEach((lease) => {
      if (lease.projectCode) projects.add(String(lease.projectCode));
    });
    let tenant = this.getTenant(request);
    let authData = this.getPersistenceAuthData(request && request.authData);
    let changed = 0;
    for (let project of projects) {
      ...
      for (let record of records) {
        let matching = activeLeases.filter(
          (lease) =>
            lease.projectCode === project &&
        ...
        let technicalModules = this.uniqueSorted(
          matching
            .map((lease) => lease.moduleName)
        ...
        if (!matching.some((lease) => lease.moduleName))
          technicalModules = record.technicalModules || [];
        if (
          record.runtimeState === runtimeState &&
          this.sameList(record.observedServers, observedServers) &&
          this.sameList(record.technicalModules, technicalModules)
        )
          continue;
        let model = {
          ...
          updatedAt: new Date(),
          updatedBy: "runtime-lease-reconciler",
        };
        let response = await this.updateRecord({
          tenant: tenant,
          authData: authData,
          query: {
            code: record.code,
            catalogueRevision: record.catalogueRevision,
          },
          model: model,
        });
  ...
  projectClientSafe: function (record, options) {
    ...
    return {
      ...
      activationData: this.buildActivationDataPlan(
        record,
  ...
  projectClientSafeWithReceipts: async function (record, options, request) {
    let receiptMap = await this.getActivationReceiptMap(
      record.projectCode,
      record.functionalModule,
      request,
    );
    let dependencyStates = await this.getFunctionalDependencyStates(
      record,
      request,
    );
    return this.projectClientSafe(
      record,
  ...
  getBody: function (request) {
    return (
      (request &&
        (request.body || (request.httpRequest && request.httpRequest.body))) ||
      {}
    );
  },
  ...
  listByState: async function (request, registrationState, code) {
    let project =
      (request && request.project) || this.getQuery(request).project;
    if (!project)
      throw new CLASSES.NodicsError(
        "ERR_BOF_00000",
    ...
    let records = await this.getAllRecords({
      tenant: tenant,
      authData: this.getPersistenceAuthData(request.authData),
  ...
  listAvailable: function (request) {
    return this.listByState(request, "AVAILABLE", "SUC_BOF_00017");
  },
  ...
  getPresentationEligibility: async function (request) {
    ...
    return {
      project: project,
      governedModules: Array.from(governed).sort(),
      eligibleModules: Array.from(eligible).sort(),
    };
  },
  ...
  detail: async function (request) {
    ...
    return {
      code: "SUC_BOF_00019",
      data: await this.projectClientSafeWithReceipts(record, {}, request),
    };
  },
  ...
  getLifecycleContext: function (request) {
    ...
    if (
      !body.project ||
      !params.functionalModule ||
      !Number.isInteger(Number(body.expectedRevision)) ||
      ...
      throw new CLASSES.NodicsError(
        "ERR_BOF_00000",
        "Invalid functional-module lifecycle request",
      );
    }
    return {
      project: String(body.project),
      functionalModule: String(params.functionalModule),
      expectedRevision: Number(body.expectedRevision),
      reason: String(body.reason),
      actor: actor,
      dryRun: body.dryRun === true,
      includeActivationData: body.includeActivationData !== false,
  ...
  transition: async function (request, action) {
    ...
    if (context.dryRun === true) {
      return {
        code: "SUC_BOF_00020",
        data: await this.projectClientSafeWithReceipts(
          existing,
          {
            action: "dryRun",
            dryRun: true,
    ...
    if (
      ...
      ["deactivate", "deregister", "rollback"].includes(action)
    ) {
      throw new CLASSES.NodicsError(
        "ERR_BOF_00000",
        "Required functional module cannot be " + action + "d",
      );
    }
    let next = {};
    if (action === "register") {
      if (!["AVAILABLE", "DEREGISTERED"].includes(existing.registrationState)) {
        throw new CLASSES.NodicsError(
          "ERR_BOF_00000",
          "Functional module is already registered",
        );
      }
      next = { registrationState: "REGISTERED", enabled: false };
    } else if (action === "activate") {
      if (existing.registrationState !== "REGISTERED")
        throw new CLASSES.NodicsError(
          "ERR_BOF_00000",
          "Functional module must be registered before activation",
        );
      if (existing.runtimeState !== "ACTIVE")
        throw new CLASSES.NodicsError(
          "ERR_BOF_00000",
          "Functional module has no active compatible runtime",
        );
      ...
      if (existing.registrationState !== "REGISTERED")
        throw new CLASSES.NodicsError(
          "ERR_BOF_00000",
          "Functional module is not registered",
        );
      next = { enabled: false };
    } else if (action === "deregister") {
      next = {
        registrationState:
          existing.runtimeState === "ACTIVE" ? "AVAILABLE" : "DEREGISTERED",
        enabled: false,
      };
      ...
      if (existing.registrationState !== "REGISTERED")
        throw new CLASSES.NodicsError(
          "ERR_BOF_00000",
          "Functional module must be registered before rollback",
        );
      if (existing.enabled !== true)
        throw new CLASSES.NodicsError(
          "ERR_BOF_00000",
          "Functional module activation is not active",
        );
      next = { enabled: false };
    } else {
    ...
    if (action === "activate")
      await this.executeRequiredActivationData(existing, context, request);
    if (["deactivate", "deregister", "rollback"].includes(action))
      await this.recordDataLeftIntact(existing, context, request);
    let model = Object.assign({}, next, {
      catalogueRevision: context.expectedRevision + 1,
      updatedAt: new Date(),
      updatedBy: context.actor,
    });
    let query = {
      code: existing.code,
      catalogueRevision: context.expectedRevision,
    };
    ...
    if (this.getAffectedCount(response) !== 1)
      throw new CLASSES.NodicsError(
        "ERR_BOF_00000",
        "Functional-module catalogue revision conflict",
      );
    ...
    if (SERVICE.DefaultBackofficeAuditService)
      await SERVICE.DefaultBackofficeAuditService.record({
        eventType: "backoffice.functional-module." + action,
        outcome: "completed",
        project: context.project,
        moduleName: context.functionalModule,
        principalId: context.actor,
        revision: model.catalogueRevision,
  ...
  executeRequiredActivationData: async function (record, context, request) {
    ...
    for (let groupKey of Object.keys(grouped)) {
      ...
      await Promise.all(
        packs.map((pack) =>
          this.upsertActivationReceipt(
            ...
            {
              message:
                "Required activation data import is running through nImport.",
      ...
      try {
        let releaseRequest = {
          dataType: dataType,
          releaseCodes: packs.map((pack) => pack.code),
        };
        let preflight = await this.runActivationDataReleaseOperation(
          "preflight",
          releaseRequest,
          packs[0],
          request,
        ...
        let releases =
          (preflight && preflight.data && preflight.data.releases) || [];
        const preflightByCode = Object.fromEntries(
        ...
        if (pending.length) {
          await this.recordActivationDataCompletion(
            record,
            packs,
            preflightByCode,
            context,
        ...
        const executable = packs.filter((pack) =>
          ["NOT_INSTALLED", "UPDATE_AVAILABLE", "FAILED"].includes(
            (preflightByCode[pack.code] || {}).status,
          ),
        );
        const result =
          executable.length === 0
            ? preflight
            : await this.runActivationDataReleaseOperation(
                "execute",
                Object.assign({}, releaseRequest, {
                  releaseCodes: executable.map((pack) => pack.code),
                }),
                packs[0],
                request,
              );
        const releaseByCode = Object.assign(
          {},
          preflightByCode,
          Object.fromEntries(
            ((result && result.data && result.data.releases) || []).map(
              (release) => [release.releaseCode, release],
            ),
          ),
        );
        const importRunId =
          result &&
          result.data &&
          result.data.importRun &&
          result.data.importRun.runId;
        await this.recordActivationDataCompletion(
          record,
          packs,
          releaseByCode,
          ...
          packs.map((pack) =>
            this.upsertActivationReceipt(
              ...
              Object.assign({}, context, { request: request }),
              {
                message:
                  (error && error.message) ||
                  "Required activation data import failed.",
   ...
   */
  recordActivationDataCompletion: async function (
    record,
    packs,
    releases,
    context,
    ...
    await Promise.all(
      packs.map((pack) => {
        ...
        const running = ["RUNNING", "QUEUED", "PENDING_IMPORT"].includes(
          release.status,
        );
        if (!current) incomplete.push(pack.code);
        return this.upsertActivationReceipt(
          record,
          pack,
          current ? "IMPORTED" : running ? "RUNNING" : "FAILED",
          Object.assign({}, context, { request }),
          {
            message: current
              ? "Required activation data release is confirmed current."
    ...
    if (incomplete.length) {
      const error = new CLASSES.NodicsError(
        "ERR_BOF_00000",
        "Required activation data is incomplete: " + incomplete.join(", "),
      );
  ...
  runActivationDataReleaseOperation: async function (
    mode,
    releaseRequest,
    pack,
    request,
    ...
    if (this.isLocalActivationDataTarget(pack)) {
      ...
      if (
        ...
        throw new CLASSES.NodicsError(
          "ERR_BOF_00000",
          "nImport data-release executor is unavailable for required activation data",
        );
      }
      ...
      return mode === "preflight"
        ? releaseService.preflight(operationRequest)
        : releaseService.execute(operationRequest);
    }
    return this.runRemoteActivationDataReleaseOperation(
      mode,
      releaseRequest,
      pack,
      request,
  ...
  isLocalActivationDataTarget: function (pack) {
    ...
    let currentServer =
      ...
      NODICS.getServerName();
    return Boolean(
      currentServer && String(targetServer) === String(currentServer),
    );
  },
  ...
  buildActivationDataReleaseUrl: function (owner, dataType, mode) {
    ...
    let contextRoot =
      pathParts[0] ||
      (configured.options && configured.options.contextRoot) ||
      "nodics";
    let suffix = mode === "preflight" ? "/validate" : "/install";
    return (
      endpoint.origin + "/" + contextRoot + "/import/v0/" + dataType + suffix
    );
  },
  /** Calls the target runtime nImport route with an internal service token. */
  runRemoteActivationDataReleaseOperation: async function (
    mode,
    releaseRequest,
    pack,
    request,
    ...
    if (
      !moduleService ||
      typeof moduleService.buildExternalRequest !== "function" ||
      typeof moduleService.fetch !== "function"
    ) {
      throw new CLASSES.NodicsError(
        "ERR_BOF_00000",
        "Module transport is unavailable for activation data",
      );
    }
    let uri = this.buildActivationDataReleaseUrl(
      owner,
      releaseRequest.dataType,
      mode,
    );
    let operationRequest = moduleService.buildExternalRequest({
      ...
      timeoutMs: Number(
        (CONFIG.get("backofficeFunctionalModuleActivationData") || {})
          .timeoutMs || 30000,
      ),
      maxAttempts: 1,
      idempotencyKey:
        request &&
        (request.correlationId || (request.body && request.body.correlationId)),
    });
    ...
    await Promise.all(
      packages.map((pack) =>
        this.upsertActivationReceipt(
          ...
          {
            executionMode:
              pack.required && pack.trigger === "ACTIVATION"
                ? "NIMPORT_RELEASE"
                : "USER_TRIGGERED",
            message:
              "Capability visibility changed; imported data was left intact for audit, rollback, or reactivation.",
          },
        ),
      ),
    );
  ...
  register: function (request) {
    return this.transition(request, "register");
  },
