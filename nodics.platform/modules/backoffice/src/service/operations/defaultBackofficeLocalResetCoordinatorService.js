...
module.exports = {
  ...
  /** Executes the documented bounded module operation. */
  policy: function () {
    return CONFIG.get("backofficeLocalReset") || {};
  },
  /** Executes the documented bounded module operation. */
  ...
  environment: function () {
    return (
      ...
      (NODICS.getSelectedEnvironmentName
        ? NODICS.getSelectedEnvironmentName()
        : NODICS.getEnvironmentName && NODICS.getEnvironmentName())
    );
  },
  /** Executes the documented bounded module operation. */
  environmentAllowed: function () {
    return []
      .concat(this.policy().environmentAllowlist || [])
  ...
  providers: function () {
    ...
    if (providers.length > Number(policy.maximumTargets || 16))
      throw new CLASSES.NodicsError(
        "ERR_BOF_00090",
        "Local reset target boundary exceeded",
      );
    return providers.map((provider) => {
      provider = Object.assign({}, policy.providerDefaults || {}, provider);
      if (
        !provider ||
        !/^[a-z][A-Za-z0-9]{1,63}$/.test(String(provider.code || "")) ||
        !/^[a-z][A-Za-z0-9]{1,63}$/.test(String(provider.moduleName || "")) ||
        !/^[a-z][A-Za-z0-9]{1,63}$/.test(String(provider.connectionName || ""))
      ) {
        throw new CLASSES.NodicsError(
          "ERR_BOF_00091",
          "Local reset provider is unavailable",
        );
      }
      return provider;
    });
  ...
  invokeProvider: function (provider, request, input) {
    ...
    if (!token)
      throw new CLASSES.NodicsError(
        "ERR_BOF_00097",
        "Local reset service authentication is unavailable",
      );
    return SERVICE.DefaultModuleService.invokeModule({
      ...
      apiName: "/operations/local-reset",
      local: false,
      timeoutMs: provider.timeoutMs || 30000,
      maxAttempts: 1,
      requestBody: {
        confirmation: this.policy().confirmation,
  ...
  status: async function () {
    ...
    return {
      ...
      ready:
        policy.enabled === true && environmentAllowed && configured.length > 0,
      reason:
        policy.enabled !== true
  ...
  execute: async function (request) {
    ...
    if (policy.enabled !== true)
      throw new CLASSES.NodicsError("ERR_BOF_00092", "Local reset is disabled");
    if (!this.environmentAllowed())
      throw new CLASSES.NodicsError(
        "ERR_BOF_00096",
        "Local reset is not allowed in this environment",
      );
    if (
      !principal ||
      ...
      throw new CLASSES.NodicsError(
        "ERR_BOF_00093",
        "Local reset requires a human administrator",
      );
    if (
      input.confirmation !== policy.confirmation ||
      String(input.reason || "").trim().length < 8
    ) {
      throw new CLASSES.NodicsError(
        "ERR_BOF_00094",
        "Local reset confirmation and reason are required",
      );
    }
    ...
    if (!providers.length)
      throw new CLASSES.NodicsError(
        "ERR_BOF_00091",
        "Local reset providers are not configured",
      );
    let results = [];
    for (let provider of providers) {
      ...
      if (!result || result.acknowledged !== true)
        throw new CLASSES.NodicsError(
          "ERR_BOF_00095",
          "Local reset provider did not acknowledge completion",
        );
