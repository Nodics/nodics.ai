...
module.exports = {
    ...
    preparePrincipalUpdate: function (request) {
        ...
        return service.get({
            ...
            const minimum = principals.reduce((value, principal) => Math.max(value, Number(principal.authVersion || 0) + 1), 1);
            let version = await SERVICE.DefaultPrincipalSecurityStampService.reserveVersion(request.tenant, minimum);
            if (!Number.isInteger(version) || version > 2147483647) throw new CLASSES.NodicsError('ERR_AUTH_00003', 'Principal security stamp exceeds persisted integer range');
    ...
    bumpLoginId: function (request) {
        ...
        return Promise.all([find(SERVICE.DefaultEmployeeService), find(SERVICE.DefaultCustomerService)]).then(results => {
            ...
            let match = matches[0];
            return match.service.update({ tenant: request.tenant, authData: system, query: { loginId: match.principal.loginId }, model: { $set: { authVersion: 1 } } });
        }).then(() => true);
    },
