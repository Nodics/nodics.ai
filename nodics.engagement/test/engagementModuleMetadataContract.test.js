 ...
 * @description Verifies the Customer Engagement ownership and dormant-capability metadata contract.
 * @layer test
...
assert.deepStrictEqual(engagementNavigation.map(item => item.id), ['customer-engagement', 'reviews-reputation', 'testimonials-advocacy', 'engagement-insights', 'contact-submissions', 'contact-handoffs', 'testimonial-candidates', 'testimonial-editorial', 'testimonial-consents', 'testimonial-publications', 'customer-reviews', 'review-moderation', 'review-responses', 'review-abuse', 'review-publications', 'review-aggregates', 'review-requests', 'review-syndication', 'customer-feedback', 'feedback-complaints', 'feedback-follow-up', 'feedback-surveys', 'feedback-insights', 'engagement-unified-queue', 'engagement-dashboards', 'engagement-repairs', 'engagement-exports', 'engagement-privacy', 'engagement-automation-decisions', 'engagement-automation-evaluations', 'engagement-delivery-attempts', 'engagement-recovery-checkpoints', 'engagement-compatibility']);
assert.deepStrictEqual(engagementNavigation.slice(0, 4).map(item => item.workbenchTarget), [undefined, undefined, undefined, undefined]);
assert.deepStrictEqual(engagementNavigation[4].workbenchTarget, { moduleName: 'contactSubmission', schemaName: 'contactRequest' });
assert(engagementNavigation[4].lifecycleActions.every(action => action.ownerModule === 'engagementApi' && action.operationRoute.startsWith('/operator/contact-submissions/')));
assert.deepStrictEqual(engagementNavigation[5].workbenchTarget, { moduleName: 'contactSubmission', schemaName: 'contactHandoff' });
assert(engagementNavigation[5].lifecycleActions.every(action => action.ownerModule === 'engagementApi' && action.operationRoute.startsWith('/operator/handoffs/')));
assert.deepStrictEqual(engagementNavigation.slice(6, 10).map(item => item.workbenchTarget.schemaName), ['testimonialCandidate', 'testimonialVersion', 'testimonialConsent', 'testimonialProjection']);
assert(engagementNavigation[9].lifecycleActions.every(action => action.ownerModule === 'engagementApi' && action.operationRoute.startsWith('/operator/testimonials/')));
assert.deepStrictEqual(engagementNavigation.slice(10, 18).map(item => item.workbenchTarget.schemaName), ['customerReview', 'customerReviewModeration', 'customerReviewResponse', 'customerReviewAbuseReport', 'customerReviewProjection', 'customerReviewAggregate', 'customerReviewRequest', 'customerReviewSyndication']);
assert(engagementNavigation[10].lifecycleActions.every(action => action.ownerModule === 'engagementApi' && action.operationRoute.startsWith('/operator/reviews/')));
assert.deepStrictEqual(engagementNavigation.slice(18, 23).map(item => item.workbenchTarget.schemaName), ['customerFeedback', 'customerFeedback', 'customerFeedbackFollowUp', 'engagementFormDefinition', 'customerFeedbackInsight']);
assert(engagementNavigation[18].lifecycleActions.every(action => action.ownerModule === 'engagementApi' && action.operationRoute.startsWith('/operator/feedback/')));
assert.deepStrictEqual(engagementNavigation.slice(23).map(item => item.workbenchTarget.schemaName), ['engagementUnifiedQueueItem', 'engagementDashboardSnapshot', 'engagementRepairCase', 'engagementExportEvidence', 'engagementPrivacyCase', 'engagementAutomationDecision', 'engagementAutomationEvaluation', 'engagementDeliveryAttempt', 'engagementRecoveryCheckpoint', 'engagementCompatibilityRecord']);
assert(engagementNavigation.slice(23).every(item => item.workbenchTarget.moduleName === 'engagementCore'));
