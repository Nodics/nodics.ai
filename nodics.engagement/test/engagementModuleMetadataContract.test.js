...
assert.strictEqual(properties.backofficeCapabilities['nodics.engagement'].enabled, true);
assert.strictEqual(backofficeContract.validateBackofficeMetadata(properties.backofficeCapabilities['nodics.engagement']), true, 'Engagement metadata must be accepted by the live BackOffice registration contract');
const engagementNavigation = properties.backofficeCapabilities['nodics.engagement'].navigation;
assert.deepStrictEqual(engagementNavigation.map(item => item.id), ['customer-engagement', 'contact-submissions', 'contact-handoffs', 'testimonial-candidates', 'testimonial-editorial', 'testimonial-consents', 'testimonial-publications', 'customer-reviews', 'review-moderation', 'review-responses', 'review-abuse', 'review-publications', 'review-aggregates', 'review-requests', 'review-syndication', 'customer-feedback', 'feedback-complaints', 'feedback-follow-up', 'feedback-surveys', 'feedback-insights', 'engagement-unified-queue', 'engagement-dashboards', 'engagement-repairs', 'engagement-exports', 'engagement-automation-decisions', 'engagement-automation-evaluations', 'engagement-delivery-attempts', 'engagement-recovery-checkpoints', 'engagement-compatibility']);
