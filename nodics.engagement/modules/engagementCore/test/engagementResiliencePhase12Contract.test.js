...
assert.strictEqual(service.compatibility({ tenant: 't1', contractType: 'API', contractCode: 'reviews', version: '1.0.0', evidence: {}, correlationId: 'c1' }, policy).compatibility, 'CURRENT');
assert.strictEqual(service.compatibility({ tenant: 't1', contractType: 'API', contractCode: 'reviews', version: '0.0.0', evidence: {}, correlationId: 'c1' }, policy).compatibility, 'BREAKING');
assert.throws(() => service.compatibility({ version: '0.0.0', deprecatedAt: '2026-01-01', sunsetAt: '2026-02-01' }, policy), /deprecation window/);
console.log('Engagement enterprise resilience contract validated');
