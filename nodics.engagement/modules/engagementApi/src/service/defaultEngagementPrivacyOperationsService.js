...
module.exports = {
    ...
    execute: async function (request) { let domain = String(request.domainType || '').toUpperCase(); let service = { CONTACT: SERVICE.DefaultContactSubmissionApiService, FEEDBACK: SERVICE.DefaultCustomerFeedbackApiService, REVIEW: SERVICE.DefaultCustomerReviewApiService, TESTIMONIAL: SERVICE.DefaultTestimonialApiService }[domain]; if (!service || typeof service.privacy !== 'function') throw Object.assign(new Error('privacy domain is unsupported'), { code: 'ERR_ENG_PRIVACY_DOMAIN' }); let output = await service.privacy(request); let command = Object.assign({}, request.payload, { actorId: request.authData && (request.authData.principalId || request.authData.code), domainType: domain, correlationId: request.correlationId || request.requestId }); let evidence = SERVICE.DefaultEngagementPrivacyService.evidence(output.record, request.payload.operation, command, output.result); let saved = await SERVICE.DefaultEngagementOperationsEvidenceRepositoryService.savePrivacy(evidence); return Object.assign({}, evidence, { code: saved.code, data: request.payload.operation === 'EXPORT' ? output.result.data : undefined }); }
};
