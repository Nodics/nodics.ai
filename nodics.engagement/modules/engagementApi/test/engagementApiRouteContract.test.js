...
assert.strictEqual(routes.length, 45);
assert.strictEqual(packageJson.prefix, 'engagement');
routes.forEach(([name, route]) => {
    if (!['getActiveForm', 'submitContact', 'listTestimonials', 'submitTestimonialCandidate', 'listReviews', 'getReviewAggregate', 'submitFeedback'].includes(name)) assert.strictEqual(route.secured, true, name + ' must be secured');
...
assert.strictEqual(routers.publicForms.submitTestimonialCandidate.secured, false);
