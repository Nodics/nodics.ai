...
/** @module contactSubmission/src/service/defaultContactSubmissionOperationService @description Implements the contact vertical slice behind the Engagement gateway. @layer service @owner contactSubmission @override Later modules may replace repositories and adapters while preserving Core intake and security contracts. */
module.exports = {
    ...
    /** Executes a purpose-bound privacy operation on one contact request. */ privacy: async function (request) { let repository = SERVICE.DefaultContactRequestRepositoryService; let record = await repository.get(Object.assign({}, request, { submissionCode: request.domainCode })); if (!record) { let error = new Error('contact request not found'); error.code = 'ERR_CONTACT_00001'; throw error; } let command = Object.assign({}, request.payload, { actorId: request.authData && (request.authData.principalId || request.authData.code), domainType: 'CONTACT', correlationId: request.correlationId }); if (request.payload.operation === 'EXPORT') return { record: record, result: SERVICE.DefaultEngagementPrivacyService.exportRecord(record, command, ['code', 'type', 'subject', 'message', 'status', 'submittedAt', 'resolvedAt', 'closedAt']) }; let result = SERVICE.DefaultEngagementPrivacyService.anonymize(record, command, { redactFields: ['subject', 'message', 'contactEmail', 'contactPhone'], clearArrayFields: ['attachmentCodes'] }); if (result.changed) result.record = await repository.save(request, result.record); return { record: record, result: result }; }
};
