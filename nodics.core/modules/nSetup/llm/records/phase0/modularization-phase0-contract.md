# Nodics Modularization Phase 0 Record

This is a historical Phase 0 modularization record. It preserves the decisions
and checklist that guided the initial framework split. Do not add new permanent
coding rules here. Promote durable rules into `contracts/` or `standards/`.

## Non-negotiable boundaries

- `nodics.ai` is the Nodics backend/framework repository root.
- `nodics.ai` owns the framework repository/module-group boundary, workspace
  governance, and release governance.
- Canonical repository-wide AI/developer contracts, principles, examples, and
  setup guidance live under `nodics.core/modules/nSetup/llm/`. Do not create a
  repository-root `llm/` folder as a parallel authority.
...
- README files are concise human-facing overviews. They must not become LLM
  behavior contracts; use AGENTS and nSetup/module-local `llm/contracts` for
  that.
...
- module-owned `llm/` for module contracts, examples, and generated context
  when applicable
...
- `nodics.docs` owns governed documentation content for the Nodics framework.
- `nodics.platform/modules/axis` owns Axis/BackOffice product documentation
  content that is imported into WCMS.
- `nodics.axis` is a separate frontend/renderer application and must not own
  backend documentation business data.
- Customer projects such as `nodics.kickoff` own project-specific docs/data
  describing their setup, customizations, sample flows, and runtime topology.
