...
`modules/nDatabase/database/vDatabase` owns the provider-neutral versioned
schema layer:
...
`modules/nDatabase/mongodb/vMongodb` owns MongoDB-specific versioned
...
`modules/nService/vService` owns service-pipeline routing for version-aware
...
`modules/nPublish` owns generic publishing contracts that should not be tied
