...
| Backend data ownership | Frontend Axis began to look like it owned CMS, documentation, catalog, page, or component records. | Backend-importable records belong in backend modules or backend content packages. Axis owns rendering and interaction, not persisted backend data. |
| Documentation ownership | Framework, Axis, and customer project documentation were mixed. | Framework documentation belongs in `nodics.docs`; Axis-specific backend documentation belongs in the platform Axis backend module; customer project documentation belongs in the customer backend project such as `nodics.kickoff`. |
