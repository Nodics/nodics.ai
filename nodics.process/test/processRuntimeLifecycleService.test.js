...
const graph = {
    nodes: [
        ...
        },
        {
            code: 'waitForAudit',
            type: 'TIMER',
            name: 'Wait for audit window',
            timer: { delayMs: 0, autoContinue: true },
        },
        {
            code: 'childGovernance',
            type: 'SUB_PROCESS',
            name: 'Child Governance',
            subProcessDefinitionCode: 'childApproval',
        },
...
const definitions = [
    { code: 'contentApproval', name: 'Content Approval', status: 'PUBLISHED', currentVersion: 1 },
];
const versions = [
    {
        code: 'contentApproval_v1',
...
global.SERVICE = {
    ...
    DefaultModuleRegistrationAgentService: {
        assertModuleOperational: async (moduleName, tenant) => {
            assert.strictEqual(moduleName, 'workflow');
            assert(tenant);
        },
    },
...
(async function run() {
    ...
    assert.strictEqual(
        auditEvents.some(
            (event) => event.eventType === 'process.node.entered' && event.metadata.nodeCode === 'review',
        ),
        true,
    );
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.task.created'),
        true,
    );
    ...
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.task.completed'),
        true,
    );
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.decision.evaluated'),
        true,
    );
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.action.executed'),
        true,
    );
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.timer.observed'),
        true,
    );
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.subProcess.referenced'),
        true,
    );
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.instance.completed'),
        true,
    );
    ...
    let multiCompleted = await runtimeService.completeTask({
        ...
        taskCode: multi.data.task.code,
        runtimeOperation: {
            decision: { approved: true, approvals: [{ actor: 'reviewer-one' }, { actor: 'reviewer-two' }] },
        },
    });
    ...
    let emergencyCompleted = await runtimeService.completeTask({
        ...
        taskCode: emergency.data.task.code,
        runtimeOperation: {
            decision: { approved: true, emergencyOverride: true, reason: 'Production recovery' },
        },
    });
    assert.strictEqual(emergencyCompleted.code, 'SUC_PROCESS_00008');

    let detail = await runtimeService.getInstanceDetail({
        tenant: 'default',
        instanceCode: 'contentApproval-001',
    });
    ...
    let listedTriggers = await runtimeService.listTriggers({
        tenant: 'default',
        query: { definitionCode: 'contentApproval' },
    });
    ...
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.trigger.created'),
        true,
    );
    ...
    assert.strictEqual(
        instances.some((instance) => instance.code === 'contentApproval-triggered-001'),
        true,
    );
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.trigger.execution.requested'),
        true,
    );
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.trigger.execution.completed'),
        true,
    );
        ...
        graph: {
            nodes: [
                { code: 'start', type: 'START' },
                {
                    code: 'unsafe',
                    type: 'ACTION',
                    action: { moduleName: 'commerce', operation: 'chargeCustomer' },
                    retry: { maximumAttempts: 3 },
                    compensation: { moduleName: 'nodics.process', operation: 'noop' },
                },
                { code: 'end', type: 'END' },
            ],
            transitions: [
    ...
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.incident.opened'),
        true,
    );
    ...
    versions
        .find((version) => version.definitionCode === 'unsafeActionProcess')
        .graph.nodes.find((node) => node.code === 'unsafe').action = {
    ...
    assert.strictEqual(
        auditEvents.some((event) => event.eventType === 'process.incident.resolved'),
        true,
    );
